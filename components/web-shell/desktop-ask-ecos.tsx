...
type DesktopAskRequestState = Readonly<{
  generation: number;
  projectId: string;
  projectName: string;
  question: string;
...
export function DesktopAskECOSWorkspace({
  ...
  const [requestState, setRequestState] = useState<DesktopAskRequestState | null>(null);
  const requestGenerationRef = useRef(0);
  const mountedRef = useRef(true);
  const contextKey = `${projectId || ''}\u0000${normalizeProjectName(projectName)}`;
  const contextKeyRef = useRef(contextKey);
  contextKeyRef.current = contextKey;
  const currentRequest = requestState?.projectId === projectId &&
    normalizeProjectName(requestState?.projectName) === normalizeProjectName(projectName)
    ? requestState
    : null;
  const answer = currentRequest?.answer || null;
  ...
  const replaceVisibleQuestion = (value: string) => {
    // The text shown beside an answer is part of the request identity. Editing
    // it invalidates any pending or completed response for the prior text.
    requestGenerationRef.current += 1;
    setRequestState(null);
    setQuestion(value);
  };
  ...
  const submit = async (nextQuestion = question) => {
    ...
    const capturedProjectName = projectName.trim();
    ...
    try {
      ...
      if (!desktopAskCompletionIsCurrent({
      ...
      setRequestState(current => current?.generation === generation &&
        current.projectId === capturedProjectId &&
        normalizeProjectName(current.projectName) === normalizeProjectName(capturedProjectName) &&
        current.question === cleanQuestion
        ? { ...current, answer: nextAnswer, error: null, loading: false }
        : current);
    } catch (reason) {
      if (!desktopAskCompletionIsCurrent({
        mounted: mountedRef.current,
      ...
      setRequestState(current => current?.generation === generation &&
        ...
        ? {
            ...current,
            answer: null,
            error: reason instanceof Error ? reason.message : 'Ask ECOS could not complete the question.',
            loading: false,
          }
        : current);
    }
    ...
    <View style={styles.workspace}>
      ...
      {!projectId || !projectName ? (
        ...
        <View style={styles.questionCard}>
          ...
          <TextInput
            value={question}
            onChangeText={replaceVisibleQuestion}
          ...
          <View style={styles.examples}>
            {EXAMPLE_QUESTIONS.map(example => (
              <Pressable
                ...
                onPress={() => replaceVisibleQuestion(example)}
                accessibilityRole="button"
              >
      ...
      {answer ? (
          ...
          <View style={[styles.answerColumn, compact && styles.answerColumnCompact]}>
            <View style={styles.answerCard}>
              <Text style={styles.label}>QUESTION ANSWERED</Text>
              <Text style={styles.answeredQuestion}>{currentRequest?.question}</Text>
            ...
            {answer.supportingEvidence.map((evidence, index) => {
              const href = evidence.sourceType === 'document' && projectName
                ...
                : {
                    ...
                    params: { project: projectName, projectId: projectId || '' },
                  };
              return (
...
function desktopAskCompletionIsCurrent({
  mounted,
  generation,
  currentGeneration,
  capturedContextKey,
  ...
  return mounted && generation === currentGeneration && capturedContextKey === currentContextKey;
}
...
const styles = StyleSheet.create({
  ...
  answeredQuestion: { color: desktopSurfaces.textMuted, fontSize: 15, lineHeight: 22, fontWeight: '800', marginTop: 6, marginBottom: 16 },
