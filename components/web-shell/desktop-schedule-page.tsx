...
import type { CloudProject } from '../../services/SupabaseService';
...
export function DesktopSchedulePage({
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!projectId) {
        throw new Error('Choose one project with an exact project ID before saving this schedule item.');
      }
      const item = {
        ...buildDAVEWebScheduleItem({
          draft,
  ...
  const applyDependencyDateChange = async (itemId: string) => {
    ...
    setNotice(null);
    try {
      if (!daveWebTaskMatchesSelectedProject(current, selectedProjectId)) {
        throw new Error('This schedule item is outside the selected project. Refresh before changing it.');
      }
  ...
  const applyAllDependencyDateChanges = async () => {
    ...
    setNotice(null);
    try {
      if (updates.some(item => !daveWebTaskMatchesSelectedProject(item, selectedProjectId))) {
        throw new Error('A calculated schedule item is outside the selected project. Refresh before applying changes.');
      }
