...
import {
  ...
  createDAVEWebTaskId,
  daveWebTaskMatchesSelectedProject,
...
import {
  ...
  daveWebReportSourceMatchesAuthorizedSource,
  formatDAVEWebReport,
...
function AuthorizedDesktopWorkspace({ page }: { page: DesktopReadOnlyPage }) {
  ...
  const projectOptions = useMemo(
    () => buildECOSProjectSelectionOptions({
      projectRecords: snapshot.projects,
      candidateProjectNames: snapshot.projects.map(project => project.name),
    }),
    [snapshot.projects],
  );
  ...
  const selectedProjectRecord = resolveECOSProjectSelection({
    projectId: requestedProjectId || null,
    projectName: requestedProject || null,
    projectRecords: snapshot.projects,
  });
  const selectedProject = selectedProjectRecord?.name || null;
  const selectedProjectId = selectedProjectRecord?.id || null;
  const selectedDocumentCount = selectedProjectId
    ? snapshot.referenceDocuments.filter(document => document.projectId === selectedProjectId).length
    : snapshot.referenceDocuments.length;
  ...
  const selectProject = (project: ECOSProjectSelection | null) => {
    router.setParams({
  ...
  return (
    <View style={[styles.root, usesSidebar && styles.rootWide]}>
      ...
      >
        {!usesSidebar ? <DesktopTopNavigation pathname={pathname} selectedProject={selectedProject} selectedProjectId={selectedProjectId} /> : null}
        ...
        {page !== 'settings' ? (
          <View style={[styles.contextBar, compactContent && styles.contextBarCompact]}>
            <View style={styles.contextScope}>
              ...
              >
                <ProjectChoice label="All projects" active={!selectedProjectId} onPress={() => selectProject(null)} />
        ...
        <DesktopPageData
          key={desktopWorkspaceScopeKey(page, selectedProjectId || selectedProject)}
          page={page}
          selectedProject={selectedProject}
          selectedProjectId={selectedProjectId}
...
function DesktopPageData({
  ...
  const projects = snapshot.projects.filter(project => selectedProjectId
    ? project.id === selectedProjectId
    : matchesProject(project.name, selectedProject));
  ...
  const tasks = selectedProject
    ? scheduleTasksForParentProject(selectedProject, [...snapshot.scheduleItems], selectedProjectId)
  ...
  const documents = snapshot.referenceDocuments.filter(document =>
    documentMatchesProjectScope(document, selectedScopes, selectedProjectId));
  ...
  if (page === 'reports') {
    if (!selectedProjectId || !selectedProject) {
      return <EmptyState text="Choose one exact project before preparing or saving a report." />;
    }
    return (
      <ReportWorkspace
        snapshot={snapshot}
...
function ProjectPortfolioCard({
  ...
  const router = useRouter();
  const projectTasks = scheduleTasksForParentProject(project.name, [...tasks], project.id);
    ...
    <View style={styles.portfolioCard}>
      ...
      <View style={styles.portfolioActions}>
        <Pressable
          ...
          onPress={() => router.push({ pathname: '/', params: { project: project.name, projectId: project.id } })}
          accessibilityRole="button"
        >
        ...
        <Pressable
          ...
          onPress={() => router.push({ pathname: '/tasks', params: { project: project.name, projectId: project.id } })}
          accessibilityRole="button"
        >
...
function TaskEditingWorkspace({
  ...
  projectRecords: NonNullable<ReturnType<typeof useDesktopAuth>['snapshot']>['projects'];
  selectedProject: string | null;
  selectedProjectId: string | null;
}) {
  const auth = useDesktopAuth();
  ...
  const saveTask = async (draft: DAVEWebTaskDraft) => {
    ...
    setNotice(null);
    try {
      const projectId = resolveExactDAVEWebTaskProjectId({
      ...
      if (!projectId) {
        throw new DAVEWebTaskValidationError(
          'Choose one project with an exact project ID before saving this task.',
        );
      }
      const item = {
        ...buildDAVEWebScheduleItem({
  ...
  const applyMyChangesAfterConflict = async () => {
    ...
    try {
      const projectId = resolveExactDAVEWebTaskProjectId({
        projectName: conflictDraft.draft.projectName,
        current: latest,
        selectedProjectId,
        projectRecords,
      });
  ...
  const deleteTask = async () => {
    ...
    setNotice(null);
    try {
      if (!daveWebTaskMatchesSelectedProject(deleteCandidate, selectedProjectId)) {
        throw new DAVEWebTaskValidationError(
          'This task is outside the selected project. Refresh before changing it.',
        );
      }
  ...
  const addPhotoToTask = async (task: DAVEWebScheduleItem, file: File | null) => {
    ...
    setNotice(null);
    try {
      if (!daveWebTaskMatchesSelectedProject(task, selectedProjectId)) {
        throw new DAVEWebTaskValidationError(
          'This task is outside the selected project. Refresh before changing it.',
        );
      }
...
function DocumentManagementWorkspace({
  ...
  tasks: readonly DAVEWebScheduleItem[];
  projects: readonly string[];
  projectRecords: readonly CloudProject[];
  selectedProject: string | null;
  selectedProjectId: string | null;
  proofFocus: ECOSDesktopDocumentProofFocus | null;
}) {
  ...
  const selectableProjectRecords = projectRecords.filter(
    (project): project is typeof project & { id: string } => Boolean(project.id?.trim()),
  );
  const [uploadProjectIds, setUploadProjectIds] = useState<string[]>(
    selectedProjectId
      ? [selectedProjectId]
      : selectableProjectRecords[0]?.id ? [selectableProjectRecords[0].id] : [],
  );
  const uploadProjectRecords = selectableProjectRecords.filter(project =>
    uploadProjectIds.includes(project.id));
  const uploadProjects = uploadProjectRecords.map(project => project.name);
  ...
  async function chooseUploadFile(
    ...
    try {
      ...
      if (normalizedName(uploadCategory) === 'drawing') {
        if (uploadProjectIds.length !== 1) {
          throw new Error('Choose one exact project before selecting a drawing file.');
        }
        const missing = [
      ...
      const maximumSourceBytes = linkedSource
        ? GOOGLE_DRIVE_LINK_MAX_BYTES
        : DAVE_WEB_MAX_DOCUMENT_BYTES;
      if (!Number.isSafeInteger(file.size) || file.size < 1 || file.size > maximumSourceBytes) {
        throw new Error('This document is too large for protected in-browser processing. Optimize or split it, then retry.');
      }
      const bytes = linkedBytes ?? await file.arrayBuffer();
      if (bytes.byteLength < 1 || bytes.byteLength > maximumSourceBytes || bytes.byteLength !== file.size) {
        throw new Error('The selected document changed during protected preparation. Choose it again.');
      }
      ...
      const preparationInput = {
        ...
        projectId: uploadProjectIds.length === 1 ? uploadProjectIds[0] : null,
  ...
  async function performDocumentReindex(
    ...
    if (normalizedName(document.category) === 'drawing') {
      throw new Error(
        'Drawing preparation runs in the protected hosted worker; browser provider re-indexing is intentionally disabled.',
      );
    }
    ...
    try {
      ...
      if (document.sourceProvider === 'google_drive' && document.externalSource) {
        ...
        bytes = await readECOSBoundedResponseBytes(response, {
          maximumBytes: DAVE_WEB_MAX_DOCUMENT_BYTES,
          label: 'protected document',
          timeoutMilliseconds: 60_000,
          signal: controller.signal,
        });
        file = new File([bytes], document.originalFileName, {
          type: document.mimeType || response.headers.get('content-type') || 'application/octet-stream',
        });
      }
      const fingerprint = await fingerprintBytes(bytes);
  ...
  async function reindexDocument(document: DAVEWebReferenceDocument) {
    ...
    try {
      if (normalizedName(document.category) === 'drawing') {
        ...
        setNotice({
          tone: 'good',
          text: `“${document.name}” was queued for protected background preparation. You can close this browser while Vitruvius works.`,
        });
        return;
      }
      ...
      {uploadOpen ? (
        <View style={styles.editorCard}>
          ...
          <View style={styles.fieldGroup}>
            ...
            <View style={styles.optionRow}>
              {selectableProjectRecords.map(project => {
                const selected = uploadProjectIds.includes(project.id);
                const duplicateName = selectableProjectRecords.filter(candidate =>
                  normalizedName(candidate.name) === normalizedName(project.name)).length > 1;
                return (
                  <Pressable
                    key={project.id}
                    style={({ pressed }) => [styles.smallChoice, selected && styles.choiceActive, pressed && styles.buttonPressed]}
                    onPress={() => {
                      setUploadProjectIds(current => selected
                  ...
                  >
                    <Text style={[styles.choiceText, selected && styles.choiceTextActive]}>
                      {selected ? '✓ ' : ''}{project.name}{duplicateName ? ` · ${project.id.slice(0, 8)}` : ''}
                    </Text>
                  </Pressable>
                );
              })}
        ...
        {onReindex ? (
          ...
          >
            <Text style={styles.secondaryButtonText}>
              {normalizedName(document.category) === 'drawing'
                ? 'Prepare drawing in background'
                : 'Legacy browser re-index (manual fallback)'}
            </Text>
          </Pressable>
        ) : null}
        {onMakeCurrent ? (
...
function ReportWorkspace({
  ...
  const briefing = useMemo(
    () => buildDAVEWebReportDraft(snapshot, {
      projectId: selectedProjectId,
      projectName: selectedProject,
    }),
  ...
  const currentReportSource = useMemo(
    () => buildDAVEWebReportSource(snapshot, {
      projectId: selectedProjectId,
      projectName: selectedProject,
    }),
    [selectedProject, selectedProjectId, snapshot],
  ...
  const downloadWordReport = async ({
    ...
    body: string;
    generatedAt: string;
    source: DAVEWebReportSource;
  }) => {
    if (pending) return;
    if (!daveWebReportSourceMatchesAuthorizedSource(source, currentReportSource)) {
      setNotice({
        tone: 'danger',
        text: 'Project facts or source authorization changed. Regenerate the report before downloading it.',
      });
      return;
    }
    setPending(true);
    setNotice(null);
    try {
      const sourceIds = new Set(source.updateIds);
      const sourceUpdates = snapshot.projectUpdates
        .filter(update => sourceIds.has(update.id) && update.updateData.id === update.id)
        .map(update => update.updateData);
      const sourceTaskIds = new Set(source.taskIds);
      const sourceDocumentIds = new Set(source.documentIds);
      const drawingReferences = buildAutomaticReportDrawingReferences({
        documents: snapshot.referenceDocuments.filter(document => sourceDocumentIds.has(document.id)),
        scheduleItems: snapshot.scheduleItems.filter(item => sourceTaskIds.has(item.id)),
        selectedProjectNames: [selectedProject],
      });
      const resolvedMedia = await resolveWebReportWordMedia({
        updates: sourceUpdates,
        reportMediaSources: source.media,
        drawingReferences,
        getArtifactUrl: auth.getArtifactUrl,
      });
      const {
  ...
  const save = async (status: 'draft' | 'approved') => {
    ...
    if (!selectedProjectId || !selectedProject) {
      setNotice({
        tone: 'danger',
        text: 'Choose one exact project before saving a report artifact.',
      });
      return;
    }
    if (!daveWebReportSourceMatchesAuthorizedSource(reportSource, currentReportSource)) {
      setNotice({
        tone: 'danger',
        text: 'Project facts or source authorization changed after this report was prepared. Regenerate it before saving.',
      });
      return;
    }
    setPending(true);
    ...
    const report: DAVEWebReportRecord = {
      ...
      sourceMedia: reportSource.media,
      audit: nextAudit,
    };
    try {
  ...
  const openReport = (document: DAVEWebReferenceDocument) => {
    ...
    if (!report || !daveWebPersistedReportSourceIsAuthorized(report, currentReportSource)) return;
  ...
  const approvedReportIsFreshAtCommunicationBoundary = async (): Promise<boolean> => {
    const refreshedSnapshot = await auth.refreshSnapshotForAuthorization().catch(() => null);
    if (!refreshedSnapshot) {
      setNotice({
        tone: 'danger',
        text: 'Vitruvius could not recheck the approved report against the cloud. Refresh and try again.',
      });
      return false;
    }
    const exactProjects = refreshedSnapshot.projects.filter(project =>
      project.id === selectedProjectId &&
      project.archived !== true &&
      project.name.trim().toLowerCase() === selectedProject.trim().toLowerCase(),
    );
    const refreshedSource = buildDAVEWebReportSource(refreshedSnapshot, {
      projectId: selectedProjectId,
      projectName: selectedProject,
    });
    const persistedDocument = refreshedSnapshot.referenceDocuments.find(document =>
      document.id === reportId &&
      document.projectId === selectedProjectId &&
      document.cloudUpdatedAt === expectedRevision,
    );
    const persistedReport = persistedDocument
      ? reportRecordFromDocument(persistedDocument)
      : null;
    const authorized = exactProjects.length === 1 &&
      persistedReport?.status === 'approved' &&
      persistedReport.title === reportTitle.trim() &&
      persistedReport.body === reportBody.trim() &&
      daveWebPersistedReportSourceIsAuthorized(persistedReport, refreshedSource) &&
      daveWebReportSourceMatchesAuthorizedSource(reportSource, refreshedSource);
    if (!authorized) {
      setNotice({
        tone: 'danger',
        text: 'Project facts or the approved report revision changed in the cloud. Regenerate or reopen the report before communicating it.',
      });
      return false;
    }
    return true;
  ...
  const shareApprovedReport = async () => {
    if (reportStatus !== 'approved' || !reportFactsAreCurrent) {
      setNotice({
        tone: 'danger',
        text: 'Project facts or source authorization changed. Regenerate the report before sharing it.',
      });
      return;
    }
    if (!await approvedReportIsFreshAtCommunicationBoundary()) return;
    setNotice(null);
    try {
      const shareData = {
        title: reportTitle.trim(),
  ...
  const prepareApprovedReportEmail = async () => {
    if (reportStatus !== 'approved' || !reportFactsAreCurrent) {
      setNotice({
        tone: 'danger',
        text: 'Project facts or source authorization changed. Regenerate the report before preparing email.',
      });
      return;
    }
    if (!await approvedReportIsFreshAtCommunicationBoundary()) return;
  ...
  const reportFactsAreCurrent = daveWebReportSourceMatchesAuthorizedSource(
    reportSource,
    currentReportSource,
  );
              ...
              <View style={styles.reportReviewPanel}>
                ...
                <View style={styles.reportActionStack}>
                  ...
                  <Pressable
                    ...
                    disabled={pending || !reportFactsAreCurrent}
                    accessibilityState={{ disabled: pending || !reportFactsAreCurrent }}
                  >
                    <Text style={styles.secondaryButtonText}>Download Word Report</Text>
                  </Pressable>
                  <Pressable style={({ pressed }) => [styles.secondaryButton, styles.reportActionButton, pressed && styles.buttonPressed]} onPress={() => { void save('draft'); }} disabled={pending || !reportFactsAreCurrent}>
                    <Text style={styles.secondaryButtonText}>Save Draft</Text>
                  </Pressable>
                  <Pressable style={({ pressed }) => [styles.primaryButton, styles.reportActionButton, pressed && styles.buttonPressed]} onPress={() => { void save('approved'); }} disabled={pending || !reportFactsAreCurrent}>
      ...
      <Section title={`Report history (${reportDocuments.length})`} detail="Open a saved draft or approved report, or download a copy for project communication.">
        <View style={styles.list}>
          {reportDocuments.map(document => {
            const report = reportRecordFromDocument(document)!;
            const sourceIsAuthorized = daveWebPersistedReportSourceIsAuthorized(
              report,
              currentReportSource,
            );
            return (
              <View key={document.id} style={[styles.dataCard, styles.taskListCard]}>
                  ...
                  <View style={styles.dataGrow}>
                    ...
                    {!sourceIsAuthorized ? (
                      <Text style={styles.errorText}>
                        Saved source proof does not match the current exact project record. Regenerate before opening or downloading this report.
                      </Text>
                    ) : null}
                  </View>
                ...
                <View style={styles.taskCardActions}>
                  <Pressable
                    style={({ pressed }) => [styles.secondaryButton, pressed && styles.buttonPressed]}
                    onPress={() => openReport(document)}
                    disabled={!sourceIsAuthorized}
                    accessibilityState={{ disabled: !sourceIsAuthorized }}
                  >
                  ...
                  <Pressable
                    ...
                    disabled={pending || !sourceIsAuthorized}
                    accessibilityState={{ disabled: pending || !sourceIsAuthorized }}
                  >
                    <Text style={styles.secondaryButtonText}>Download Word Report</Text>
                  </Pressable>
                </View>
...
function SettingsWorkspace({
  ...
  const [driveConnectionState, setDriveConnectionState] = useState<GoogleDriveConnectionState>(
  ...
  const disconnectDrive = async () => {
    ...
    setDriveConnectionState(current => nextGoogleDriveConnectionState(current, result.status));
    ...
    <View style={styles.settingsWorkspace}>
      ...
      >
        <View style={styles.dataCard}>
          ...
          {googleDriveDisconnectActionIsAvailable(driveConnectionState) ? (
            <Pressable
              style={({ pressed }) => [
                styles.secondaryButton,
                styles.syncButton,
...
function DesktopTopNavigation({ pathname, selectedProject, selectedProjectId }: {
  pathname: string;
  selectedProject: string | null;
  selectedProjectId: string | null;
}) {
  return (
    <View style={styles.topNavigation}>
      ...
      <View style={styles.topNavigationLinks} role="navigation">
        {desktopNavigationItems.map(item => (
          <DesktopNavigationLink key={item.href} pathname={pathname} item={item} selectedProject={selectedProject} selectedProjectId={selectedProjectId} compact />
        ))}
      </View>
    </View>
  );
...
function DesktopNavigationLink({
  ...
  const href = selectedProject && selectedProjectId
    ? { pathname: item.href, params: { project: selectedProject, projectId: selectedProjectId } }
    : item.href;
  return (
...
function documentMatchesProjectScope(
  document: DAVEWebReferenceDocument,
  scopeProjects: readonly string[],
  selectedProjectId: string | null = null,
): boolean {
  if (selectedProjectId) return document.projectId === selectedProjectId;
  if (scopeProjects.length === 0) return true;
