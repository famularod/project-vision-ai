...
import {
  ...
  type DAVEWebArtifactUrlOptions,
...
type DesktopAuthContextValue = Readonly<{
  ...
  refreshSnapshotForAuthorization: () => Promise<DAVEWebReadOnlySnapshot | null>;
...
export function DesktopAuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadAuthorizedSnapshot = useCallback(async (
    ...
    try {
      ...
      if (!mountedRef.current || loadSequenceRef.current !== loadSequence) return null;
  ...
  const signInWithPassword = useCallback(async (email: string, password: string) => {
    ...
    return Boolean(await loadAuthorizedSnapshot(result.session));
  ...
  const refreshSnapshot = useCallback(async () => {
    ...
    return Boolean(await loadAuthorizedSnapshot(status.session));
  }, [clearSessionView, loadAuthorizedSnapshot]);

  const refreshSnapshotForAuthorization = useCallback(async () => {
    const status = await daveWebSupabaseGateway.getSessionStatus();
    if (!status.session) {
      clearSessionView();
      return null;
  ...
  const value = useMemo<DesktopAuthContextValue>(() => ({
    ...
    signOutOfDesktop,
    refreshSnapshot,
    refreshSnapshotForAuthorization,
    loadDocumentCoverageSummary,
    getArtifactUrl,
    createTask,
    updateTask,
