...
export type DAVECaptureProjectOption = Readonly<{
  id: string;
  name: string;
  label?: string;
}>;
...
export function DAVECaptureConfirmationSheet({
  ...
  projects: readonly DAVECaptureProjectOption[];
  ...
  function chooseProject(project: DAVECaptureProjectOption) {
    setWorking(current => correctCaptureMemoryProject(
      current,
      project.id,
      project.name,
  ...
  function confirmRecommendedProject() {
    const exactProject = projects.find(project => project.id === working.projectId);
    if (!exactProject) {
      setSaveError('Choose one exact project before confirming this memory.');
      return;
    }
    setWorking(current =>
      current.projectId === exactProject.id &&
      current.recommendedProject.value?.trim() === exactProject.name.trim()
        ? confirmCaptureProject(current, exactProject.name)
        : correctCaptureMemoryProject(
            current,
            exactProject.id,
            exactProject.name,
...
function ProjectRecommendation({
  ...
  confidence: string;
  confirmed: boolean;
  options: readonly DAVECaptureProjectOption[];
  onConfirm: () => void;
  onChoose: (value: DAVECaptureProjectOption) => void;
}) {
  const currentProjectOption = options.find(option => option.id === projectId);
  const currentProjectLabel = currentProjectOption?.label || (
    currentProjectOption
      ? `${currentProjectOption.name} (${currentProjectOption.id})`
      : value
  );
  return <View style={styles.recommendation}>
    <Text style={styles.label}>{label}</Text>
    <Text style={styles.recommendationValue}>{currentProjectLabel || 'No project recommended'}</Text>
    {projectId ? <Text style={styles.detail}>Project ID: {projectId}</Text> : null}
    <Text style={styles.detail}>Confidence: {friendlyConfidence(confidence)}</Text>
    {value && !confirmed ? <TouchableOpacity style={styles.confirmButton} onPress={onConfirm}><Text style={styles.confirmText}>Confirm {value}</Text></TouchableOpacity> : null}
    {confirmed ? <Text style={styles.confirmedText}>Confirmed</Text> : null}
    <View style={styles.options}>{options.filter(option => option.id !== projectId).map(option => {
      const labelText = option.label || `${option.name} (${option.id})`;
      return <TouchableOpacity key={option.id} style={styles.option} onPress={() => onChoose(option)}><Text style={styles.optionText}>{labelText}</Text></TouchableOpacity>;
    })}</View>
  </View>;
}
