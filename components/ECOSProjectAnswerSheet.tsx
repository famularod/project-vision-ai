...
import { ECOSThinkingBrain } from './ecos-thinking-brain';
      ...
      <View style={[styles.backdrop, tablet && styles.backdropTablet]}>
        <View style={[styles.sheet, tablet && styles.sheetTablet]}>
          ...
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
            <View
              style={styles.brainStage}
              accessibilityRole={loading ? 'progressbar' : undefined}
              accessibilityLabel={loading ? 'ECOS is thinking and checking project evidence' : 'ECOS Brain ready'}
            >
              <ECOSThinkingBrain mode={loading ? 'thinking' : 'idle'} compact={!loading} />
              <View style={styles.brainStatusRow}>
                <View style={[styles.brainStatusDot, loading && styles.brainStatusDotActive]} />
                <Text style={styles.brainStatusText}>
                  {loading ? 'ECOS is researching your project evidence…' : 'ECOS Brain is ready'}
                </Text>
              </View>
            </View>
            ...
            {loading ? (
              <View style={styles.loadingCard}>
                <Text style={styles.loadingTitle}>Checking tasks, field updates, and indexed drawings</Text>
                <Text style={styles.loadingText}>
                  ECOS Assurance will verify the exact sources before the answer appears.
                </Text>
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  brainStage: { gap: 10 },
  brainStatusRow: { minHeight: 28, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  brainStatusDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#6E8DB8' },
  brainStatusDotActive: { backgroundColor: '#12BDF7', shadowColor: '#12BDF7', shadowOpacity: 0.9, shadowRadius: 7 },
  brainStatusText: { color: colors.text, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  ...
  loadingCard: { borderRadius: 15, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center', padding: spacing.md },
  loadingTitle: { color: colors.text, fontSize: 15, fontWeight: '900', textAlign: 'center' },
  loadingText: { color: colors.mutedText, fontSize: 13, lineHeight: 19, marginTop: 5, textAlign: 'center', maxWidth: 520 },
