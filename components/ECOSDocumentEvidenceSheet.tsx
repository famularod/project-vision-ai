...
export function ECOSDocumentEvidenceSheet({
  ...
  const highlight = protectedPage && imageBounds && imageLayout.width > 0 && imageLayout.height > 0
    ? {
        left: imageBounds.x * imageLayout.width,
        top: imageBounds.y * imageLayout.height,
        width: Math.max(8, imageBounds.width * imageLayout.width),
        height: Math.max(8, imageBounds.height * imageLayout.height),
      }
    : null;
  return (
          ...
          <ScrollView contentContainerStyle={styles.content}>
            ...
            {imageUri ? (
              <View
                style={[
                  styles.imageCard,
                  protectedPage ? { aspectRatio: imageWidth / imageHeight } : null,
                ]}
                onLayout={onImageLayout}
              >
                <Image
                  source={{ uri: imageUri }}
                  style={protectedPage ? styles.protectedImage : styles.image}
                  resizeMode="contain"
                  accessibilityLabel={`Protected cited page ${citation?.pageNumber || ''}`.trim()}
                />
                {highlight ? (
                  <View
                    pointerEvents="none"
                    style={[styles.highlight, highlight]}
                    accessibilityLabel="Exact Ask ECOS cited region"
                  />
                ) : null}
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  protectedImage: { ...StyleSheet.absoluteFill, width: '100%', height: '100%' },
  highlight: { position: 'absolute', borderWidth: 3, borderColor: '#D92D20', backgroundColor: 'rgba(217,45,32,0.14)', borderRadius: 3 },
