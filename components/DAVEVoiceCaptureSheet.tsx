...
import type { ECOSProjectSelectionOption } from '../services/ECOSProjectSelection';
...
export function DAVEVoiceCaptureSheet({
  ...
  candidateProjectOptions?: readonly ECOSProjectSelectionOption[];
  ...
  ecosExperience?: boolean;
  onMemoryReady: (result: DAVEVoiceUnderstandingResponse) => void;
  onProjectChange?: (projectName: string) => void;
  onProjectSelectionChange?: (project: ECOSProjectSelectionOption) => void;
    ...
    >
      <View style={[styles.backdrop, usesTabletSheet && styles.backdropTablet]}>
        ...
        >
          ...
          {ecosExperience ? (
            <View style={styles.ecosBrainStage}>
              <ECOSThinkingBrain mode="idle" />
              <Text style={styles.ecosBrainTitle}>ECOS is ready</Text>
              <Text style={styles.ecosBrainGuidance}>Ask about this project. The Brain will examine only verified project evidence.</Text>
            </View>
          ) : null}

          {!projectName && (candidateProjectOptions.length > 0 || candidateProjects.length > 0) ? (
            <View style={styles.projectChoiceCard}>
              ...
              <View style={styles.projectChoices}>
                {(candidateProjectOptions.length > 0
                  ? candidateProjectOptions
                  : candidateProjects.map(name => ({ id: '', name, label: name }))).map(candidate => (
                  <TouchableOpacity
                    key={candidate.id || candidate.name}
                    style={styles.projectChoiceButton}
                    onPress={() => {
                      if (candidate.id) onProjectSelectionChange?.(candidate);
                      else onProjectChange?.(candidate.name);
                    }}
                    accessibilityRole="button"
                    accessibilityLabel={`Use ${candidate.label} for Talk`}
                  >
                    <Ionicons name="folder-outline" size={18} color={colors.primary} />
                    <Text style={styles.projectChoiceText}>{candidate.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  ecosBrainStage: { gap: 8, marginBottom: spacing.lg },
  ecosBrainTitle: { color: colors.text, fontSize: 18, fontWeight: '900', textAlign: 'center', marginTop: 3 },
  ecosBrainGuidance: { color: colors.mutedText, fontSize: 13, lineHeight: 18, textAlign: 'center', paddingHorizontal: spacing.md },
