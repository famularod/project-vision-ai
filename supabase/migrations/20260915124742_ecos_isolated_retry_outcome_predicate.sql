begin;
-- Only expose a yes/no outcome predicate, not the protected request table.
create function public.ecos_isolated_provider_outcomes_completed(p_job_id uuid)
returns boolean language sql stable security definer set search_path='' as $helper$
 select coalesce(auth.jwt()->>'role','')='service_role'
   and exists(select 1 from public.ecos_hosted_index_jobs j
     where j.id=p_job_id and j.mode='shadow_refresh:'||j.id::text)
   and exists(select 1 from public.ecos_drawing_analysis_requests where hosted_job_id=p_job_id)
   and not exists(select 1 from public.ecos_drawing_analysis_requests where hosted_job_id=p_job_id
     and (status<>'completed' or completed_at is null or failure_code is not null));
$helper$;
revoke all on function public.ecos_isolated_provider_outcomes_completed(uuid) from public,anon,authenticated;
grant execute on function public.ecos_isolated_provider_outcomes_completed(uuid) to service_role;
create or replace function public.ecos_retry_isolated_page_refresh(p_refresh_id uuid,p_expected_page_sha256 text)
...
begin
  ...
  if not public.ecos_isolated_provider_outcomes_completed(j.id) then
