-- ECOS Hosted Indexer bounded organization concurrency
--
-- The first shadow deployment serialized all organizations to one active
-- document. Keep claims coordinated, but allow a small organization-scoped
-- concurrency limit so large drawing sets finish in a practical time without
-- creating an unbounded provider or cost burst.
...
begin
  ...
  -- Claims remain serialized for the short selection transaction. Processing
  -- runs concurrently only up to each organization's explicit bound.
