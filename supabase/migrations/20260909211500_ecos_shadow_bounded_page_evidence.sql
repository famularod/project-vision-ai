...
-- Return only the proof-oriented regions needed to answer one measurement
-- question. Exact page authority remains database-owned, while query terms are
-- treated as literal text (never as regular expressions). Deterministic table
-- relationships, assured vision regions, and measurement-shaped regions are
-- retained ahead of lexical context and the result is strictly bounded.
create or replace function public.ecos_load_hosted_shadow_bounded_page_evidence_pairs_v24(
  p_project_id text,
  p_document_ids text[],
        ...
        from (
          ...
          where normalized.region_source in (
            ...
            or normalized.region_text ~ $measurement$(?ix)(
                [0-9]+(?:[.][0-9]+)?[[:space:]]*
                (?:inches?|inch|in[.]?|feet|foot|ft[.]?|sf|sq[.]?[[:space:]]*ft|square[[:space:]]+feet)
                |
                [0-9]+(?:[.][0-9]+)?[[:space:]]*['"”’]
              )$measurement$
            or exists (
          ...
          order by
            case
              when normalized.region_source = 'deterministic_structured_table_relationship' then 100
              when normalized.region_source = 'vision' then 95
...
revoke all on function public.ecos_load_hosted_shadow_bounded_page_evidence_pairs_v24(
  text, text[], integer[], text[], integer, integer
) from public, anon, authenticated;
grant execute on function public.ecos_load_hosted_shadow_bounded_page_evidence_pairs_v24(
  text, text[], integer[], text[], integer, integer
) to service_role;
