...
-- Candidate-only projection: exclude quarantined regions before applying the
-- unchanged payload budget. Responsive regions precede generic drawing labels.
-- Original region objects and all current-source assurance checks are retained.
create or replace function public.ecos_load_hosted_shadow_bounded_page_evidence_pairs_v28(
  p_project_id text,
  p_document_ids text[],
    ...
    jsonb_build_object(
      ...
      'regions', coalesce((
        ...
        from (
          ...
          where coalesce(region.value->>'searchable', 'true') <> 'false'
            and (normalized.region_source in (
          ...
          order by
            (
              select count(*) from normalized_terms term
              where position(term.term in normalized.region_text) > 0
            ) desc,
            case
...
revoke all on function public.ecos_load_hosted_shadow_bounded_page_evidence_pairs_v28(
  text, text[], integer[], text[], integer, integer
) from public, anon, authenticated;
grant execute on function public.ecos_load_hosted_shadow_bounded_page_evidence_pairs_v28(
  text, text[], integer[], text[], integer, integer
) to service_role;
