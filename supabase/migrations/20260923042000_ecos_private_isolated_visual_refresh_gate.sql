-- Private isolated page refresh visual gate, bounded to the exact active
-- refresh. Captured from the current live functions on 2026-09-22; only
-- the disabled-shadow isolated-refresh predicate is changed.
begin;

-- Only an active, source-bound isolated refresh may make private visual calls
-- while the organization's hosted indexer is disabled. Ordinary hosted jobs
-- retain the enabled-configuration requirement in both reservation RPCs.
create or replace function public.ecos_private_isolated_visual_refresh_allowed(
  p_job_id uuid, p_page_number integer
) returns boolean
language sql volatile security definer set search_path = '' as $function$
  select coalesce(auth.jwt()->>'role','') = 'service_role'
    and exists (
      select 1
      from public.ecos_hosted_index_jobs candidate
      join public.ecos_hosted_page_refreshes refresh
        on refresh.candidate_job_id = candidate.id
       and refresh.page_number = p_page_number
       and refresh.source_sha256 = candidate.source_sha256
       and refresh.state = 'preparing'
      join public.ecos_hosted_index_jobs parent
        on parent.id = refresh.parent_job_id
       and parent.mode = 'shadow'
       and parent.state = 'ready'
       and parent.source_sha256 = candidate.source_sha256
       and parent.organization_id = candidate.organization_id
       and parent.project_id = candidate.project_id
       and parent.document_id = candidate.document_id
       and parent.source_owner_id = candidate.source_owner_id
       and parent.source_page_count = candidate.source_page_count
      join public.ecos_hosted_index_pages parent_page
        on parent_page.job_id = parent.id
       and parent_page.page_number = p_page_number
       and parent_page.state = 'assured'
       and parent_page.unresolved_region_count = 0
       and parent_page.assurance_result->>'accepted' = 'true'
      join public.ecos_hosted_index_configuration config
        on config.organization_id = candidate.organization_id
       and config.enabled = false
       and config.publication_mode = 'shadow'
      where candidate.id = p_job_id
        and candidate.mode = 'shadow_refresh:' || candidate.id::text
        and candidate.state in ('extracting','mapping','awaiting_visual','assuring')
        and candidate.source_scan_status = 'clean'
        and candidate.claim_token is not null
        and candidate.lease_expires_at > clock_timestamp()
        and public.ecos_hosted_job_matches_reference(parent.id,true)
    );
$function$;
revoke all on function public.ecos_private_isolated_visual_refresh_allowed(uuid,integer) from public,anon,authenticated;
grant execute on function public.ecos_private_isolated_visual_refresh_allowed(uuid,integer) to service_role;

CREATE OR REPLACE FUNCTION public.ecos_reserve_hosted_visual_region_v2(p_job_id uuid, p_claim_token uuid, p_page_number integer, p_region_key text, p_evidence_version text, p_exception_fingerprint text, p_estimated_cost_microusd bigint DEFAULT 0)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  for update;
  if not found or (not current_config.enabled and not public.ecos_private_isolated_visual_refresh_allowed(current_job.id, p_page_number)) then return false; end if;
...
CREATE OR REPLACE FUNCTION public.ecos_begin_drawing_analysis(p_organization_id text, p_project_id text, p_document_id text, p_source_sha256 text, p_page_number integer, p_hosted_job_id uuid, p_hosted_claim_token uuid, p_evidence_version text, p_visual_exception_fingerprint text, p_visual_region_key text, p_provider_operation_id text, p_idempotency_key text, p_payload_sha256 text, p_payload_bytes bigint)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
begin
  ...
  join public.ecos_hosted_index_configuration configuration
   ...
   and (configuration.enabled = true or public.ecos_private_isolated_visual_refresh_allowed(job.id, p_page_number))
  ...
  where configuration.organization_id = current_job.organization_id
    and (configuration.enabled = true or public.ecos_private_isolated_visual_refresh_allowed(current_job.id, p_page_number));
  ...
  -- Daily request creation follows the organization's visual cap, including
  -- exact private shadow refreshes, bounded to an absolute maximum of 500.
  -- Source-owner, project, and
