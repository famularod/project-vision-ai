...
-- The private question worker needs drawing text, bounded regions, structured
-- facts, and verified sheet identity. Large visual-coverage diagnostics are
-- useful to the indexer but are not answer evidence; transferring them for
-- every question can exhaust the Edge worker before composition begins.
create or replace function public.ecos_load_hosted_shadow_compact_page_context_pairs_v23(
  p_project_id text,
  p_document_ids text[],
...
begin
  ...
  select
    ...
    jsonb_build_object(
      ...
      'regions', page.final_page_data->'regions',
...
revoke all on function public.ecos_load_hosted_shadow_compact_page_context_pairs_v23(
  text, text[], integer[], integer
) from public, anon, authenticated;
grant execute on function public.ecos_load_hosted_shadow_compact_page_context_pairs_v23(
  text, text[], integer[], integer
) to service_role;
