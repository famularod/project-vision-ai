...
-- The hosted statement timeout cannot safely contain 20 serial delegated
-- searches. Keep the transport helper deliberately small; callers preserve
-- the complete query plan by issuing these small batches in parallel.
create or replace function public.ecos_search_hosted_shadow_chunks_v23(
  ...
  p_result_limit integer default 120
)
returns table(
  document_id text,
  page_number integer,
...
declare
  ...
  bounded_result_limit integer := greatest(
    1,
    least(coalesce(p_result_limit, 120), 120)
  );
begin
  if coalesce(auth.jwt()->>'role', '') <> 'service_role' then
    raise insufficient_privilege using message = 'service role required';
  end if;
  if p_search_queries is null or cardinality(p_search_queries) not between 1 and 5 then
    raise invalid_parameter_value using message = 'one to five search queries required';
  end if;
  if exists (
    select 1
