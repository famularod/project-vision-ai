-- Bind one assured hosted citation to the exact current ECOS V2 protected
-- page raster. The app receives immutable identifiers and dimensions only;
-- private object paths and document content remain server-only.
...
begin
  if to_regprocedure(
    ...
    raise exception 'Protected ECOS page source predecessor is missing';
  end if;
end;
$precondition$;

drop function public.dave_verify_current_ecos_document_proof(
