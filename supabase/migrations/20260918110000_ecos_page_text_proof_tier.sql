-- ECO-04: page-level proof tier ("Verified from page text").
--
-- Prepared page text that has no hosted coordinate region (general notes,
-- specifications, page-level text) could never support an Ask ECOS answer:
-- the proof authority required a region id. This replaces
-- public.dave_verify_current_ecos_document_proof with the same signature and
-- the same exact-region behaviour, plus one addition: a NULL p_region_id is a
-- page-text claim and binds to the whole assured current page (bounds
-- 0/0/1/1, region_id NULL, same protected-page source view citation).
-- The runtime lowers such answers to "verified_with_limits" and labels the
-- source "Verified from page text". Only the owner can call this function
-- (unchanged). Everything else in the function is copied verbatim from
-- 20260913210000_ecos_atomic_customer_proof_contract_after_authoritative_binding.sql.

begin;

do $precondition$
begin
  if to_regprocedure('public.dave_is_app_owner()') is null
     or to_regprocedure('public.ecos_hosted_job_matches_reference(uuid,boolean)') is null
     or to_regprocedure('public.dave_verify_current_ecos_document_proof(text,text,text,text,text,integer,text,text)') is null then
    raise exception 'Ask ECOS proof authority dependencies are missing';
  end if;
end;
$precondition$;
...
begin
  ...
  if requested_project_id is null
      ...
      or length(coalesce(requested_region_id, '')) > 512
      or requested_region_id = 'page'
  ...
  with exact_job as materialized (
    ...
    -- Exact tier: the one hosted region whose id was claimed.
    -- Page-text tier (ECO-04, null region claim): the whole assured page,
    -- bounds 0/0/1/1. The customer sees "Verified from page text" and the
    -- answer is never presented as fully verified.
    select matched.document_id, matched.value, matched.bounds, count(*) over () as match_count
    from (
    select
      page.document_id,
      region.value,
    ...
    where requested_region_id is not null
      and region.value->>'id' = requested_region_id
    ...
    select
      page.document_id,
      jsonb_build_object('id', 'page') as value,
      jsonb_build_object('x', 0, 'y', 0, 'width', 1, 'height', 1) as bounds
    from exact_page page
    where requested_region_id is null
    ) matched
...
comment on function public.dave_verify_current_ecos_document_proof(
  text, text, text, text, text, integer, text, text
) is 'Owner-only exact hosted-region (or, for a null region claim, whole assured page: ECO-04 page-text tier) and current protected-page binding. Returns no document text, private object path, or public URL. The source viewer must still reauthorize and revalidate every supplied raster pin before returning bytes.';
