...
async function testConcurrentEnqueuePreservesBothItems() {
  ...
  assert.strictEqual(
    ...
    3,
    'name-only legacy project archives must remain quarantined beside the current exact intent',
  );
  assert.deepStrictEqual(
    collapsedProjectArchiveQueue.map(item => item.id).sort(),
    [
      'legacy-shell-archive-a',
      'legacy-shell-archive-b',
      'project-archive-fire%20pump%20house',
    ],
    'queue normalization must not convert unresolved legacy names into exact deletion authority',
  );
