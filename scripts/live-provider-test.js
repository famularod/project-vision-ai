...
assert(
  app.includes('const liveAuthorityInput = useMemo') &&
    app.includes('const authorityEvidence = buildExactPIELiveAuthorityProjectEvidence({') &&
    app.includes("updates: authorityEvidence.updates as unknown as PIELiveAuthorityInput['updates']") &&
    app.includes("scheduleItems: authorityEvidence.scheduleItems as unknown as PIELiveAuthorityInput['scheduleItems']") &&
    app.includes("currentUpdate: authorityEvidence.currentUpdate as unknown as PIELiveAuthorityInput['currentUpdate']") &&
    app.includes('captureMemories:'),
  'App must include exact-scoped live evidence in provider input.',
);
assert(
  app.includes('const scopedProjectId = combinedReportScope?.projectId || authorityProject?.id || null;') &&
    app.includes('reportScope: reportEvidenceScope,') &&
    app.includes('project: authorityProject,') &&
    app.includes('projectRecords,'),
  'App must fail closed to empty evidence when one exact project cannot be resolved.',
);
assert(
