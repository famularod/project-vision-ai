...
const modes = Object.freeze({
  rehearsal: Object.freeze({
    databaseEnv: 'VITRUVIUS_SOURCE_PROVENANCE_REHEARSAL_DATABASE_URL',
    confirmationEnv: 'VITRUVIUS_SOURCE_PROVENANCE_REHEARSAL_CONFIRM',
    expectedConfirmation:
      `ROLLBACK_ONLY_SOURCE_PROVENANCE_${EXPECTED_PROJECT_REF.toUpperCase()}`,
    marker: REHEARSAL_MARKER,
    receiptPath: rehearsalReceiptPath,
  }),
  postapply: Object.freeze({
    databaseEnv: 'VITRUVIUS_SOURCE_PROVENANCE_POSTAPPLY_DATABASE_URL',
    confirmationEnv: 'VITRUVIUS_SOURCE_PROVENANCE_POSTAPPLY_CONFIRM',
    expectedConfirmation:
      `READ_ONLY_SOURCE_PROVENANCE_${EXPECTED_PROJECT_REF.toUpperCase()}`,
...
function runEvidenceMode(mode, env = process.env, options = {}) {
  ...
  if (!config) {
    throw new Error(
      'Usage: vitruvius-source-provenance-migration-evidence.js rehearsal|postapply',
    );
  }
  ...
  if (candidate.repository.clean !== true) {
    throw new Error(
      'Source-provenance evidence requires a clean, exact committed candidate.',
    );
  }
  ...
  if (mode === 'postapply') {
    if (!fs.existsSync(rehearsalReceiptPath)) {
      throw new Error('Source-provenance postapply requires its rehearsal receipt.');
    }
...
function main() {
  try {
    ...
    console.error(`Source-provenance migration evidence FAIL: ${error.message}`);
    process.exitCode = 1;
  }
}
