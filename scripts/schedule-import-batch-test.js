...
assert(
  appSource.includes('extractSchedulePdfWithServer({') &&
    !appSource.includes('isDavePdfTextExtractionAvailable') &&
    !appSource.includes('extractTextFromPdf') &&
    !appSource.includes('Review imported PDF schedule:'),
  'PDF imports should use bounded server extraction and must not turn a review placeholder into a schedule task.',
);
assert(
...
assert(
  scheduleScreenSource.includes('sections={mobileTaskSections}') &&
    scheduleScreenSource.includes('() => groupedTaskSections.map(section => {') &&
    scheduleScreenSource.includes('data: collapsed ? [] : section.data,') &&
    scheduleScreenSource.includes('[collapsedMobileTaskAreas, groupedTaskSections]') &&
    ...
    scheduleScreenSource.includes('if (aDays !== bDays) return aDays - bDays;') &&
    scheduleScreenSource.includes('return priorityRank[a.priority] - priorityRank[b.priority] ||') &&
    scheduleScreenSource.includes("if (taskView === 'Completed Tasks') return complete;") &&
    scheduleScreenSource.includes('if (complete) return false;') &&
    scheduleScreenSource.includes('setCollapsedMobileTaskAreas(current => {') &&
    scheduleScreenSource.includes('Schedule Sources') &&
    scheduleScreenSource.includes('onPress={() => setSourcesOpen(open => !open)}') &&
    scheduleScreenSource.includes('sourcesOpen ? scheduleDocuments.map(document => {'),
  'Tasks should remain grouped by project and area, urgency-ordered within each group, split into open and completed views, with collapsible task areas and schedule sources.',
);
assert(
