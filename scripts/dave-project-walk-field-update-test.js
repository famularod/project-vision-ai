...
function load(relative, cache = new Map()) {
  ...
  const localRequire = request => request.startsWith('.')
    ? load(path.relative(root, path.resolve(path.dirname(filename), `${request}.ts`)), cache)
    : require(request);
...
assert.strictEqual(draft.projectId, 'project-alpha',
  'The review draft must carry the immutable project ID into the saved Field Update boundary.');
