...
assert(!nativeSource.includes('import PDFKit'), 'Untrusted PDFs must not enter an in-process native PDFKit parser or renderer.');
assert(!nativeSource.includes('AsyncFunction("renderPdfExcerpt")'), 'The native module must not expose in-process PDF proof rendering.');
assert(!nativeSource.includes('AsyncFunction("extractTextFromPdf")'), 'The native module must not expose uncancellable PDF text extraction.');
assert(!nativeSource.includes('selection.selectionsByLine()'), 'The native module must not materialize unbounded PDF text selections.');
assert(
  ...
  'Schedule PDFs must use the bounded server extractor with no stale native capability branch.',
);
assert(
  scheduleIntelligenceSource.includes(".split('\\n')") &&
    scheduleIntelligenceSource.includes(".join('\\n')"),
  'Schedule text cleanup must retain row boundaries returned by bounded extraction.',
);
