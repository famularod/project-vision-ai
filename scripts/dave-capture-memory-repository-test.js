...
async function run() {
  ...
  const projectCorrected = memory.correctCaptureMemoryProject(
    draft, 'project-alpha', 'Alpha', '2026-07-12T08:01:00.000Z',
  );
  ...
  assert.strictEqual(hydrated.recommendedProject.value, 'Alpha');
  ...
  const concurrentRepositoryA = repositoryModule.createDAVECaptureMemoryRepository(concurrentStorage);
  const concurrentRepositoryB = repositoryModule.createDAVECaptureMemoryRepository(concurrentStorage);
  const concurrentMemoryA = {
    ...saveable,
    id: 'memory-concurrent-a',
  };
  const concurrentMemoryB = {
    ...saveable,
    id: 'memory-concurrent-b',
  };
  await Promise.all([
    concurrentRepositoryA.save(concurrentMemoryA),
    concurrentRepositoryB.save(concurrentMemoryB),
  ]);
  assert.deepStrictEqual(
    [...(await concurrentRepositoryA.list()).map(item => item.id)].sort(),
    ['memory-concurrent-a', 'memory-concurrent-b'],
    'Concurrent repository writers must be serialized so neither confirmed memory is lost.',
  );
