...
function main() {
  ...
  if (!fs.existsSync(infoPath) || !fs.existsSync(bundlePath)) {
    throw new Error('The selected artifact is not a complete Vitruvius iOS app.');
  }
  ...
  execFileSync('/usr/bin/codesign', ['--verify', '--deep', '--strict', resolvedAppPath], {
    stdio: 'pipe',
  });
...
if (require.main === module) {
  try {
    ...
    process.stderr.write(`Vitruvius iOS release artifact gate FAIL: ${error.message}\n`);
    process.exitCode = 1;
  }
}
