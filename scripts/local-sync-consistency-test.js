...
includes(app, 'queuedProjectDeletionBarriers(queuedChanges)', 'pending project deletes must be classified into exact-id and legacy-name startup barriers before cloud hydration');
includes(app, 'queuedProjectDeletions.exactProjectIds', 'exact pending project deletes must remain immutable-id barriers');
includes(app, 'queuedProjectDeletions.legacyProjectNames', 'only historical name-only pending deletes may enter the legacy name barrier');
...
includes(app, 'setCloudProjectArchived(project.id, project.name, true)', 'archiving a project must persist one exact project identity to cloud sync');
includes(app, 'setCloudProjectArchived(project.id, project.name, false)', 'reopening a project must persist one exact project identity to cloud sync');
