...
const REQUIRED_DEVICE_JOURNEYS = Object.freeze([
  ...
  'cloud_sync_and_cross_device_consistency',
...
const REQUIRED_RECOVERY_SCENARIOS = Object.freeze([
  ...
  'zero_pending_after_sync',
  'same_records_on_iphone_ipad_and_web',
  'repeated_clean_sync_after_reopen',
  'offline_edit_reconnects_exactly_once',
  'no_duplicate_lost_or_stale_records',
  'connection_status_matches_sync_health',
]);
...
function validateDeviceEvidence(input, options = {}) {
  ...
  if (value.schemaVersion !== 1) failures.push('Device evidence schemaVersion must be 1.');
...
function runCli(argv = process.argv.slice(2)) {
  ...
  const device = validateDeviceEvidence(parseJson(args.device), { evidenceExists });
  const operations = validateOperationsEvidence(parseJson(args.operations), { evidenceExists });
  const pilot = validatePilotEvidence(parseJson(args.pilot), { evidenceExists });
  const ok = device.ok && operations.ok && pilot.ok;
