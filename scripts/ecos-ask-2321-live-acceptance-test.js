...
const EXACT_PROJECT_ID = '607c7eed-5dea-4a5a-8b52-0f165c71c4b5';
...
const pageRows = [{
  ...
  regions: [{
    ...
    text: 'SITE PHOTOMETRICS PLAN — ALL — 2.6 fc average — 21.1 fc maximum — 0.0 fc minimum',
    x: 0.82,
    y: 0.86,
    width: 0.15,
    height: 0.08,
...
function acceptanceContext(currentPageRows = pageRows) {
  return {
    projectId: EXACT_PROJECT_ID,
    projectName: definition.projectName,
    currentDocuments: [{
      id: 'electrical-2321',
      name: '06 - PLZ CORP - 2321 THIRD STREET - ELECTRICAL',
      projectId: EXACT_PROJECT_ID,
...
const passing = evaluateAcceptanceCase(testCase, response, acceptanceContext());
...
function evaluateWithCoverage(visualCoverage, overrides = {}) {
  return evaluateAcceptanceCase(
    testCase,
    response,
    acceptanceContext([{ ...pageRows[0], ...overrides, visual_coverage: visualCoverage }]),
  );
}
