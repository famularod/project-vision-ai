...
const exhaustFanCase = definition.cases.find(item => item.id === '2321-restroom-exhaust-fan');
const naturalExhaustFanAnswer =
  'The schedule identifies EF-1 as a 100-CFM exhaust fan serving four restrooms and one janitor closet.';
for (const pattern of exhaustFanCase.requiredAnswerPatterns) {
  assert(
    new RegExp(pattern, 'ims').test(naturalExhaustFanAnswer),
    `Natural hyphenated airflow wording must satisfy the contract: ${pattern}`,
  );
}

const installedTreeCase = definition.cases.find(
  item => item.id === '2321-installed-tree-count-negative-control',
);
assert(
  installedTreeCase.requiredAnswerPatterns.every(pattern =>
    new RegExp(pattern, 'ims').test(installedTreeCase.expectedAnswer)
  ),
  'The installed-tree expected answer must satisfy every required pattern.',
);
assert(
  installedTreeCase.forbiddenAnswerPatterns.some(pattern =>
    new RegExp(pattern, 'ims').test(
      'Yes. The drawing shows 78 trees but does not verify they are installed in the field.',
    )
  ),
  'A contradictory Yes-prefixed installed-condition answer must be rejected.',
);

const plantCase = definition.cases.find(item => item.id === '2321-plant-material-quantities');
const naturalPlantAnswer =
  'Planting Plan A is shown on Sheet L-4, Rev 1. Forest Pansy Redbud: 6 trees. Palo Verde: 12 trees. Lemon Scented Gum: 3 trees. Golden Rain Tree: 9 trees.';
for (const pattern of plantCase.requiredAnswerPatterns) {
  assert(
    new RegExp(pattern, 'ims').test(naturalPlantAnswer),
    `Natural plant-quantity wording must satisfy the contract: ${pattern}`,
  );
}
assert(
  plantCase.requiredAnswerPatterns.some(pattern =>
    !new RegExp(pattern, 'ims').test(
      'Forest Pansy Redbud: 7 trees. Palo Verde: 12 trees. Lemon Scented Gum: 3 trees. Golden Rain Tree: 9 trees.',
    )
  ),
  'The relaxed plant-quantity order must not accept an incorrect count.',
);
