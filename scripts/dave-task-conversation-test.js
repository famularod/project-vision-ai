...
assert(
  /scheduleTasksForParentProject\(\s*projectName,\s*authoritativeScheduleItems[\s\S]*?,\s*projectId,\s*\)/.test(app),
  'Talk task candidates must be collected under the selected immutable project ID.',
);
assert(
  app.includes('const actionProject = resolveECOSProjectSelection({') &&
    app.includes('projectId: talkTaskAction.projectId') &&
    app.includes('const currentTask = authoritativeScheduleItems.find(') &&
    app.includes("'Task scope changed'") &&
    app.includes('const latestTask = scheduleItemsCurrentRef.current.find'),
  'Talk confirmation and Undo must revalidate the live task against the selected immutable project.',
);
