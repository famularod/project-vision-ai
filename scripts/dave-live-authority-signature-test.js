...
const captureMemoryScope = { ...input(), captureMemoryProjectIds: ['project-1'] };
const changedCaptureMemoryScope = { ...input(), captureMemoryProjectIds: ['project-2'] };
assert.notStrictEqual(
  authorityInputSignature(changedCaptureMemoryScope),
  authorityInputSignature(captureMemoryScope),
  'capture-memory project identity changes must refresh project authority',
);
assert.notStrictEqual(
  authorityInputScopeSignature(changedCaptureMemoryScope),
  authorityInputScopeSignature(captureMemoryScope),
  'capture-memory project identity changes must bypass the typing debounce',
);
