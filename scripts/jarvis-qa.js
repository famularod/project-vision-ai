...
if (
  ...
  bottomNav.indexOf('onPress={onTalk}') < bottomNav.indexOf('label="Ask ECOS"') &&
  bottomNav.indexOf('label="Ask ECOS"') < bottomNav.indexOf('label="Reports"')
) {
  pass(
    'Bottom navigation',
    'The live workflow labels Overview, Tasks, Talk, Ask ECOS, and Reports are present in the correct order.',
    'components/app-bottom-tabs.tsx',
  );
} else {
  fail(
    ...
    'Ensure the live bottom navigation exposes Overview, Tasks, Talk, Ask ECOS, and Reports in workflow order.',
    'components/app-bottom-tabs.tsx',
  );
}
...
if (
  hasAll(scheduleScreen + app, [
    'extractSchedulePdfWithServer',
    'isDaveTextRecognitionAvailable',
    'no dated activities were extracted',
  ]) &&
  !app.includes('extractTextFromPdf') &&
  !app.includes('isDavePdfTextExtractionAvailable') &&
  !app.includes('extractScheduleItemsWithAiEndpoint') &&
  !adminScreen.includes('Advanced schedule OCR endpoint')
) {
  pass(
    'Schedule import',
    'Schedule PDFs use bounded server extraction, image OCR stays local, and the workflow has a user-facing failure path with no arbitrary upload endpoint.',
    'App.tsx',
  );
} else {
  fail(
    'Schedule import',
    'Safe bounded schedule extraction or its failure handling was not found.',
    'Preserve bounded server PDF extraction, local image OCR, and clear failure handling without a user-configurable upload endpoint.',
    'App.tsx',
  );
}
...
if (
  hasAll(app, [
    ...
    'liveAuthority.projectTruth.schedule.length > 0',
    'liveAuthority.projectTruth.briefing.schedule',
  ]) &&
  hasAll(daveProjectTruth, [
