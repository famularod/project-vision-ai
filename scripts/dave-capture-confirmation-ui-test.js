...
assert(sheet.includes('correctCaptureMemoryProject(') && sheet.includes("correctCaptureMemory(current, 'location'"), 'Project and location corrections must preserve independent exact identity.');
...
assert(sheet.includes('currentProjectOption?.label') && sheet.includes('option.label'),
  'The current and alternate project choices must render the caller supplied exact-identity labels.');
assert(sheet.includes('projects.find(project => project.id === working.projectId)') &&
  sheet.includes('correctCaptureMemoryProject('),
  'Confirming or changing the project must bind the displayed choice to its exact project ID.');
assert(!sheet.includes("option.id.slice(-8)"),
  'Project confirmation must never abbreviate the only exact-identity discriminator.');
...
let draft = memory.createCaptureMemory({
  id: 'ui-memory', projectId: 'project-canopy-b', transcript: 'ABC Electric will finish conduit.', transcriptSourceRecordId: 'ui-transcript', createdAt: '2026-07-12T08:00:00.000Z',
...
draft = memory.correctCaptureMemoryProject(draft, 'project-canopy-c', 'Canopy C', '2026-07-12T08:01:00.000Z');
