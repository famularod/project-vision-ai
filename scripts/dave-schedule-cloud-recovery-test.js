...
assert(supabase.includes("${exactProjectBinding ? 'project_id, ' : ''}${jsonColumn}"));
assert(supabase.includes('exactProjectBinding: true'));
assert(supabase.includes('if (!rowProjectId || rowProjectId !== embeddedProjectId) return null;'));
