...
function runGate(options = {}) {
  ...
  const rehearsal = options.rehearsal
    ...
    : readReceipt(
        ...
        'Source-provenance ACL rehearsal receipt',
      );
  const postapply = options.postapply
    ? suppliedReceipt(options.postapply)
    : readReceipt(
        options.postapplyReceiptPath || postapplyReceiptPath,
        'Source-provenance ACL postapply receipt',
      );
  const expected = options.expectedDigests || {
    rehearsal:
      process.env.VITRUVIUS_SOURCE_PROVENANCE_ACL_REHEARSAL_RECEIPT_SHA256,
    postapply:
      process.env.VITRUVIUS_SOURCE_PROVENANCE_ACL_POSTAPPLY_RECEIPT_SHA256,
  };
  if (!/^[a-f0-9]{64}$/.test(String(expected.rehearsal || '')) ||
    ...
    throw new Error(
      'Source-provenance ACL independent receipt digest is missing or does not match.',
    );
  }
  validateEvidence({
...
function main() {
  try {
    ...
    console.log('VITRUVIUS_SOURCE_PROVENANCE_ACL_MIGRATION_EVIDENCE_OK');
    console.log(`Candidate commit: ${result.candidate.repository.commit}`);
  } catch (error) {
    console.error(`Source-provenance ACL evidence gate FAIL: ${error.message}`);
    process.exitCode = 1;
  }
}
