...
const projectDocumentLifecycle = fs.readFileSync(
  path.join(root, 'services/ProjectDocumentLifecycle.ts'),
  'utf8',
);
...
assert(
  app.includes('const selectedWorkspaceDocuments = projectDocumentsForExactWorkspace({') &&
    app.includes('projectId: selectedWorkspaceCoverProject?.id') &&
    app.includes('documentCount={selectedWorkspaceDocuments.length}') &&
    app.includes('documents={selectedWorkspaceDocuments}') &&
    projectDocumentLifecycle.includes('document.projectId === exactProjectId') &&
    app.includes("onOpenDocuments={() => setScreen('ProjectDocuments')}"),
  'Project Workspace Documents should render and count the exact selected project on every device.',
);
