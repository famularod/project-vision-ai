...
const syncService = fs.readFileSync(
  path.join(root, 'services/SyncService.ts'),
  'utf8',
);
...
const saveProjectUpdateSource = service.match(
  /export async function saveProjectUpdate<[\s\S]*?\n\}\n\nexport async function deleteProjectUpdate/m,
)?.[0] || '';
includes(
  saveProjectUpdateSource,
  'const projectId = exactPersistedProjectId(updateRecord.projectId)',
  'field-update writes must require the producer-supplied exact project ID',
);
includes(
  saveProjectUpdateSource,
  'project_id: projectId',
  'field-update writes must persist the same exact project ID in the database row',
);
const upsertScheduleItemSource = service.match(
  /export async function upsertScheduleItem\([\s\S]*?\n\}\n\nexport async function upsertReferenceDocument/m,
)?.[0] || '';
includes(
  upsertScheduleItemSource,
  'const projectId = exactPersistedProjectId(item.projectId)',
  'task writes must reject name-only project authority',
);
includes(
  upsertScheduleItemSource,
  'project_id: projectId',
  'task writes must persist the exact project ID beside item_data',
);
includes(
  upsertScheduleItemSource,
  "select('id, project_id, item_data')",
  'task writes must acknowledge both row and embedded project identity',
);
const uploadPhotoSource = service.match(
  /export async function uploadPhoto\([\s\S]*?\n\}\n\nexport async function downloadPhoto/m,
)?.[0] || '';
includes(
  uploadPhotoSource,
  'contentSha256: integrity.sha256',
  'large photo uploads must return the digest verified by the resumable uploader',
);
includes(
  uploadPhotoSource,
  'sha256(new Uint8Array(fileData))',
  'bounded photo uploads must hash the exact ArrayBuffer sent to storage',
);
const stageProjectUpdateSource = syncService.match(
  /export async function stageProjectUpdateForSync\([\s\S]*?\n\}\n\nexport async function runFieldUpdateCloudSync/m,
)?.[0] || '';
includes(
  stageProjectUpdateSource,
  'projectUpdateWithPhotoUploadReceipts(',
  'the queued field-update payload must bind storage upload digests before database persistence',
);
includes(
  stageProjectUpdateSource,
  'photoAttempt.photoContentSha256ById',
  'field-update persistence must use receipts from the exact photo upload attempt',
);
