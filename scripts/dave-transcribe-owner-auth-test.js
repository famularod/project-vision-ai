...
assert(
  source.includes("return json({ error: 'forbidden' }, 403, corsHeaders)"),
  'non-owner access must be forbidden',
);
assert(
  source.includes("return json({ error: 'authorization_unavailable' }, 503, corsHeaders)"),
  'owner-check failures must fail closed',
);
