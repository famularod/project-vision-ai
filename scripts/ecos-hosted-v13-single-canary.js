...
let baselineReferenceDocument = null;
const HOSTED_DERIVED_REFERENCE_FIELDS = new Set([
  'extractedPages',
  'extractedText',
  'extractionStatus',
  'searchablePageCount',
...
const executionIsTerminal = execution => (
...
const verifySchedulerAndImage = () => {
  ...
  // Cloud Run can omit completionTime for an execution that failed before a
  // task started. A terminal Completed=False condition is still finished and
  // must not permanently block later contained canaries as if it were active.
  const active = executions.filter(execution => !executionIsTerminal(execution));
...
const referenceAuthorityPayload = documentData => Object.fromEntries(
  Object.entries(documentData || {}).filter(([key]) => !HOSTED_DERIVED_REFERENCE_FIELDS.has(key)),
);

const restoreReferenceDocumentIfChanged = async () => {
  if (!baselineReferenceDocument) return null;
  const currentResult = await supabase
    .from('reference_documents')
    .select('id,owner_id,document_data,updated_at')
    .eq('id', targetDocumentId);
  if (currentResult.error) throw currentResult.error;
  const current = exactlyOne(currentResult.data, 'Canary reference document disappeared during cleanup');
  assert(current.owner_id === baselineReferenceDocument.owner_id,
    'Canary reference document owner changed; automatic restore refused');
  const currentSha = sha256Json(current.document_data || {});
  const baselineSha = sha256Json(baselineReferenceDocument.document_data || {});
  if (currentSha === baselineSha) {
    return { restored: false, baselineSha256: baselineSha, observedSha256: currentSha };
  }
  assert(
    sha256Json(referenceAuthorityPayload(current.document_data))
      === sha256Json(referenceAuthorityPayload(baselineReferenceDocument.document_data)),
    'Canary reference document authority changed outside hosted derived fields; automatic restore refused',
  );
  const restoreResult = await supabase
    .from('reference_documents')
    .update({ document_data: baselineReferenceDocument.document_data })
    .eq('id', targetDocumentId)
    .eq('owner_id', baselineReferenceDocument.owner_id)
    .eq('updated_at', current.updated_at)
    .select('id,document_data');
  if (restoreResult.error) throw restoreResult.error;
  const restored = exactlyOne(
    restoreResult.data,
    'Canary reference document changed concurrently; automatic restore refused',
  );
  assert(
    sha256Json(restored.document_data || {}) === baselineSha,
    'Canary reference document automatic restore did not match the exact baseline',
  );
  return { restored: true, baselineSha256: baselineSha, observedSha256: currentSha };
};
...
async function main() {
  ...
  baselineReferenceDocument = structuredClone(reference);
  ...
  try {
    ...
    try {
      audit.cleanup.referenceDocument = await restoreReferenceDocumentIfChanged();
    } catch (error) {
      cleanupErrors.push(`restore reference document: ${error.message}`);
    }
    try {
    ...
    } else if (audit.cleanup.referenceDocument?.restored) {
      operationError = new Error(
        'Shadow canary changed customer-facing reference metadata; the exact baseline was restored automatically',
      );
    }
  }
  ...
  const embeddingResult = await supabase
    .from('ecos_hosted_chunk_embeddings')
    .select('job_id', { count: 'exact', head: true })
    .eq('job_id', targetJobId)
    .eq('organization_id', organizationId)
    .eq('project_id', targetAfter.project_id)
    .eq('document_id', targetDocumentId)
    .eq('source_sha256', targetSourceSha)
    .eq('evidence_version', EVIDENCE_VERSION)
    .eq('source_kind', 'shadow')
    .eq('embedding_model', 'text-embedding-3-small')
    .eq('embedding_dimensions', 1536);
  if (embeddingResult.error) throw embeddingResult.error;
  assert(
    (embeddingResult.count || 0) === (shadowResult.count || 0),
    'Canary semantic embeddings do not cover every exact shadow chunk',
  );
  ...
  audit.postconditions = {
    ...
    semanticEmbeddingCount: embeddingResult.count,
    shadowPageCounts,
    liveCountsAfter,
    liveSnapshotAfter,
