...
export function createDAVECaptureMemoryRepository(
  ...
  function scopedRecords(
    records: readonly DAVEConfirmedCaptureMemory[],
    projectId?: string,
  ): readonly DAVEConfirmedCaptureMemory[] {
    const exactProjectId = projectId === undefined
      ? null
      : captureMemoryProjectId(projectId);
    return Object.freeze(records
      .filter(memory => !exactProjectId || memory.projectId === exactProjectId)
  ...
  return Object.freeze({
    ...
    async list(projectId) {
      return exclusive(async () => scopedRecords(
        await hydrateRecords(storage),
        projectId,
      ));
    },
    ...
    async read(id) {
      ...
      return exclusive(async () =>
        (await hydrateRecords(storage)).find(memory => memory.id === stableId) ?? null,
      );
    },
...
/** Strict transaction-time read. Corrupt or partially valid stores fail
 * closed; ordinary repository hydration owns quarantine and salvage. */
export function parseCaptureMemoryRepositoryStorageValue(
  raw: string | null,
  ...
  try {
    ...
    throw new Error('Stored capture memories contain invalid JSON.');
  }
  if (
    !isRecord(parsed) ||
    parsed.schemaVersion !== DAVE_CAPTURE_MEMORY_REPOSITORY_VERSION ||
    ...
    throw new Error('Stored capture memories use an invalid repository format.');
  }
  return normalizeConfirmedCaptureMemories(parsed.records);
}

export function captureMemoriesAfterProjectDeletion(
  records: readonly DAVEConfirmedCaptureMemory[],
  projectId: string,
): DAVEConfirmedCaptureMemory[] {
  const exactProjectId = captureMemoryProjectId(projectId);
  return normalizeConfirmedCaptureMemories(records).filter(
    memory => memory.projectId !== exactProjectId,
  );
}
...
export function normalizeConfirmedMemory(value: unknown): DAVEConfirmedCaptureMemory {
  ...
  const memory = {
    ...
    projectId: captureMemoryProjectId(value.projectId),
    status: value.status,
...
function assertSourceEvidencePreserved(
  ...
  if (existing.projectId !== next.projectId) {
    throw new Error('A confirmed memory update must preserve its project identity.');
  }
