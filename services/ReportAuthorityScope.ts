...
export type CombinedReportAuthorityScope = {
  ...
  captureMemoryProjectIds: string[];
  contacts: ContactBook;
...
function buildSupportingEvidenceScope(
  ...
  const selectedCaptureMemoryProjectIds = new Set<string>();
  ...
  selectedProjectNames.forEach(projectName => {
    const matchingRecords = input.projectRecords
      .filter(record => normalizeKey(record.name) === normalizeKey(projectName));
    const exactIds = matchingRecords
      .map(record => cleanText(record.id))
      .filter((id): id is string => Boolean(id));
    if (matchingRecords.length === 1 && exactIds.length === 1) {
      selectedCaptureMemoryProjectIds.add(exactIds[0]);
    }
  });
  ...
  const captureMemories = (input.captureMemories || []).filter(memory => {
    return selectedCaptureMemoryProjectIds.has(memory.projectId);
  });
  ...
  return {
    ...
    projectDocuments,
    captureMemories,
    captureMemoryProjectIds: Array.from(selectedCaptureMemoryProjectIds).sort(),
    contacts: {
