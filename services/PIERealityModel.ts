...
export function summarizeRealityModel(
  ...
  return {
    ...
    strongestCurrentRealityStatement: strongestObject
      ...
      : 'ECOS does not have a strong current reality statement yet.',
...
export function buildObjectNextBestAction(object: PIERealityObject): PIERealityNextAction {
  ...
  if (object.currentStatus === 'contradicted') {
    return { action: object.currentState.nextAction || `Resolve contradiction for ${object.name}.`, reason: 'Contradicted objects should be reconciled before ECOS relies on them.', priority: 'high', ownerNeeded };
  }
  ...
  if (object.currentStatus === 'needs_verification' || object.currentState.uncertain) {
    return { action: object.currentState.nextAction || `Collect current evidence for ${object.name}.`, reason: 'This object needs verification before ECOS relies on it.', priority: 'medium', ownerNeeded };
  }
  if (object.currentState.stale) {
