...
export function scheduleTasksForParentProject(
  ...
  if (exactScopeWasSupplied) {
    const exactProjectId = immutableProjectId(selectedProjectId);
    if (!exactProjectId) return [];
    return items.filter(item => immutableProjectId(item.projectId) === exactProjectId);
  }
...
export function buildDAVEProjectScheduleRollup({
  ...
  const tasks = scheduleTasksForParentProject(projectName, items, selectedProjectId);
