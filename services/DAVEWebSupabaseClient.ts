...
import {
  ...
  daveWebReportPhotoSha256,
  type DAVEWebDocumentExtension,
  type DAVEWebReportMediaSource,
...
import {
  ...
  loadECOSHostedIndexStatusSnapshot,
  reconcileECOSHostedIndexStatus,
...
export type DAVEWebArtifactUrlOptions = Readonly<{
  preview?: boolean;
  reportPhotoSource?: DAVEWebReportMediaSource;
}>;
...
export type DAVEWebTaskPhotoUploadInput = Readonly<{
  task: ScheduleItem & Readonly<{ cloudUpdatedAt: string | null }>;
  bytes: ArrayBuffer;
  fileName: string;
  mimeType: string;
}>;
...
export function createDAVEWebSupabaseGateway(client: SupabaseClient | null) {
  ...
  return Object.freeze({
    ...
    async loadAuthorizedRows(
      ...
      const [projects, scheduleItems, projectUpdates, referenceDocuments, syncTombstones] = await Promise.all([
        ...
        shouldRead('reference_documents')
          ? readOwnerReferenceDocumentRows(client, userId)
      ...
      const exactReferenceDocuments = referenceDocuments.length === 0
        ? referenceDocuments as readonly Record<string, any>[]
        : referenceDocuments
            .map(authoritativeReferenceDocumentRow)
            .filter((value): value is Record<string, any> => value !== null);
      let hostedSnapshot: Awaited<ReturnType<typeof loadECOSHostedIndexStatusSnapshot>> = {
        authority: 'unavailable',
        statuses: [],
      };
      if (shouldReadReferenceDocuments && exactReferenceDocuments.length > 0) {
        try {
          hostedSnapshot = await loadECOSHostedIndexStatusSnapshot({
            client,
            documentIds: exactReferenceDocuments.map(value => value.id as string),
          });
        } catch {
      ...
      const statusByDocumentId = new Map(hostedSnapshot.statuses.map(status => [status.documentId, status]));
      const referenceDocumentsWithHostedStatus = exactReferenceDocuments.length === 0
        ? exactReferenceDocuments
        : exactReferenceDocuments.map(value => {
        const data = isRecord(value.document_data) ? value.document_data : {};
        const documentId = value.id as string;
        return {
          ...value,
          document_data: reconcileECOSHostedIndexStatus(
            data as ReferenceDocument,
            statusByDocumentId.get(documentId) ?? null,
            hostedSnapshot.authority,
          ),
        };
        });
    ...
    async createAuthorizedArtifactSignedUrl(
      ...
      options: DAVEWebArtifactUrlOptions = {},
    ): Promise<string> {
      ...
      let exactReportPhotoPath = false;
      if (options.reportPhotoSource) {
        if (bucket !== 'project-photos' || options.preview) {
          throw new DAVEWebArtifactAccessError(
            'The approved report photo receipt cannot authorize this project file request.',
          );
        }
        const authorizedPath = await loadExactAuthorizedReportPhotoPath(
          client,
          ownerId,
          options.reportPhotoSource,
        );
        exactReportPhotoPath = authorizedPath === safePath;
        if (!exactReportPhotoPath) {
          throw new DAVEWebArtifactAccessError(
            'The current photo no longer matches the approved report source receipt.',
          );
        }
      }
      ...
      if (
        !safePath.startsWith(`${ownerId}/`) &&
        !pathWasLoadedFromOwnerRecord &&
        !exactReportPhotoPath
      ) {
        throw new DAVEWebArtifactAccessError(
    ...
    async createAuthorizedScheduleItem(item: ScheduleItem): Promise<string> {
      if (!client) throw new Error('The desktop cloud connection is not configured.');
      const projectId = exactScheduleItemProjectId(item);
      if (!projectId) throw exactTaskProjectConflict();
      const ownerId = await requireAuthorizedOwnerCached();
      const cloudUpdatedAt = new Date().toISOString();
      const { data, error } = await client
        .from('schedule_items')
        .insert(scheduleItemRow(item, ownerId, cloudUpdatedAt, projectId))
        .select('updated_at')
        .single();
    ...
    async updateAuthorizedScheduleItem(
      ...
      const { data, error } = await client
        .from('schedule_items')
        .update(scheduleItemRow(item, ownerId, cloudUpdatedAt, projectId))
        .eq('owner_id', ownerId)
        .eq('id', item.id)
    ...
    async uploadAuthorizedReferenceDocument({
      ...
      if (!client) throw new Error('The desktop cloud connection is not configured.');
      const scheduleProjectIds = scheduleItems.map(exactScheduleItemProjectId);
      if (scheduleProjectIds.some(projectId => !projectId)) {
        throw new DAVEWebDocumentMutationError(
          'conflict',
          'Every imported schedule task must have one exact project ID before upload.',
        );
      }
      const ownerId = await requireAuthorizedOwnerCached();
      ...
      if (scheduleItems.length > 0) {
        const rows = scheduleItems.map((item, index) => scheduleItemRow(
          item,
          ownerId,
          cloudUpdatedAt,
          scheduleProjectIds[index] as string,
        ));
    ...
    async uploadAuthorizedTaskPhoto({
      ...
      const contentSha256 = bytesToHex(sha256(new Uint8Array(bytes)));
      const projectId = exactScheduleItemProjectId(task);
      if (!projectId) throw exactTaskPhotoProjectConflict();
      const projectName = (
        task.scheduleProjectName ||
        task.projectName ||
        'Unassigned Project'
      ).trim();
      await requireCurrentTaskPhotoAuthority({
        client,
        ownerId,
        task,
        projectId,
    ...
    async saveAuthorizedReportArtifact({
      ...
      if (
        ...
        throw new DAVEWebDocumentMutationError(
          'conflict',
          'Choose one exact project before saving this report.',
        );
      }
      ...
      const document: ReferenceDocument & DAVEWebDocumentExtension = {
        ...
        importBatchId: null,
        webVersionGroupId: `report:${projectId}`,
        webReport: report,
      };
...
function scheduleItemRow(
  ...
  return {
    ...
    owner_id: ownerId,
    project_id: projectId,
    project_name: item.scheduleProjectName?.trim() || item.projectName,
...
function exactScheduleItemProjectId(item: Pick<ScheduleItem, 'projectId'>): string | null {
  return typeof item.projectId === 'string' &&
    item.projectId.length > 0 &&
    item.projectId.trim() === item.projectId
    ? item.projectId
    : null;
}

function exactTaskProjectConflict(): DAVEWebTaskMutationError {
  return new DAVEWebTaskMutationError(
    'conflict',
    'Refresh and choose one project with an exact project ID before saving this task.',
  );
}

function exactTaskPhotoProjectConflict(): DAVEWebDocumentMutationError {
  return new DAVEWebDocumentMutationError(
    'conflict',
    'Refresh and choose one task with an exact project ID before adding a photo.',
  );
}

function staleTaskPhotoAuthorityConflict(): DAVEWebDocumentMutationError {
  return new DAVEWebDocumentMutationError(
    'conflict',
    'This task or project changed before the photo was added. Refresh and try again.',
  );
}

async function requireCurrentTaskPhotoAuthority({
  client,
  ownerId,
  task,
  projectId,
  ...
  const expectedCloudUpdatedAt = task.cloudUpdatedAt?.trim() || '';
  if (!expectedCloudUpdatedAt) throw staleTaskPhotoAuthorityConflict();

  const { data: taskRow, error: taskError } = await client
    .from('schedule_items')
    .select('id, project_id, project_name, task_name, item_data, updated_at')
    .eq('owner_id', ownerId)
    .eq('id', task.id)
    .maybeSingle();
  if (taskError || !isRecord(taskRow) || !isRecord(taskRow.item_data)) {
    throw staleTaskPhotoAuthorityConflict();
  }
  const currentTask = taskRow.item_data;
  const expectedProjectName = projectName.trim().toLocaleLowerCase();
  const currentProjectName = String(
    currentTask.scheduleProjectName || currentTask.projectName || '',
  ).trim().toLocaleLowerCase();
  if (
    taskRow.id !== task.id ||
    exactScheduleItemProjectId({ projectId: taskRow.project_id }) !== projectId ||
    exactScheduleItemProjectId({ projectId: currentTask.projectId }) !== projectId ||
    currentTask.id !== task.id ||
    taskRow.updated_at !== expectedCloudUpdatedAt ||
    currentProjectName !== expectedProjectName ||
    String(currentTask.taskName || '').trim() !== task.taskName.trim() ||
    String(currentTask.locationName || '').trim() !== String(task.locationName || '').trim()
  ) throw staleTaskPhotoAuthorityConflict();
  ...
  if (
    ...
    projectRow.archived !== false ||
    String(projectRow.name || '').trim().toLocaleLowerCase() !== expectedProjectName
  ) throw staleTaskPhotoAuthorityConflict();
}
...
async function loadExactAuthorizedReportPhotoPath(
  client: SupabaseClient,
  ownerId: string,
  source: DAVEWebReportMediaSource,
): Promise<string> {
  if (
    source.bucket !== 'project-photos' ||
    !source.projectId ||
    source.projectId.trim() !== source.projectId ||
    !source.updateId ||
    source.updateId.trim() !== source.updateId ||
    !source.photoId ||
    source.photoId.trim() !== source.photoId ||
    !source.storagePath ||
    source.storagePath.trim() !== source.storagePath ||
    !source.contentSha256 ||
    !/^[a-f0-9]{64}$/.test(source.contentSha256)
  ) {
    throw new DAVEWebArtifactAccessError(
      'The approved report photo receipt is incomplete or malformed.',
    );
  }
  const { data, error } = await client
    .from('project_updates')
    .select('id,project_id,update_data')
    .eq('owner_id', ownerId)
    .eq('id', source.updateId)
    .maybeSingle();
  if (error || !isRecord(data) || data.id !== source.updateId || !isRecord(data.update_data)) {
    throw new DAVEWebArtifactAccessError(
      'The approved report photo is no longer present in the current project record.',
    );
  }
  const update = data.update_data;
  if (
    data.project_id !== source.projectId ||
    update.id !== source.updateId ||
    update.projectId !== source.projectId ||
    update.isArchived === true
  ) {
    throw new DAVEWebArtifactAccessError(
      'The current photo no longer belongs to the approved exact project and update.',
    );
  }
  const matches = Array.isArray(update.photos)
    ? update.photos.filter(photo => isRecord(photo) && photo.id === source.photoId)
    : [];
  if (matches.length !== 1) {
    throw new DAVEWebArtifactAccessError(
      'The approved report photo identity is missing or ambiguous in the current project record.',
    );
  }
  const photo = matches[0];
  const storagePath = typeof photo.cloudStoragePath === 'string'
    ? safeArtifactStoragePath(photo.cloudStoragePath)
    : null;
  const currentSha256 = daveWebReportPhotoSha256(photo as UpdatePhoto);
  if (storagePath !== source.storagePath || currentSha256 !== source.contentSha256) {
    throw new DAVEWebArtifactAccessError(
      'The current photo storage object no longer matches the approved report source receipt.',
    );
  }
  return storagePath;
}
...
function authoritativeReferenceDocumentRow(value: unknown): Record<string, any> | null {
  if (!isRecord(value)) return null;
  const rowId = typeof value.id === 'string' && value.id.trim() === value.id && value.id
    ? value.id
    : null;
  const data = isRecord(value.document_data) ? value.document_data : {};
  const embeddedIdSupplied = data.id !== null && data.id !== undefined && data.id !== '';
  const embeddedId = typeof data.id === 'string' && data.id.trim() === data.id && data.id
    ? data.id
    : null;
  if (!rowId || (embeddedIdSupplied && embeddedId !== rowId)) return null;
  return {
    ...value,
    id: rowId,
    document_data: { ...data, id: rowId },
  };
}
...
// The operational document row can contain a multi-megabyte extracted page
// index. The database removes only that redundant page/text payload once,
// under the same authenticated owner boundary, before returning metadata.
async function readOwnerReferenceDocumentRows(
  client: SupabaseClient,
  _ownerId: string,
): Promise<readonly unknown[]> {
