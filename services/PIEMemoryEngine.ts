...
function buildProjectStoryFromParts(
  ...
  const patternSummary = patterns.length > 0
    ? ` ECOS found ${patterns.length} recurring pattern${patterns.length === 1 ? '' : 's'}, led by ${patterns[0].title.toLowerCase()}.`
    : '';
    ...
    return [
      {
        ...
        summary: 'ECOS does not have project events, updates, schedule dates, documents, or reports to build a timeline.',
        startAt: null,
        endAt: null,
        eventCount: 0,
        updateCount: 0,
...
function buildMemoryGapsFromParts(parts: MemoryParts): PIEMemoryGap[] {
  ...
  if (!hasRecentUpdate) {
    gaps.push(gapItem({
      ...
      impact: 'ECOS may not know what changed most recently in the field.',
  ...
  if (!hasScheduleImport) {
    gaps.push(gapItem({
      ...
      summary: 'ECOS did not find an imported schedule source for this project.',
  ...
  if (parts.photos.length === 0) {
    gaps.push(gapItem({
      ...
      summary: 'ECOS did not find photos for this project.',
  ...
  if (!hasInspectionStatus) {
    gaps.push(gapItem({
      ...
      summary: 'ECOS did not find inspection events for this project.',
      impact: 'ECOS cannot tell whether inspections are blocking, passed, failed, or pending.',
    ...
    gaps.push(gapItem({
      ...
      summary:
        parts.documents.length === 0
          ? 'ECOS did not find related document metadata.'
          : 'ECOS found documents, but no current reference document is marked.',
  ...
  if (!hasReportHistory) {
    gaps.push(gapItem({
      ...
      summary: 'ECOS did not find generated report history for this project.',
      impact: 'ECOS cannot compare current communication needs against prior reporting cadence or commitments.',
...
function buildMemoryInsightsFromParts(
  ...
  if (riskConcerns.length > 0 || highPriorityPatterns.length > 0) {
    ...
    insights.push(insightItem({
      ...
      summary:
        concern?.summary ||
        pattern?.summary ||
        'ECOS memory sees project conditions that should be reviewed.',
  ...
  if (highPriorityGaps.length > 0 || gaps.length >= 3) {
    insights.push(insightItem({
      ...
      whyItMatters: 'ECOS can reason better when updates, schedule, photos, inspections, documents, and report history are present.',
  ...
  if (parts.intelligence.communicationReadiness.level === 'ready') {
    insights.push(insightItem({
      ...
      summary: 'ECOS found enough current context to support stakeholder communication.',
  ...
  if (patterns.length > 0 && parts.timelinePoints.length > 0) {
    insights.push(insightItem({
      ...
      summary: `ECOS found ${parts.timelinePoints.length} timeline point${parts.timelinePoints.length === 1 ? '' : 's'} and ${patterns.length} pattern${patterns.length === 1 ? '' : 's'}.`,
