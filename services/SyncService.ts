...
import { uploadAndCommitProjectCoverMutation } from './ProjectCoverSync';
...
export type LocalSyncPayload = {
  projects: string[];
  projectRecords?: Array<Readonly<{ id: string; name: string }>>;
...
type ProjectUpdatePayload = {
  ...
  isFavorite?: boolean;
  data?: JsonValue | null;
  coverPhotoCommit?: ProjectCoverCommitMutation;
  coverPhotoUpload?: {
    localUri: string;
    remotePath: string;
    mimeType: string;
...
export function queuedProjectDeletionBarriers(
  items: readonly SyncQueueItem[],
): Readonly<{
  exactProjectIds: readonly string[];
  legacyProjectNames: readonly string[];
}> {
  const exactProjectIds: string[] = [];
  const legacyProjectNames: string[] = [];
  for (const item of items) {
    if (item.entity !== 'project' || item.operation !== 'delete') continue;
    const payload = item.payload as { projectId?: unknown; name?: unknown };
    const projectId = exactProjectUpdateId(payload.projectId);
    if (projectId) {
      exactProjectIds.push(projectId);
      continue;
    }
    if (Object.prototype.hasOwnProperty.call(payload, 'projectId')) {
      throw new Error('Pending project deletion contains an invalid project ID.');
    }
    if (typeof payload.name === 'string' && payload.name.trim()) {
      legacyProjectNames.push(payload.name.trim());
    }
  }
  return Object.freeze({
    exactProjectIds: Object.freeze([...new Set(exactProjectIds)]),
    legacyProjectNames: Object.freeze([...new Set(legacyProjectNames)]),
  });
}
...
export async function queueProjectCreate(id: string, name: string): Promise<void> {
  const projectId = exactProjectUpdateId(id);
  const projectName = name.trim();
  if (!projectId || !projectName) {
    throw new Error('Project creation requires one canonical project ID and display name.');
  }
...
export async function queueProjectUpdate(
  ...
  if (projectUpdateCarriesCoverAuthority(payload) && !projectCoverAuthorityIsExact(payload)) {
    throw new Error('Cover photo updates require one exact project ID and storage path.');
  }
  if (!exactProjectUpdateId(payload.id)) {
    throw new Error('Project updates require one canonical project ID.');
  }
...
function exactProjectUpdateId(value: unknown): string | null {
...
function exactProjectCoverStoragePath(projectId: string, value: unknown): string | null {
  return typeof value === 'string' && projectCoverStoragePathKind(projectId, value)
    ? value
    : null;
}

function projectCoverAuthorityIsExact(payload: Partial<ProjectUpdatePayload>): boolean {
  const projectId = exactProjectUpdateId(payload.id);
  if (!projectId) return false;
  const data = payload.data && typeof payload.data === 'object' && !Array.isArray(payload.data)
    ? payload.data as Record<string, JsonValue | undefined>
    : null;
  const cover = data?.coverPhoto && typeof data.coverPhoto === 'object' && !Array.isArray(data.coverPhoto)
    ? data.coverPhoto as Record<string, JsonValue | undefined>
    : null;
  const dataPath = cover?.remotePath;
  if (!payload.coverPhotoCommit || !validProjectCoverCommitMutation(
    projectId,
    payload.coverPhotoCommit,
  )) return false;
  if (!sameProjectCoverAuthority(
    projectCoverAuthorityFromData(payload.data),
    payload.coverPhotoCommit.target,
  )) return false;
  if (payload.coverPhotoUpload) {
    if (data?.coverPhotoMode !== 'manual') return false;
    const uploadPath = exactProjectCoverStoragePath(projectId, payload.coverPhotoUpload.remotePath);
    if (
      !uploadPath ||
      projectCoverStoragePathKind(projectId, uploadPath) !== 'immutable' ||
      dataPath !== uploadPath ||
      cover?.contentSha256 !== payload.coverPhotoUpload.contentSha256 ||
      cover?.sizeBytes !== payload.coverPhotoUpload.sizeBytes
    ) return false;
  }
  if (data?.coverPhotoMode === 'manual') {
    return Boolean(
      exactProjectCoverStoragePath(projectId, dataPath) &&
      cover &&
      typeof cover.updatedAt === 'string' &&
      cover.updatedAt.trim() === cover.updatedAt &&
      cover.updatedAt.length > 0 &&
      data.coverPhotoUpdatedAt === cover.updatedAt
    );
  }
  return data?.coverPhotoMode === 'automatic';
}

function projectUpdateCarriesCoverAuthority(payload: Partial<ProjectUpdatePayload>): boolean {
  if (payload.coverPhotoUpload || payload.coverPhotoCommit) return true;
  if (!payload.data || typeof payload.data !== 'object' || Array.isArray(payload.data)) return false;
  return Object.prototype.hasOwnProperty.call(payload.data, 'coverPhoto') ||
    Object.prototype.hasOwnProperty.call(payload.data, 'coverPhotoMode') ||
    Object.prototype.hasOwnProperty.call(payload.data, 'coverPhotoUpdatedAt');
}

function projectArchiveQueueItemId(payload: ProjectUpdatePayload): string | null {
  const projectId = exactProjectUpdateId(payload.id);
  if (typeof payload.archived !== 'boolean' || !projectId) return null;
  return `project-archive-${encodeURIComponent(projectId)}`;
}
...
function sameProjectArchiveMutation(
  ...
  return (
    ...
    exactProjectUpdateId(currentPayload.id) === exactProjectUpdateId(attemptedPayload.id) &&
    Boolean(exactProjectUpdateId(attemptedPayload.id))
  );
}

export async function queueProjectDelete(
  projectId: string,
  name: string,
): Promise<void> {
  const exactProjectId = exactProjectUpdateId(projectId);
  const projectName = name.trim();
  if (!exactProjectId || !projectName) {
    throw new Error('Project deletion requires one exact project ID and display name.');
  }
  await enqueuePendingChange<ProjectDeletePayload>({
    entity: 'project',
    operation: 'delete',
    payload: { projectId: exactProjectId, name: projectName },
    changedAt: new Date().toISOString(),
  });
}
...
export async function stageProjectUpdateForSync(
  ...
  const receiptBoundUpdate = projectUpdateWithPhotoUploadReceipts(
    cloudRecoverableUpdate,
    photoAttempt.photoContentSha256ById,
  );
  await persistProjectUpdateRecord(
    receiptBoundUpdate,
    false,
...
async function runUploadPendingChanges(): Promise<SyncUploadResult> {
  ...
  const coverProjectsWaitingOnEarlierMutation = new Set<string>();
  let uploaded = 0;
  ...
  for (const item of uploadBatch) {
    const coverProjectId = queuedProjectCoverMutationProjectId(item);
    if (
      coverProjectId &&
      coverProjectsWaitingOnEarlierMutation.has(coverProjectId)
    ) {
      // Cover mutations form a predecessor chain. If an earlier mutation for
      // this exact project is retryable, a later mutation must remain queued
      // behind it. Sending the later mutation would turn a temporary failure
      // into a false predecessor conflict and permanently discard the user's
      // newer cover selection.
    ...
    if (
      ...
      if (coverProjectId) {
        coverProjectsWaitingOnEarlierMutation.add(coverProjectId);
      }
...
function queueItemMatchesDAVESyncTombstone(
  ...
  const recordId = queueItemDAVETombstoneRecordId(item);
...
export function queueItemDAVETombstoneRecordId(item: SyncQueueItem): string | null {
  const payload = item.payload as { id?: unknown };
  if (item.entity === 'project') return exactProjectUpdateId(payload.id);
...
> {
  ...
  const deletedProjectRecordIds = deletedDAVERecordIds(
  ...
  const projects = collectionErrors.projects === null && projectsResult.data
    ? projectsResult.data.filter(project =>
        !deletedProjectRecordIds.some(recordId =>
          projectTombstoneMatchesRecord(recordId, project, projectsResult.data || []),
        ))
    : [];
...
function requireLocalSyncProjectRecords(
  projectNames: readonly string[],
  projectRecords: readonly Readonly<{ id: string; name: string }>[],
): Array<Readonly<{ id: string; name: string }>> {
  const requestedNames = projectNames.map(name => name.trim());
  if (
    requestedNames.some(name => !name) ||
    new Set(requestedNames.map(normalizedProjectAuthorityKey)).size !== requestedNames.length
  ) {
    throw new Error('Full sync requires unique project display names.');
  }
  const selected = requestedNames.map(name => {
    const nameKey = normalizedProjectAuthorityKey(name);
    const matches = projectRecords.filter(record =>
      normalizedProjectAuthorityKey(record.name) === nameKey,
    );
    if (
      matches.length !== 1 ||
      typeof matches[0].id !== 'string' ||
      !matches[0].id ||
      matches[0].id !== matches[0].id.trim()
    ) {
      throw new Error(
        `Full sync requires one immutable project ID for “${name}”.`,
      );
    }
    return matches[0];
  });
  if (
    new Set(selected.map(record => normalizedProjectAuthorityKey(record.id))).size !==
    selected.length
  ) {
    throw new Error('Full sync project identities are duplicated.');
  }
  return selected;
}
...
export async function synchronizeLocalData(
  ...
  const projectRecords = payload.projectRecords || [];
  const configuration = getSupabaseConfigurationStatus();
  const errors: string[] = [];
  ...
  let syncableProjects = requireLocalSyncProjectRecords(
    payload.projects,
    projectRecords,
  );
  ...
  syncableProjects = syncableProjects.filter(project =>
    !deletedProjectRecordIds.some(recordId =>
      projectTombstoneMatchesRecord(recordId, project, projectRecords),
    ));
  ...
  const existingProjectIds = new Set(
    cloudProjects.data?.flatMap(project => {
      const projectId = exactProjectUpdateId(project.id);
  ...
  for (const project of syncableProjects) {
    ...
    const projectId = exactProjectUpdateId(project.id);
    if (!projectId) {
      errors.push(`Project “${normalizedName}” is missing its immutable identity.`);
      continue;
    }
    ...
    const result = await createProject({ id: projectId, name: normalizedName });
...
function queuedProjectCoverMutationProjectId(
  item: SyncQueueItem,
): string | null {
  if (item.entity !== 'project' || item.operation !== 'update') return null;
  const payload = item.payload as ProjectUpdatePayload;
  if (!payload.coverPhotoCommit) return null;
  return exactProjectUpdateId(payload.id);
}
...
async function uploadProjectQueueItem(
  ...
  if (
    ...
    !exactProjectUpdateId(payload.id)
  ) {
    return 'Project sync requires one canonical project ID.';
  }
  if (
    item.operation === 'update' &&
    projectUpdateCarriesCoverAuthority(payload) &&
    !payload.coverPhotoCommit
  ) {
    // Build 162 and earlier queued cover writes targeted one shared object and
    // carried no predecessor receipt. They cannot be safely replayed after the
    // atomic authority transition. Resolve only the obsolete queue row; the
    // local cover selection remains visible and can be reselected on the new
    // build if the user still wants to publish it.
    return 'conflict';
  }
  if (
    item.operation === 'update' &&
    projectUpdateCarriesCoverAuthority(payload) &&
    !projectCoverAuthorityIsExact(payload)
  ) {
    return 'Cover photo updates require one exact project ID and storage path.';
  }
  if (item.operation === 'update' && payload.coverPhotoCommit) {
    const currentProjects = await listProjects();
    const expectedName = String(payload.previousName || '').trim();
    const exactCurrentProjects = currentProjects.ok && !currentProjects.stubbed
      ? (currentProjects.data || []).filter(project =>
          canonicalProjectCoverId(project.id) === payload.id &&
          project.name.trim() === expectedName)
      : [];
    if (!currentProjects.ok || currentProjects.stubbed || !Array.isArray(currentProjects.data)) {
      return currentProjects.error || currentProjects.message ||
        'Project cover authority could not be checked.';
    }
    if (exactCurrentProjects.length !== 1) {
      await recordConflict({
        id: createQueueId('project_cover_conflict', new Date().toISOString()),
        entity: 'project',
        localId: payload.id!,
        localChangedAt: item.changedAt,
        remoteChangedAt: null,
        reason: 'The project cover changed or the project is no longer current.',
        detectedAt: new Date().toISOString(),
        localPayload: payload,
        remotePayload: exactCurrentProjects[0] || null,
      });
      return 'conflict';
    }
    const currentProject = exactCurrentProjects[0];
    const currentAuthority = projectCoverAuthorityFromData(currentProject.data);
    const sync = await uploadAndCommitProjectCoverMutation({
      mutation: payload.coverPhotoCommit,
      currentAuthority: async () => currentAuthority,
      ...(payload.coverPhotoUpload
        ? {
            uploadAttempt: async () => {
              const upload = await uploadPhoto({
                ...
                reportedSizeBytes: payload.coverPhotoUpload!.sizeBytes,
                expectedContentSha256: payload.coverPhotoUpload!.contentSha256,
                // Immutable revision paths are insert-only. A duplicate can
                // occur after a lost upload response; the database commit
                // independently verifies its exact digest and byte count.
                upsert: false,
                cacheControl: '86400',
              });
              const duplicate = !upload.ok && (
                upload.status === 409 ||
                /duplicate|already exists|resource already exists/i.test(
                  `${upload.error || ''} ${upload.message || ''} ${upload.code || ''}`,
                )
              );
              return {
                ok: (
                  upload.ok &&
                  !upload.stubbed &&
                  upload.data?.contentSha256 === payload.coverPhotoUpload!.contentSha256 &&
                  upload.data?.sizeBytes === payload.coverPhotoUpload!.sizeBytes
                ) || duplicate,
                message: upload.error || upload.message || null,
              };
            },
            removeAttempt: async () => removeProtectedStorageObject({
      ...
      commitMutation: async () => {
        const commit = await commitProjectCoverPhoto({
          projectId: payload.id!,
          expectedProjectName: expectedName,
          mutation: payload.coverPhotoCommit!,
        });
        return {
          ok: commit.ok && !commit.stubbed,
          receipt: commit.data || null,
          message: commit.error || commit.message || null,
        };
      },
    });
    if (sync.outcome === 'uploaded') {
      await clearConflictsForLocalRecord('project', payload.id!);
      return 'uploaded';
    }
    if (sync.outcome === 'conflict') {
      await recordConflict({
        ...
        localId: payload.id!,
        localChangedAt: item.changedAt,
        remoteChangedAt: currentAuthority.updatedAt,
        reason: sync.message || 'The project cover changed on another device.',
        detectedAt: new Date().toISOString(),
        localPayload: payload,
        remotePayload: currentProject,
      });
      return 'conflict';
    }
    return sync.message || 'Project cover sync is waiting for Supabase.';
  }
  const result =
    item.operation === 'create'
      ? await createProject({ id: payload.id, name: payload.name || 'Untitled Project' })
      : item.operation === 'delete'
        ? await deleteProject({
            projectId: payload.projectId,
            name: payload.name || payload.previousName || '',
          })
...
export async function uploadLocalPhotoWithDiagnostics(
  ...
  const existingContentSha256 = canonicalPhotoContentSha256(photo.contentSha256);
  if (
    cloudCopy.ok &&
    !cloudCopy.stubbed &&
    cloudCopy.data &&
    ...
    return {
      result: 'skipped',
      message: null,
      contentSha256: existingContentSha256,
      diagnostic: {
        ...diagnosticBase,
        bucketExists: 'yes',
  ...
  // Legacy objects without an immutable digest remain available in storage,
  // but cannot become report media until the exact local bytes can be uploaded
  // again and receipted. Do not invent authority from the storage path alone.
  if (
    !fileState.exists &&
    cloudCopy.ok &&
    !cloudCopy.stubbed &&
  ...
  try {
    ...
    if (result.ok && !result.stubbed) {
      return {
        result: 'uploaded',
        message: null,
        contentSha256: result.data?.contentSha256 || null,
        diagnostic: {
          ...diagnosticBase,
          bucketExists: 'yes',
          localFileExists: true,
...
export function projectUpdateWithPhotoUploadReceipts<TUpdate extends ProjectUpdate>(
  update: TUpdate,
  photoContentSha256ById: Readonly<Record<string, string>>,
): TUpdate {
  return {
    ...update,
    photos: update.photos.map(photo => {
      const uploadedSha256 = canonicalPhotoContentSha256(
        photoContentSha256ById[photo.id],
      );
      return uploadedSha256
        ? { ...photo, contentSha256: uploadedSha256 }
        : photo;
    }),
  };
}
...
> & {
  ...
  photoContentSha256ById: Record<string, string>;
}> {
  ...
  return {
    ...
    photoContentSha256ById: Object.fromEntries(results.flatMap((result, index) => {
      const contentSha256 = canonicalPhotoContentSha256(result.contentSha256);
      return contentSha256
        ? [[update.photos[index].id, contentSha256] as const]
        : [];
    })),
  };
}

function canonicalPhotoContentSha256(value: unknown): string | null {
