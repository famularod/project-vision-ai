...
function hasExactHostedRegionBinding({
  ...
  const page = (document.extractedPages ?? []).find(item => item.pageNumber === pageNumber);
  const authoritativeRegion = page?.regions?.find(item =>
    clean(item.id) === clean(citation?.regionId));
  return Boolean(
    citation &&
    citationEvidenceVersion.startsWith('ecos-hosted-evidence/') &&
    (hasECOSAuthorizedProof(document, evidence) || (
    ...
    validBounds(region) &&
    validBounds(authoritativeRegion) &&
    sameBounds(region, authoritativeRegion),
  );
}

function sameBounds(left: unknown, right: unknown) {
  if (!validBounds(left) || !validBounds(right)) return false;
  const leftBounds = left as Record<string, number>;
  const rightBounds = right as Record<string, number>;
  return (['x', 'y', 'width', 'height'] as const).every(key =>
    Number(leftBounds[key].toFixed(6)) === Number(rightBounds[key].toFixed(6)));
}
