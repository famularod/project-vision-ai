...
export function reconcileDAVEOperationalProjects({
  ...
  const pendingDeletesRecord = (record: ProjectRecord) => {
    const id = text(record.id);
    return Boolean(id && pending.deletedIds.has(id)) ||
      pending.deletedNames.has(normalizedName(record.name));
  };
  const cloudRecords = [...cloudActiveRecords, ...cloudArchivedRecords]
    .filter(record =>
      !deletedKeys.has(normalizedName(record.name)) && !pendingDeletesRecord(record));
  ...
  for (const localRecord of localRecords) {
    ...
    if (
      ...
      pendingDeletesRecord(localRecord)
    ) {
      continue;
    }
  ...
  for (const createdName of pending.created.values()) {
    if (
      ...
      pending.deletedNames.has(normalizedName(createdName)) ||
  ...
  cloudArchivedRecords.forEach(record => {
    ...
    if (key && !deletedKeys.has(key) && !pendingDeletesRecord(record)) {
      archived.set(key, record.name);
    }
  });
  ...
  pending.deletedNames.forEach(key => archived.delete(key));

  const visibleRecords = uniqueProjectRecords(
    projectRecords.filter(record => {
      const key = normalizedName(record.name);
      return key && !deletedKeys.has(key) && !pendingDeletesRecord(record);
    }),
  );
...
function queuedProjectState(changes: readonly DAVEOperationalQueuedProjectChange[]) {
  ...
  return { created, deletedIds, deletedNames, touched, archived, renames };
}
