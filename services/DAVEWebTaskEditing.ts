...
export type DAVEWebTaskProjectRecord = Readonly<{
  id?: string | null;
  name: string;
}>;
...
export function resolveExactDAVEWebTaskProjectId({
  projectName,
  current = null,
  ...
  projectName: string;
  current?: Pick<ScheduleItem, 'projectId' | 'projectName' | 'scheduleProjectName'> | null;
  selectedProjectId?: string | null;
  projectRecords: readonly DAVEWebTaskProjectRecord[];
}): string | null {
  const targetName = normalized(projectName);
  if (!targetName) return null;
  const exactRecords = projectRecords.filter(record =>
    exactProjectId(record.id) && normalized(record.name) === targetName,
  );
  const exactRecordForId = (projectId: string | null) => projectId &&
    exactRecords.filter(record => record.id === projectId).length === 1
    ? projectId
    : null;
  const currentProjectId = exactProjectId(current?.projectId);
  if (current) {
    const selectedProjectIdWasSupplied = selectedProjectId !== null && selectedProjectId !== undefined;
    const selected = exactProjectId(selectedProjectId);
    if (selectedProjectIdWasSupplied && (!selected || currentProjectId !== selected)) return null;
    return exactRecordForId(currentProjectId);
  }
  const selected = exactRecordForId(exactProjectId(selectedProjectId));
  if (selected) return selected;
  return exactRecords.length === 1 ? exactProjectId(exactRecords[0].id) : null;
}

/** Invocation-time guard for exact desktop project task mutations. */
export function daveWebTaskMatchesSelectedProject(
  item: Pick<ScheduleItem, 'projectId'>,
  selectedProjectId: string | null | undefined,
): boolean {
  if (selectedProjectId === null || selectedProjectId === undefined) return true;
  const selected = exactProjectId(selectedProjectId);
  return Boolean(selected && exactProjectId(item.projectId) === selected);
}
