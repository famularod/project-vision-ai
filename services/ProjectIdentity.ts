...
/** Restores only portable project records supplied by the backup itself.
 * Target-device name matches are never allowed to manufacture authority. */
export function restoreProjectRecords<T extends {
  id?: string | null;
  name: string;
}>(
  restoredRecords: readonly T[],
): Array<T & ProjectIdentityRecord> {
  const ids = new Set<ProjectId>();
  const names = new Set<string>();
  return restoredRecords.map(record => {
    const id = requireProjectId(record.id);
    const name = normalizeProjectDisplayName(record.name);
    const nameKey = name.toLocaleLowerCase().replace(/\s+/g, ' ');
    if (ids.has(id) || names.has(nameKey)) {
      throw new Error('Restored project records contain a duplicate immutable ID or display name.');
    }
    ids.add(id);
    names.add(nameKey);
    return { ...record, id, name } as T & ProjectIdentityRecord;
  });
}
