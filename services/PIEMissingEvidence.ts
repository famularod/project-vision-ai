...
function buildPhotoGap(
  ...
  return buildItem({
    ...
    summary: current
      ? `DAVE does not have a current photo for ${area}.`
      : `DAVE does not have photo evidence for ${area}.`,
    whyItMatters: 'A photo is the fastest way to verify actual field progress before DAVE strengthens recommendations or report language.',
...
function buildLocationGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE does not have enough location context to trust the project or area assignment.',
...
function buildScheduleGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE does not have a usable current schedule.',
...
function buildOwnerGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    type: 'missing_owner',
    title: 'Owner needed',
    summary: 'DAVE found work or an action item without a clear owner.',
...
function buildDecisionGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE has evidence that points to a decision, but the decision is not confirmed.',
    whyItMatters: 'DAVE should not present recommendations as settled when a user decision is still open.',
...
function buildInspectionGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE cannot tell whether inspection readiness has been verified.',
...
function buildSafetyGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE needs safety confirmation before it treats the area as clear.',
...
function buildProgressNoteGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  ...
  return buildItem({
    ...
    summary: `DAVE needs a short progress note for ${area}.`,
    whyItMatters: 'A short note tells DAVE what the photo means and improves report wording.',
...
function buildDocumentGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE does not have the supporting document needed for this item.',
...
function buildReportReviewGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE has report content that still needs user review.',
...
function buildUserConfirmationGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'DAVE needs confirmation before treating this understanding as reliable.',
    whyItMatters: 'User confirmation prevents DAVE from overstating uncertain evidence.',
    decisionAffected: 'Confidence and recommendation readiness.',
    smallestEvidenceRequest: 'Confirm, correct, or reject DAVE’s current understanding.',
    nextCaptureAction: 'Confirm or correct DAVE.',
    priority: 'medium',
    uncertaintyReduction: 18,
    source: 'User Confirmation',
  });
  ...
  return [
    ...quality.staleEvidence.slice(0, 2).map((item, index) =>
      buildItem({
        ...
        whyItMatters: 'Stale evidence should be refreshed before DAVE strengthens recommendations.',
...
function runtimeDrivenGaps(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem[] {
  return [
    ...
    buildItem({
      ...
      decisionAffected: 'DAVE recommendation readiness.',
...
function buildItem(config: {
  ...
  return {
    ...
    impact: {
      ...
      recommendationImpact: 'DAVE should reduce recommendation confidence until this evidence is supplied.',
