...
function buildPhotoGap(
  ...
  return buildItem({
    ...
    summary: current
      ? `PIE does not have a current photo for ${area}.`
      : `PIE does not have photo evidence for ${area}.`,
    whyItMatters: 'A photo is the fastest way to verify actual field progress before PIE strengthens recommendations or report language.',
...
function buildLocationGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE does not have enough location context to trust the project or area assignment.',
...
function buildScheduleGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE does not have a usable current schedule.',
...
function buildOwnerGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    type: 'missing_owner',
    title: 'Owner needed',
    summary: 'PIE found work or an action item without a clear owner.',
...
function buildDecisionGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE has evidence that points to a decision, but the decision is not confirmed.',
    whyItMatters: 'PIE should not present recommendations as settled when a user decision is still open.',
...
function buildInspectionGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE cannot tell whether inspection readiness has been verified.',
...
function buildSafetyGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE needs safety confirmation before it treats the area as clear.',
...
function buildProgressNoteGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  ...
  return buildItem({
    ...
    summary: `PIE needs a short progress note for ${area}.`,
    whyItMatters: 'A short note tells PIE what the photo means and improves report wording.',
...
function buildDocumentGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE does not have the supporting document needed for this item.',
...
function buildReportReviewGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE has report content that still needs user review.',
...
function buildUserConfirmationGap(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem {
  return buildItem({
    ...
    summary: 'PIE needs confirmation before treating this understanding as reliable.',
    whyItMatters: 'User confirmation prevents PIE from overstating uncertain evidence.',
    decisionAffected: 'Confidence and recommendation readiness.',
    smallestEvidenceRequest: 'Confirm, correct, or reject PIE’s current understanding.',
    nextCaptureAction: 'Confirm or correct PIE.',
    priority: 'medium',
    uncertaintyReduction: 18,
    source: 'User Confirmation',
  });
  ...
  return [
    ...quality.staleEvidence.slice(0, 2).map((item, index) =>
      buildItem({
        ...
        whyItMatters: 'Stale evidence should be refreshed before PIE strengthens recommendations.',
...
function runtimeDrivenGaps(input: PIEMissingEvidenceInput): PIEMissingEvidenceItem[] {
  return [
    ...
    buildItem({
      ...
      decisionAffected: 'PIE recommendation readiness.',
...
function buildItem(config: {
  ...
  return {
    ...
    impact: {
      ...
      recommendationImpact: 'PIE should reduce recommendation confidence until this evidence is supplied.',
