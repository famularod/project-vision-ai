...
export function recoverDAVEScheduleRecords({
  ...
  localRecords.forEach(record => {
    ...
    const projectIdentity = mergedScheduleProjectIdentity(record, cloudRecord);
    if (!projectIdentity.mergeable) {
      // Do not turn a malformed or conflicting exact identity into a legacy
      // name-authorized task. Quarantine both revisions until the durable row
      // is repaired with one canonical project ID.
      combined.delete(id);
      return;
    }
    combined.set(id, mergeScheduleRevisions(
      record,
      cloudRecord,
...
function mergeScheduleRevisions(
  ...
  return {
    ...base,
    // The cloud row has already passed the relational/embedded exact-project
    // check in listScheduleItems(). Older device snapshots predate projectId
    // persistence, so retain the exact cloud identity while preserving newer
    // PM progress and notes. Any name or exact-ID disagreement fails closed
    // instead of silently rebinding one task to another project.
    projectId,
...
function mergedScheduleProjectIdentity(
  local: ScheduleItem,
  cloud: ScheduleItem,
): { mergeable: boolean; projectId: string | null } {
  const localProjectIdSupplied = local.projectId !== null && local.projectId !== undefined;
  const cloudProjectIdSupplied = cloud.projectId !== null && cloud.projectId !== undefined;
  const localProjectId = canonicalScheduleProjectId(local.projectId);
  const cloudProjectId = canonicalScheduleProjectId(cloud.projectId);

  if (
    (localProjectIdSupplied && !localProjectId) ||
    (cloudProjectIdSupplied && !cloudProjectId) ||
    (localProjectId && cloudProjectId && localProjectId !== cloudProjectId)
  ) {
    return { mergeable: false, projectId: null };
  }
  if (localProjectId && cloudProjectId) {
    return { mergeable: true, projectId: localProjectId };
  }

  const localProjectName = normalized(
    local.scheduleProjectName || local.projectName,
  );
  const cloudProjectName = normalized(
    cloud.scheduleProjectName || cloud.projectName,
  );
  if (!localProjectName || localProjectName !== cloudProjectName) {
    return { mergeable: false, projectId: null };
  }
  return {
    mergeable: true,
...
function canonicalScheduleProjectId(value: unknown): string | null {
