...
function buildAttentionItems({
      ...
      reasons: [
        {
          ...
          summary: 'DAVE Reporter has a draft that requires user review before copy or email.',
      ...
      reasons: [
        {
          ...
          summary: 'DAVE has a photo comparison that should be accepted, edited, or rejected.',
  ...
  if (gpsNeedsCorrection) {
    items.push({
      ...
      whatMattersNow: 'DAVE is unsure of your location.',
      whyItMatters:
  ...
  if (walkArea && !items.some(item => item.id === 'attention-schedule')) {
    items.push({
      ...
      whyItMatters: 'DAVE selected this area from schedule, mission, and evidence signals.',
      priority: 'low',
      confidence: confidenceToScore(runtime.scheduleConfidence),
      reasons: [
        {
...
function fallbackAttentionItem(): PIEAttentionItem {
  return {
    ...
    whatMattersNow: 'DAVE is preparing today’s project attention.',
    whyItMatters: 'More project evidence will help DAVE recommend the next action.',
    priority: 'low',
    confidence: 50,
    reasons: [
      {
...
function buildWalkAttentionItem(input: PIEWalkAttentionInput): PIEAttentionItem {
  ...
  if (input.needsUserSelection) {
    return {
      ...
      currentProject,
      currentArea,
      whatMattersNow: 'DAVE is unsure of your Walk location.',
  ...
  if (input.photoConfirmation && input.nextAreaName) {
    return {
      ...
      whyItMatters: input.nextAreaReason || 'DAVE found another area that may need field evidence.',
      priority: 'medium',
      confidence: input.confidence,
      reasons: [
        {
  ...
  if (input.photoConfirmation || input.photoCount > 0) {
    return {
      ...
      whyItMatters: input.hasNote
        ? 'DAVE has photo and note evidence ready for review.'
        : 'A short note will help DAVE explain what changed.',
      priority: 'medium',
      confidence: input.confidence,
      reasons: [
        {
  ...
  if (!acceptedLocation) {
    return {
      ...
      currentProject,
      currentArea,
      whatMattersNow: `DAVE believes you are at ${locationSummary}.`,
  ...
  return {
    ...
    currentProject,
    currentArea,
    whatMattersNow: `DAVE needs one progress photo for ${currentArea || currentProject}.`,
    whyItMatters: input.recommendationReason || 'Current photo evidence will help DAVE explain progress.',
    priority: 'medium',
    confidence: input.confidence,
    reasons: [
      {
