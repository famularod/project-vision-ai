...
type ProjectRecordLike = Readonly<{ id?: string | null; name: string }>;
...
type ProjectDocumentLike = Readonly<{ id?: string; projectId: string }>;
...
/**
 * Computes the complete local cascade before any bytes are changed. An
 * immutable project ID is the only destructive ownership predicate. If a
 * legacy name-only child appears to belong to the selected project, deletion
 * stops before any operation is prepared so those bytes can be repaired
 * instead of guessed into a destructive cascade.
 */
...
>({
  ...
  draftHasUnboundProjectScope: boolean;
...
> {
  ...
  const exactAuthorityProjectId = exactProjectId(authorityProjectId);
  if (!exactAuthorityProjectId) {
    throw new Error('Project deletion requires an authoritative project id.');
  }
  const authorityRecords = projectRecords.filter(record =>
    exactProjectId(record.id) === exactAuthorityProjectId,
  );
  if (
    authorityRecords.length !== 1 ||
    normalizedScope(authorityRecords[0].name) !== normalizedProjectName
  ) {
    throw new Error('Project deletion authority does not match one exact project record.');
  }
  ...
  const nameMatches = (value: string | null | undefined) =>
    normalizedScope(value) === normalizedProjectName;
  const exactMatch = (projectId: string | null | undefined) =>
    exactProjectId(projectId) === exactAuthorityProjectId;
  const unboundLegacyScopes = [
    ...updates.filter(update =>
      !exactProjectId(update.projectId) &&
      projectUpdateBelongsToParentProject({ update, projectName, scheduleItems }),
    ).map(update => `project_update:${update.id}`),
    ...projectDocuments.filter(document =>
      !exactProjectId(document.projectId) && nameMatches(document.projectId),
    ).map((document, index) => `project_document:${document.id || index}`),
    ...referenceDocuments.filter(document =>
      !exactProjectId(document.projectId) && (
        nameMatches(document.projectName) ||
        (document.projectNames || []).some(nameMatches)
      ),
    ).map(document => `reference_document:${document.id}`),
    ...projectAreas.filter(area =>
      !exactProjectId(area.projectId) && nameMatches(area.projectName),
    ).map(area => `project_area:${area.id}`),
    ...scheduleItems.filter(item =>
      !exactProjectId(item.projectId) &&
      nameMatches(item.scheduleProjectName || item.projectName),
    ).map(item => `schedule_item:${item.id}`),
    ...(draftHasUnboundProjectScope ? ['active_draft'] : []),
  ];
  if (unboundLegacyScopes.length > 0) {
    throw new Error(
      `Project deletion is blocked until name-only child records have immutable project IDs: ${unboundLegacyScopes.join(', ')}.`,
    );
  }
  const updateMatchesProject = (update: TUpdate) => exactMatch(update.projectId);
  ...
  const deletionTombstones: DAVESyncTombstone[] = [
    {
      ...
      recordId: exactAuthorityProjectId,
  ...
  return Object.freeze({
    projectName,
    deletedAt: normalizedDeletedAt,
    remainingProjectRecords: projectRecords.filter(record =>
      exactProjectId(record.id) !== exactAuthorityProjectId,
    ),
    nextDeletedProjectNames: [...deletedProjectNames],
    remainingArchivedProjects: [...archivedProjects],
    remainingUpdates,
    removedUpdates,
    ...
    nextCloudIntents: mergeProjectDeletionCloudIntents(
      [{
        projectId: exactAuthorityProjectId,
        projectName,
        requestedAt: normalizedDeletedAt,
      }],
      cloudIntents,
...
export function referenceDocumentMatchesProject(
  ...
  const exactAuthorityProjectId = exactProjectId(authorityProjectId);
  const documentProjectId = exactProjectId(document.projectId);
  if (documentProjectId) {
    return Boolean(exactAuthorityProjectId && documentProjectId === exactAuthorityProjectId);
  }
  return allowLegacyNameFallback &&
    normalizedScope(document.projectName) === normalizedScope(projectName);
}
...
export function projectDocumentMatchesProject(
  ...
  const exactAuthorityProjectId = exactProjectId(authorityProjectId);
  const exactDocumentProjectId = exactProjectId(document.projectId);
  if (exactDocumentProjectId) {
    return Boolean(exactAuthorityProjectId && exactDocumentProjectId === exactAuthorityProjectId);
  }
  return allowLegacyNameFallback && documentProjectId === normalizedScope(projectName);
}
...
export function scheduleItemMatchesProject(
  ...
  const itemProjectId = exactProjectId(item.projectId);
  if (itemProjectId) {
    const exactAuthorityProjectId = exactProjectId(authorityProjectId);
    return Boolean(exactAuthorityProjectId && itemProjectId === exactAuthorityProjectId);
  }
  if (!allowLegacyNameFallback) return false;
...
export function mergeProjectDeletionCloudIntents(
  ...
  return uniqueBy(
    ...
    intent => intent.projectId
      ? `id:${normalizedScope(intent.projectId)}`
      : `legacy-name:${normalizedScope(intent.projectName)}`,
  );
}
...
export function withoutProjectDeletionCloudIntent(
  ...
  if (exactId) {
    return intents.filter(intent =>
      !intent.projectId || exactProjectId(intent.projectId) !== exactId,
    );
  }
  return intents.filter(intent =>
    Boolean(intent.projectId) || normalizedScope(intent.projectName) !== projectKey,
  );
}
...
function normalizeProjectDeletionCloudIntent(
  ...
  const projectId = value.projectId === undefined
    ? undefined
    : exactProjectId(value.projectId);
  const requestedAt = normalizedTimestamp(value.requestedAt);
  if (!projectName || !requestedAt || value.projectId !== undefined && !projectId) return null;
  return Object.freeze(projectId
    ? { projectId, projectName, requestedAt }
    : { projectName, requestedAt });
}
...
function exactProjectId(value: unknown): string | null {
