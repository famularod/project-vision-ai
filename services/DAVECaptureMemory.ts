export const DAVE_CAPTURE_MEMORY_VERSION = 'dave-capture-memory/1.1' as const;
...
export function createCaptureMemory(input: CreateCaptureMemoryInput): DAVECaptureMemory {
  ...
  return freezeMemory({
    ...
    id: clean(input.id),
    projectId: captureMemoryProjectId(input.projectId),
    status: 'draft',
    transcript,
...
export function correctCaptureMemory(
  ...
  assertDraft(memory);
  if (field === 'project') {
    throw new Error('Project correction requires an exact project ID.');
  }
...
export function correctCaptureMemoryProject(
  memory: DAVECaptureMemory,
  projectId: string,
  projectName: string,
  correctedAt: string,
  ...
  const value = clean(projectName);
  if (!value) throw new Error('Project correction requires a project name.');
  const correction = Object.freeze({
    field: 'project' as const,
    previousValue: memory.recommendedProject.value,
  ...
  const evidence = uniqueEvidence([...memory.evidence, Object.freeze({
    id: `correction:${memory.id}:${memory.corrections.length + 1}`,
    kind: 'pm_correction' as const,
    sourceRecordId: exactProjectId,
    summary: 'PM corrected project.',
  })]);
  return replace(memory, {
    projectId: exactProjectId,
    recommendedProject: Object.freeze({
      ...memory.recommendedProject,
      value,
      confirmed: true,
      confidence: 'high',
    }),
    corrections: [...memory.corrections, correction],
    evidence,
  });
}
...
export function confirmCaptureMemory(memory: DAVECaptureMemory, confirmedAt: string): DAVECaptureMemory {
  assertDraft(memory);
  captureMemoryProjectId(memory.projectId);
...
export function captureMemoryProjectId(value: unknown): string {
  if (typeof value !== 'string' || value !== value.trim() || value.length < 1 || value.length > 160
      || /[\u0000-\u001f\u007f]/.test(value)) {
    throw new Error('Capture memory project ID is invalid.');
  }
  return value;
}
