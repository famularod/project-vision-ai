...
export const GOOGLE_DRIVE_LINK_MAX_BYTES = ECOS_MAX_WEB_PDF_BYTES;
...
export async function fetchGoogleDriveMetadata(
  ...
  if (sizeBytes > GOOGLE_DRIVE_LINK_MAX_BYTES) {
    throw new GoogleDriveDocumentError(
      'too_large',
      'This PDF is larger than 96 MB. Optimize or split it before linking so the browser can process it safely.',
    );
  }
  return Object.freeze({
    provider: 'google_drive',
...
export async function downloadGoogleDriveBytes(
  ...
  const bytes = await readECOSBoundedResponseBytes(response, {
...
import { ECOS_MAX_WEB_PDF_BYTES } from './ECOSPDFResourceLimits';
