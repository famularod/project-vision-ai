...
import type { ECOSDocumentProofClaim } from './ECOSDocumentProofAuthority';
...
const ANSWER_SCHEMA = 'ecos-owner-source-answer/2.1';
...
export async function loadECOSProtectedDocumentPage({
  client,
  claim,
  citation,
  ...
  const normalized = normalizeECOSProtectedSourceCitation(citation, claim);
  if (!normalized) throw new ECOSProtectedDocumentPageError();
  const initial = await client.auth.getSession();
  const session = initial.data.session;
  const ownerId = session?.user?.id || '';
  const accessToken = session?.access_token || '';
  if (
    initial.error || !validUuid(ownerId) ||
    ownerId !== normalized.locator.owner_id ||
    !/^[\x21-\x7e]{1,8192}$/.test(accessToken)
  ) throw new ECOSProtectedDocumentPageError('Sign in before opening the cited page.');

  const requestId = Crypto.randomUUID();
  if (!validUuid(requestId, true)) throw new ECOSProtectedDocumentPageError();
  ...
  try {
    ...
    try {
      response = await fetchImpl(ECOS_PROTECTED_SOURCE_ENDPOINT, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          requestId,
          answerSchemaVersion: ANSWER_SCHEMA,
          citation: normalized,
        }),
      ...
      throw new ECOSProtectedDocumentPageError(
        sessionChanged
          ? 'Your signed-in workspace changed. Open the cited page again.'
          : controller.signal.aborted
            ? 'The protected cited page timed out. Try again.'
            : 'The protected cited page could not be reached.',
      );
    }
    if (
      response.redirected ||
    ...
    if (!response.ok) {
      throw new ECOSProtectedDocumentPageError(
        response.status === 401
          ? 'Sign in before opening the cited page.'
          : response.status === 403
            ? 'The current account cannot open this project source.'
            : response.status === 429
              ? 'The protected source viewer is busy. Try again shortly.'
              : 'The protected cited page is temporarily unavailable.',
      );
    }
    ...
    const page = validateSourceResponse(parsed, {
      requestId,
      projectId: claim.projectId,
      citation: normalized,
    });
    const final = await client.auth.getSession();
    if (sessionChanged || final.error || final.data.session?.user?.id !== ownerId) {
      throw new ECOSProtectedDocumentPageError(
        'Your signed-in workspace changed. Open the cited page again.',
      );
    }
...
function validateSourceResponse(
  ...
  if (
    ...
    digest(bytes) !== locator.raster_sha256 ||
