    ...
    const upload = new Upload(
      { uri: input.uri, sizeBytes: input.sizeBytes } as never,
      {
        ...
        metadata: {
          ...
          // Supabase persists custom TUS metadata only when it is encoded in
          // the reserved metadata field. Flat custom TUS keys are not written
          // to storage.objects.user_metadata and therefore cannot bind the
          // database commit receipt to the uploaded bytes.
          metadata: JSON.stringify({
            vitruviusContentSha256: input.contentSha256,
            vitruviusSizeBytes: String(input.sizeBytes),
          }),
        },
