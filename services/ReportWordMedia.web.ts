...
type ArtifactUrlResolver = (
  ...
  path: string,
  options?: Readonly<{ reportPhotoSource?: DAVEWebReportMediaSource }>,
) => Promise<string>;
...
export async function resolveWebReportWordMedia(args: {
  ...
  reportMediaSources: readonly DAVEWebReportMediaSource[];
  ...
  const photosByExactIdentity = new Map(
    args.updates.flatMap(update =>
      update.photos.map(photo => [reportPhotoIdentity(update.id, photo.id), { photo, update }] as const)),
  );

  for (const [index, source] of args.reportMediaSources.entries()) {
    const displayNumber = index + 1;
    const match = photosByExactIdentity.get(reportPhotoIdentity(source.updateId, source.photoId));
    if (!match || !reportPhotoMatchesApprovedSource(match.update, match.photo, source)) {
      unavailableMedia.push({
        id: source.photoId,
        kind: 'photo',
        label: `Photo ${displayNumber}`,
        reason: 'The current photo no longer matches the approved report source receipt.',
      });
      continue;
    }
  ...
  if (!source.contentSha256) {
    return {
      unavailable: {
        ...
        kind: 'photo',
        label: numberedLabel,
        reason: 'The approved report source receipt does not contain a canonical photo digest.',
      },
    };
  }
  try {
...
export async function resolveProtectedImageWithRetry(args: {
  ...
  return resolveProtectedArtifactWithRetry({
    bucket: args.bucket,
    path: args.path,
    getArtifactUrl: async (bucket, path) => args.getArtifactUrl(
      bucket,
      path,
      args.reportPhotoSource ? { reportPhotoSource: args.reportPhotoSource } : undefined,
    ),
    render: url => rasterizeImageUrl(url, args.region, args.expectedSha256),
  });
}
...
async function rasterizeImageUrl(
  ...
  if (expectedSha256) {
    const observedSha256 = bytesToHex(sha256(bytes));
    if (observedSha256 !== expectedSha256) {
      throw new Error('The protected photo bytes no longer match the approved report source receipt.');
    }
  }
...
function reportPhotoIdentity(updateId: string, photoId: string): string {
  return `${updateId}\u0000${photoId}`;
}

function reportPhotoMatchesApprovedSource(
  update: ProjectUpdate,
  photo: UpdatePhoto,
  source: DAVEWebReportMediaSource,
): boolean {
  const currentSha256 = daveWebReportPhotoSha256(photo);
  return source.bucket === 'project-photos' &&
    update.id === source.updateId &&
    update.projectId === source.projectId &&
    photo.id === source.photoId &&
    (photo.cloudStoragePath?.trim() || null) === source.storagePath &&
    currentSha256 === source.contentSha256;
}
