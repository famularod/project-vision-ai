...
import {
  ...
  ECOS_PDF_OCR_TIMEOUT_MILLISECONDS,
  ECOS_PDF_PARSE_TIMEOUT_MILLISECONDS,
  ECOS_PDF_RENDER_TIMEOUT_MILLISECONDS,
  readBoundedECOSPDFTextItems,
  remainingECOSPDFDocumentMilliseconds,
  type ECOSPDFDocumentResourceBudget,
...
async function extractPdf(
  ...
  assertECOSPDFByteBudget(bytes.byteLength);
  const resourceBudget = createECOSPDFDocumentResourceBudget();
  const pdfjs = await import('pdfjs-dist/legacy/build/pdf.mjs');
  if (typeof window !== 'undefined') {
    pdfjs.GlobalWorkerOptions.workerSrc = '/pdf.worker.min.mjs';
  }
  const loadingTask = pdfjs.getDocument({
    data: copyECOSPdfBytesForWorker(bytes),
  ...
  let pdf: any;
  try {
    pdf = await awaitECOSPDFLoadingTask(
      loadingTask as never,
      remainingECOSPDFDocumentMilliseconds(resourceBudget, ECOS_PDF_PARSE_TIMEOUT_MILLISECONDS),
    );
  ...
  try {
    for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
      ...
      if (resumedPage && (!analyzeDrawingPage || hasCompleteECOSDrawingVisualCoverage(resumedPage))) {
        assertECOSPDFPageRegionBudget(resumedPage.regions?.length || 0);
        consumeECOSPDFDocumentRegionBudget(resourceBudget, resumedPage.regions ?? []);
        pages.push(resumedPage);
        onProgress?.(pageNumber / pageCount);
        continue;
      }
      const page = await awaitECOSPDFWorkerTask(
      ...
      if (resumedPage) {
        ...
        const embeddedLines = await pdfTextLines(page, resourceBudget);
        ...
        if (needsDrawingOCR && ocrPageCount < MAX_OCR_PAGES) {
          ...
          const recognizedLines = await ocrPdfPage(page, ocrWorker, resourceBudget);
      ...
      assertECOSPDFPageRegionBudget(extractedPage.regions?.length || 0);
      ...
      if (analyzeDrawingPage) {
        ...
        if (!overviewAnalyzed || missingDeepReads.length > 0) {
          try {
            ...
            const overviewImageDataUrl = await renderPDFPageOverviewForVisualAnalysis(page, resourceBudget);
            for (const request of missingDeepReads) {
              throwIfExtractionCancelled(isCancelled);
              tileImages.push({
                bounds: request.bounds,
                imageDataUrl: await renderPDFPageTileForVisualAnalysis(page, request.bounds, resourceBudget),
              });
            }
      ...
      assertECOSPDFPageRegionBudget(extractedPage.regions?.length || 0);
      consumeECOSPDFDocumentRegionBudget(resourceBudget, extractedPage.regions ?? []);
...
async function pdfTextLines(
  page: any,
  resourceBudget: ECOSPDFDocumentResourceBudget,
): Promise<ExtractLine[]> {
  ...
  const rawItems = await readBoundedECOSPDFTextItems(
    textStream.getReader(),
    remainingECOSPDFDocumentMilliseconds(resourceBudget, ECOS_PDF_PARSE_TIMEOUT_MILLISECONDS),
    resourceBudget,
  );
  const items: ExtractLine[] = [];
...
async function ocrPdfPage(
  ...
  await awaitECOSPDFRenderTask(
    renderTask,
    remainingECOSPDFDocumentMilliseconds(resourceBudget, ECOS_PDF_RENDER_TIMEOUT_MILLISECONDS),
  );
  const result = await awaitECOSPDFOCRTask(
    worker.recognize(canvas, {}, { blocks: true }),
    () => worker.terminate(),
    remainingECOSPDFDocumentMilliseconds(resourceBudget, ECOS_PDF_OCR_TIMEOUT_MILLISECONDS),
  );
  ...
  if (shouldRunECOSConstructionNoteOCR(recognizedLines)) {
    ...
    if (noteBounds) {
      const noteLines = await ocrFocusedPDFRegion(canvas, worker, noteBounds, 3, resourceBudget);
  ...
  }, TITLE_BLOCK_OCR_SCALE, resourceBudget);
...
async function ocrFocusedPDFRegion(
  ...
  focusedCanvas.width = renderPlan.width;
  focusedCanvas.height = renderPlan.height;
  ...
  const focusedResult = await awaitECOSPDFOCRTask(
    worker.recognize(focusedCanvas, {}, { blocks: true }),
    () => worker.terminate(),
    remainingECOSPDFDocumentMilliseconds(resourceBudget, ECOS_PDF_OCR_TIMEOUT_MILLISECONDS),
  );
...
async function ocrImage(
  ...
  try {
    const result = await awaitECOSPDFOCRTask(
      worker.recognize(file, {}, { blocks: true }),
      () => worker.terminate(),
    );
  ...
  for (const block of Array.isArray(data?.blocks) ? data.blocks : []) {
    for (const paragraph of Array.isArray(block?.paragraphs) ? block.paragraphs : []) {
      for (const line of Array.isArray(paragraph?.lines) ? paragraph.lines : []) {
        seenLineCount += 1;
        assertECOSPDFTextItemBudget(seenLineCount);
        const rawText = typeof line?.text === 'string' ? line.text : '';
        const nextCharacterCount = characterCount + rawText.length;
        assertECOSPDFDecodedTextBudget({
          itemCount: seenLineCount,
          characterCount: nextCharacterCount,
          byteCount,
        });
        const nextByteCount = byteCount + encoder.encode(rawText).byteLength;
        assertECOSPDFDecodedTextBudget({
          itemCount: seenLineCount,
          characterCount: nextCharacterCount,
...
function pageFromLines(
  ...
  assertECOSPDFPageRegionBudget(lines.length);
...
async function renderPDFPageTileForVisualAnalysis(
  ...
  const baseViewport = page.getViewport({ scale: 1 });
  const renderPlan = boundedECOSPDFRenderPlan({
    sourceWidth: baseViewport.width * bounds.width,
    sourceHeight: baseViewport.height * bounds.height,
    targetWidth: VISUAL_TILE_RENDER_WIDTH,
    targetHeight: 4_096,
  });
  const viewport = page.getViewport({ scale: renderPlan.scale });
  const canvas = document.createElement('canvas');
...
async function renderPDFPageOverviewForVisualAnalysis(
  page: any,
  resourceBudget: ECOSPDFDocumentResourceBudget,
) {
  if (typeof document === 'undefined') throw new Error('Drawing vision requires a browser canvas.');
...
function mergeDistinctLines(
  ...
  assertECOSPDFPageRegionBudget(embedded.length);
  assertECOSPDFPageRegionBudget(recognized.length);
  const next = [...embedded];
  const cellSize = 0.03;
  const index = new Map<string, ExtractLine[]>();
  const addToIndex = (line: ExtractLine) => {
    const normalized = normalize(line.text);
    const xCell = Math.floor(line.x / cellSize);
    const yCell = Math.floor(line.y / cellSize);
    const key = `${normalized}\u0000${xCell}\u0000${yCell}`;
    const bucket = index.get(key);
    if (bucket) bucket.push(line);
    else index.set(key, [line]);
  };
  embedded.forEach(addToIndex);
  for (const line of recognized) {
    ...
    for (let xOffset = -1; xOffset <= 1 && !duplicate; xOffset += 1) {
      for (let yOffset = -1; yOffset <= 1 && !duplicate; yOffset += 1) {
        const bucket = index.get(`${normalized}\u0000${xCell + xOffset}\u0000${yCell + yOffset}`) ?? [];
        duplicate = bucket.some(existing =>
          Math.abs(existing.x - line.x) <= 0.03 && Math.abs(existing.y - line.y) <= 0.03
        );
      }
    }
    if (!duplicate) {
      assertECOSPDFPageRegionBudget(next.length + 1);
      next.push(line);
      addToIndex(line);
    }
  }
...
export function mergeECOSPdfTextItems(lines: readonly ExtractLine[]): ExtractLine[] {
  assertECOSPDFTextItemBudget(lines.length);
  const ordered = [...lines].sort((left, right) => left.y - right.y || left.x - right.x);
  const rows: ExtractLine[][] = [];
  for (const line of ordered) {
    const row = rows.at(-1);
    if (row && Math.abs(row[0].y - line.y) <= 0.006) row.push(line);
    else rows.push([line]);
  }
  const merged: ExtractLine[] = [];
  ...
  assertECOSPDFPageRegionBudget(merged.length);
  return merged;
}
