...
import {
  ...
  hasECOSHostedCommitReceipt,
} from './ECOSHostedPageGraphAuthority';
...
export function buildECOSDocumentReadiness(
  ...
  const hasHostedCommitReceipt = hasECOSHostedCommitReceipt(document);
  const hasAuthoritativePageGraph = pages.length > 0 &&
  ...
  const unboundLocalIndex = pages.length > 0 && (
    !canonicalSha256(document.contentSha256) ||
    !canonicalSha256(document.indexedContentSha256)
  );
  const untrustedLocalPageGraph = pages.length > 0 && !hasAuthoritativePageGraph;
  ...
  if (stale || unboundLocalIndex || untrustedLocalPageGraph || staleDrawingIndex) {
    return readiness(base, 'stale', 'Preparing for ECOS',
      staleDrawingIndex
        ...
        : untrustedLocalPageGraph
          ? 'The searchable page evidence does not match the server-issued hosted graph receipt.'
          : unboundLocalIndex
          ? 'The searchable page index is not cryptographically bound to this exact source file.'
  ...
  if (hostedStatus) {
    const needsOperatorReview = hostedStatus === 'Needs Review' ||
      hostedStatus === 'Reconnect Files';
    return readiness(
      base,
      needsOperatorReview ? 'failed' : 'pending',
      hostedStatus,
      clean(document.ecosHostedIndexCustomerMessage) || hostedStatusDetail(hostedStatus),
      false,
    );
  }
...
function hostedStatusDetail(status: NonNullable<ReferenceDocument['ecosHostedIndexStatus']>) {
  if (status === 'Waiting') return 'Background preparation is queued.';
  if (status === 'Preparing') return 'Vitruvius is preparing this document in the background.';
  if (status === 'Needs Review') return 'Background preparation needs review before ECOS can use this document.';
  if (status === 'Reconnect Files') return 'Reconnect the source file before ECOS can use this document.';
  if (status === 'Temporarily Unavailable') return 'Vitruvius could not confirm the current hosted preparation status.';
  return 'The hosted source is not ready for ECOS answers.';
}
