...
function buildFacts({
  ...
  const facts: PIEFact[] = [
    ...
    factItem({
      id: 'issue-counts',
      projectName,
      statement: `${intelligence.metrics.openIssueCount} open issue${intelligence.metrics.openIssueCount === 1 ? '' : 's'} and ${intelligence.metrics.safetyConcernCount} safety concern${intelligence.metrics.safetyConcernCount === 1 ? '' : 's'} are visible to ECOS.`,
  ...
  if (intelligence.communicationReadiness.level !== 'ready') {
    relationships.push(
      relationshipItem({
        ...
        summary: 'ECOS is connecting missing notes, photos, recipients, documents, or sync freshness to stakeholder communication quality.',
  ...
  if (intelligence.confidence.level === 'low') {
    concerns.push(
      concernItem({
        ...
        impact: 'ECOS may be missing project reality needed for confident decisions.',
        evidenceIds: evidenceIdsBySource(evidence, ['confidence']),
        confidence: 'high',
        priority: 'medium',
        source: 'confidence',
  ...
  if (questions.length === 0 && intelligence.recommendedNextAction.action === 'continue-monitoring') {
    questions.push(
      questionItem({
        ...
        reason: 'ECOS found no urgent risks, so the useful next question is whether the record is current.',
...
function buildCommunicationInsight({
  ...
  return {
    ...
    summary:
      intelligence.communicationReadiness.level === 'ready'
        ? 'ECOS found enough project context to support a stakeholder-ready update.'
        : intelligence.communicationReadiness.level === 'needs-context'
          ? 'ECOS can support a draft, but the communication should be reviewed for missing context.'
          : 'ECOS needs more project context before this is ready for stakeholder communication.',
    talkingPoints,
    ...
    thoughts.push(
      thoughtItem({
        ...
        summary:
          ...
          `ECOS found ${parts.intelligence.metrics.scheduleItemCount} schedule item${parts.intelligence.metrics.scheduleItemCount === 1 ? '' : 's'} for this project.`,
...
function questionForConcern(
  ...
  const mapping: Record<string, string> = {
    ...
    'missing-schedule': 'Which schedule or milestone should ECOS use for this project?',
    ...
    'low-confidence-concern': 'What missing project context would most improve ECOS confidence?',
...
function impactForAction(action: ProjectRecommendation['action']) {
  ...
  if (action === 'capture-update') {
    return 'Refreshes project reality so ECOS can reason from current field information.';
  }
