    ...
    const processing = (async () => {
      ...
      for (;;) {
        ...
        for (const intent of intents) {
          if (!intent.projectId) {
            throw new Error(
              'Pending project deletion is missing its immutable project ID.',
            );
          }
          ...
          const alreadyQueued = queue.some(item => {
            ...
            return item.payload.projectId === intent.projectId;
          });
          if (!alreadyQueued) {
            await queueCloudProjectDelete(intent.projectId, intent.projectName);
          }
          await mutateArray(
            storageKeys.cloudIntents,
            parseProjectDeletionCloudIntents,
            current => withoutProjectDeletionCloudIntent(
              current,
              intent.projectId,
              intent.projectName,
            ),
