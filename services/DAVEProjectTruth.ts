...
export type BuildDAVEProjectTruthInput = {
  ...
  /** Exact immutable project IDs whose memories belong to this truth scope. */
  captureMemoryProjectIds?: readonly string[];
...
export function buildDAVEProjectTruth(input: BuildDAVEProjectTruthInput): DAVEProjectTruth {
  ...
  const updates = input.updates.filter(update => projectRecordMatchesScope(
    input.projectId,
    projectKey,
    update.projectId,
    update.projectName,
  ...
  const scheduleItems = canonicalScheduleItems.filter(item => scheduleMatchesProject(
    input.projectId,
    projectKey,
    item,
  )).map(item => item.projectId
    ? { ...item, projectName: input.projectName, scheduleProjectName: input.projectName }
    : item);
  ...
  const captureMemoryProjectIds = new Set(
    (input.captureMemoryProjectIds ?? [input.projectId]).map(captureMemoryProjectId),
  );
  const captureMemories = (input.captureMemories ?? []).filter(memory =>
    captureMemoryProjectIds.has(memory.projectId),
  );
  ...
  const referenceDocuments = (input.referenceDocuments ?? []).filter(document => {
    ...
    return clean(document.projectId) === input.projectId;
  });
...
function scheduleMatchesProject(projectId: string, projectKey: string, item: ScheduleItem) {
  if (item.projectId !== null && item.projectId !== undefined) {
    return item.projectId.length > 0 && item.projectId === item.projectId.trim() && item.projectId === projectId;
  }
...
function projectRecordMatchesScope(
  projectId: string,
  projectKey: string,
  recordProjectId: string | null | undefined,
  recordProjectName: string | null | undefined,
) {
  if (recordProjectId !== null && recordProjectId !== undefined) {
    return recordProjectId.length > 0 &&
      recordProjectId === recordProjectId.trim() &&
      recordProjectId === projectId;
  }
  return projectMatches(projectKey, recordProjectName);
}
