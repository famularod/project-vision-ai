...
export type CommitProjectCoverPhotoParams = {
  projectId: string;
...
export function normalizeDAVEOperationalRealtimeRecord(
  ...
  if (entity === 'schedule_item') {
    const rowProjectId = exactPersistedProjectId(row.project_id);
    const embeddedProjectId = exactPersistedProjectId(jsonRecord.projectId);
    if (!rowProjectId || rowProjectId !== embeddedProjectId) return null;
  }
...
export async function uploadPhoto({
  ...
  const expectedSha256 = expectedContentSha256 === undefined || expectedContentSha256 === null
    ? null
    : expectedContentSha256.trim().toLowerCase();
  if (expectedSha256 !== null && !/^[a-f0-9]{64}$/.test(expectedSha256)) {
    return errorResult('A canonical SHA-256 upload identity is required.', 422, 'invalid_upload_sha256');
  }
  ...
  if (verifiedSizeBytes > RESUMABLE_UPLOAD_THRESHOLD_BYTES) {
    ...
    try {
      ...
      if (expectedSha256 && integrity.sha256 !== expectedSha256) {
        return errorResult(
  ...
  onProgress?.(0);
  const contentSha256 = Array.from(sha256(new Uint8Array(fileData)))
    .map(value => value.toString(16).padStart(2, '0'))
    .join('');
  if (expectedSha256 && contentSha256 !== expectedSha256) {
    return errorResult(
      'The selected file changed before upload.',
      409,
      'upload_content_changed',
  ...
  const { data, error } = await client.storage
    ...
    .upload(path, fileData, {
      ...
      upsert,
      metadata: {
        vitruviusContentSha256: contentSha256,
        vitruviusSizeBytes: String(fileData.byteLength),
      },
    });
...
export async function createProject(
  ...
  const projectId = exactPersistedProjectId(project.id);
  const projectName = project.name.trim();
  if (!projectId || !projectName) {
    return errorResult('Project creation requires one canonical project ID and display name.');
  }
...
export async function updateProject(
  ...
  if (
    projectDataCarriesCoverAuthority(project.data)
  ) {
    return errorResult(
      'Cover photo updates require the atomic project-cover commit authority.',
      409,
      'project_cover_atomic_commit_required',
    );
  }
  ...
  const projectId = exactPersistedProjectId(project.id);
  if (!projectId) {
    return errorResult('Project update requires one canonical project ID.');
  }
...
function normalizeProjectCoverCommitReceipt(value: unknown): ProjectCoverCommitReceipt | null {
  ...
  return {
    status: status as ProjectCoverCommitStatus,
    reason: typeof record.reason === 'string' ? record.reason : null,
  };
}

export async function commitProjectCoverPhoto({
  projectId,
  expectedProjectName,
  mutation,
}: CommitProjectCoverPhotoParams): Promise<SupabaseServiceResult<ProjectCoverCommitReceipt>> {
  const client = getSupabaseClient();
  if (!client) return notConfiguredResult<ProjectCoverCommitReceipt>();
  const owner = await requireAuthenticatedOwnerId(client);
  if (!owner.ok || !owner.data) {
    return errorResult(owner.error || 'Sign in is required.', owner.status, owner.code);
  }
  const exactProjectId = canonicalProjectCoverId(projectId);
  const projectName = expectedProjectName.trim();
  if (
    !exactProjectId ||
    ...
    return errorResult('Project cover commit requires one exact project and receipt.', 422);
  }

  const { data, error, status } = await client.rpc(
    'dave_commit_project_cover_photo',
    {
      p_project_id: exactProjectId,
      p_expected_project_name: projectName,
      p_expected_cover_photo: mutation.expected.coverPhoto,
      p_expected_cover_photo_mode: mutation.expected.mode,
      p_expected_cover_updated_at: mutation.expected.updatedAt,
      p_target_cover_photo: mutation.target.coverPhoto,
      p_target_cover_photo_mode: mutation.target.mode,
      p_target_cover_updated_at: mutation.target.updatedAt,
    },
  );
  if (error) {
    if (
      error.code === 'PGRST202' ||
      error.message.toLowerCase().includes('dave_commit_project_cover_photo')
    ) {
      return errorResult(
        'Atomic project-cover sync is not available yet. Apply the approved Vitruvius database migration, then retry.',
        status,
        error.code,
      );
    }
    return errorResult(
      'The project cover commit could not be confirmed. Its immutable upload is retained for a safe retry.',
      status,
      error.code,
    );
  }
  const receipt = normalizeProjectCoverCommitReceipt(data);
  if (!receipt) {
    return errorResult('The cloud did not return an exact project-cover commit receipt.', status);
  }
  return okResult(receipt, status);
}

function projectDataCarriesCoverAuthority(data: JsonValue | null | undefined): boolean {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return false;
  return Object.prototype.hasOwnProperty.call(data, 'coverPhoto') ||
    Object.prototype.hasOwnProperty.call(data, 'coverPhotoMode') ||
    Object.prototype.hasOwnProperty.call(data, 'coverPhotoUpdatedAt');
}
...
export async function deleteProject({
  ...
  const exactProjectId = exactPersistedProjectId(projectId);

  if (!exactProjectId || !projectName) {
    return errorResult('Project delete requires one exact project ID and display name.');
  }

  const { error, status } = await client.rpc(
    'dave_delete_project_atomically',
    { p_project_id: exactProjectId, p_project_name: projectName },
  );
  if (error) {
    if (
      error.code === 'PGRST202' ||
      ...
      return errorResult(
        'Exact-ID protected project deletion is not available yet. Apply the approved Vitruvius database migration, then retry.',
        status,
        error.code,
      );
    }
...
> {
  ...
  const projectId = exactPersistedProjectId(updateRecord.projectId);
  if (!projectId) {
    return errorResult(
      'Field update sync requires one exact project ID in both the row and payload.',
      409,
      'exact_project_id_required',
    );
  }
  if (
    (typeof updateRecord.id === 'string' && updateRecord.id !== id) ||
    (
      typeof updateRecord.projectName === 'string' &&
      updateRecord.projectName.trim() !== projectName.trim()
    )
  ) {
    return errorResult(
      'Field update identity changed before cloud persistence. Refresh and try again.',
      409,
  ...
  const normalized = normalizeProjectUpdate<TUpdate>(data);
  if (!normalized) {
    return errorResult(
      'The cloud did not confirm the exact field-update project identity.',
      409,
      'cloud_acknowledgement_missing',
    );
  }
  return okResult(normalized, status);
}
...
> {
  ...
  return okResult(
    result.rows
      .map(row => normalizeProjectUpdate<TUpdate>(row))
      .filter((row): row is CloudProjectUpdate<TUpdate> => Boolean(row)),
    result.status,
  );
}
...
export async function upsertScheduleItem(
  ...
  const projectId = exactPersistedProjectId(item.projectId);
  if (!projectId) {
    return errorResult(
      'Task sync requires one exact project ID in both the row and payload.',
      409,
      'exact_project_id_required',
    );
  }
  ...
  if (!projectName) {
    return errorResult('Task sync requires its confirmed project name.', 409);
  }
  ...
  const { data, error, status } = await client
    ...
    .select('id, project_id, item_data')
    .single();
  ...
  const acknowledged = exactPersistedProjectId(toRecord(data).project_id) === projectId &&
    exactPersistedProjectId(toRecord(toRecord(data).item_data).projectId) === projectId &&
    await confirmScheduleItemCloudAcknowledgement(
    item,
    data,
    async () => {
      ...
      if (
        exactPersistedProjectId(toRecord(confirmation.data).project_id) !== projectId ||
        exactPersistedProjectId(
          toRecord(toRecord(confirmation.data).item_data).projectId,
        ) !== projectId
      ) return null;
...
export async function upsertReferenceDocument(
  ...
  if (document.projectName?.trim() && !exactPersistedProjectId(document.projectId)) {
    return errorResult(
      'Project documents require one exact project ID before cloud persistence.',
      409,
      'exact_project_id_required',
    );
  }
...
export async function listReferenceDocuments(): Promise<SupabaseServiceResult<ReferenceDocument[]>> {
  const result = await listOwnedJsonRecords<ReferenceDocument>({
    table: REFERENCE_DOCUMENTS_TABLE,
  ...
  if (!client) {
    return okResult(result.data.map(document => reconcileECOSHostedIndexStatus(
      document,
      null,
      'unavailable',
    )), result.status, result.message);
  }
  let snapshot: Awaited<ReturnType<typeof loadECOSHostedIndexStatusSnapshot>> = {
    authority: 'unavailable',
    statuses: [],
  };
  try {
    snapshot = await loadECOSHostedIndexStatusSnapshot({
      client,
      documentIds: result.data.map(document => document.id),
    });
  } catch {
  ...
  const statusByDocument = new Map(snapshot.statuses.map(status => [status.documentId, status]));
  return okResult(result.data.map(document => reconcileECOSHostedIndexStatus(
    document,
    statusByDocument.get(document.id) ?? null,
    snapshot.authority,
  )), result.status, result.message);
}
...
async function listOwnedJsonRecords<T>({
  ...
  exactProjectBinding = false,
}: {
  table: string;
  jsonColumn: string;
  includeCloudUpdatedAt?: boolean;
  exactProjectBinding?: boolean;
}): Promise<SupabaseServiceResult<T[]>> {
  const client = getSupabaseClient();
  if (!client) return notConfiguredResult<T[]>();
  ...
  const result = await paginateSupabaseCollection(async ({ from, to, includeExactCount }) =>
    client
      .from(table)
      .select(
        `id, updated_at, ${exactProjectBinding ? 'project_id, ' : ''}${jsonColumn}`,
        { count: includeExactCount ? 'exact' : undefined },
      )
      .eq('owner_id', owner.data)
  ...
  const records = result.rows
    .map(row => {
      ...
      if (exactProjectBinding) {
        const rowProjectId = exactPersistedProjectId(databaseRow.project_id);
        const embeddedProjectId = exactPersistedProjectId(jsonRecord.projectId);
        if (!rowProjectId || rowProjectId !== embeddedProjectId) return null;
      }
...
function normalizeProjectUpdate<TUpdate>(
  value: unknown,
): CloudProjectUpdate<TUpdate> | null {
  const row = toRecord(value);
  const updateData = toRecord(row.update_data);
  const rowProjectId = exactPersistedProjectId(row.project_id);
  const embeddedProjectId = exactPersistedProjectId(updateData.projectId);
  if (!rowProjectId || rowProjectId !== embeddedProjectId) return null;
  if (
    typeof updateData.id === 'string' &&
    String(row.id || '') !== updateData.id
  ) return null;
...
function exactPersistedProjectId(value: unknown): string | null {
