...
export function identifyExecutiveConstraints(
  ...
  const evidenceConstraint = input.evidenceQuality?.evidenceReadiness &&
    ...
    ? {
        ...
        limits: 'Verification is required before a confident executive recommendation.',
...
export function explainExecutiveJudgment(
  ...
  return {
    summary: highestValueAction?.governance.why || 'ECOS needs stronger evidence before a confident executive action.',
...
export function analyzeExecutiveTradeoffs(
  ...
  return {
    summary: preferredOption
      ...
      : 'ECOS needs stronger evidence before comparing executive options.',
    dimensions,
    options,
    preferredOption,
...
export function analyzeEscalationNeed(
  ...
  return {
    ...
    justification: shouldEscalate
      ...
      : evidenceIsWeak
        ? 'Escalation should wait until ECOS Assurance verifies the evidence unless safety risk becomes clear.'
    ...
    escalationRisk: shouldEscalate
      ...
      : 'Unnecessary escalation may create noise and reduce trust in ECOS recommendations.',
...
export function explainTradeoffDecision(
  ...
  if (!preferredOption) {
    return 'ECOS cannot compare options until stronger evidence is available.';
  }
  ...
  const evidenceBoundary = input.evidenceQuality?.evidenceReadiness &&
    !['strong', 'good'].includes(input.evidenceQuality.evidenceReadiness)
    ? ' Evidence is not strong, so ECOS favors verification before irreversible action.'
    : '';
  const noiseBoundary = preferredOption.unnecessaryNoiseRisk <= 3
    ? ' It also avoids unnecessary noise.'
    : ' Unnecessary communication noise should remain under review.';
...
function actionFromConstraint(
  ...
  return createAction({
    ...
    expectedOutcome: 'ECOS can make a stronger executive recommendation after the constraint is cleared.',
...
function noActionCandidate(
  ...
  return createAction({
    ...
    expectedOutcome: 'ECOS continues monitoring without creating unnecessary work.',
...
function readinessReason(
  ...
  if (readiness === 'Uncertain') return 'ECOS does not have enough confidence to treat this as final.';
