...
2. create and edit a task, including percent and status consistency;
3. complete guided voice or text capture, including Skip;
4. capture a field note and verify desktop receipt/editing;
5. add a document, leave the page, and resume background preparation;
6. ask ECOS a supported question and open its exact current-document proof; and
7. review a report without an unapproved write.

For the same exact version, build, and source revision, complete the structured
function matrix on iPhone, iPad, and web. Each surface must prove cover select,
replace, and remove propagation; exact document count, list, and open behavior;
and uploaded-photo source/observed byte-count and SHA-256 parity. Each surface
must also exercise offline save, restart recovery, reconnect propagation,
duplicate-write prevention, and visible conflict preservation. The final
sync-zero record must show zero queued items, conflicts, errors, missing photos,
queued changes, and recovery copies; no recovery state available; a valid last
sync time; and successful peer propagation. Attach durable behavior evidence to
each platform's `exactBuildEvidenceRefs`; a broad journey pass alone does not
satisfy this matrix. Native entries must retain `physical_device` as their
evidence source and web must retain `interactive_web`; simulator or automated
source-check evidence cannot fill these fields.
...
candidate, an accessibility or recovery scenario is pending, ownership or
operating decisions are missing, pilot thresholds are missed, evidence files do
not exist, or a blocking defect remains open.
