...
| P0-02 | Partial — immutable identity contract, restart-safe UUID migration (1,309 lines), cloud identity preservation landed (7fd54636, 7c05fca7, 0d98ed6e); full child-record enforcement and rename/collision device matrix pending | A1.1 | Technical Lead + Data/Supabase + Mobile | Verified UUID migration and rollback; rename, punctuation, non-ASCII, duplicate-name, area-collision, and Project A/B isolation tests | Open |
| P0-03 | Partial — singular role selection aligned with schema (ddcf401f); staging authorization matrix and cloudTrusted proof pending | A1.2 | Data/Supabase + Security | Staging query/schema contract test using singular `role`; owner and non-owner authorization matrix; visible fail-closed mismatch state | Open |
...
| P0-06 | Partial — PhotoAnalysisCoordinator exact-target guard, analysis targets, byte/version identity landed (bdfda3af, 95827aea, f203f9ec); device race matrix pending | A4.1 | Mobile + Intelligence | Project switch, new draft, delete/replace, changed bytes, out-of-order completion, retry, background, and stale GPS tests proving exact generation targeting | Open |
...
| P0-09 | Partial — owned-file manifest boundary and verified copies landed (77c27596, 6f0c0535); restore-path integration and traversal device tests pending | A2.3, A2.4 | Mobile + Security | Owned-file manifest tests for traversal, encoded separators, absolute URI, sibling prefix, missing manifest, legitimate restore, and ownership-verified delete | Open |
| P0-10 | Partial — DurableLocalTransaction journal and verified save-before-clear landed (0ff6e439, bf67e3ee); failure-injection device tests pending | A2.1 | Mobile | Failure-injection tests for disk full, storage exception, double tap, edit/cancel during save, and process kill proving UI success follows verified durable commit | Open |
| P0-11 | Partial — fail-closed startup reads and hydration-gated actions landed (0de38cf3, 60836359); per-domain zero-fallback-write proof pending | A2.2 | Mobile + QA/Release | Per-domain hydration tests proving transient/corrupt reads cause zero fallback writes and block save, sync, restore, authority, and export until recovery | Open |
...
| P1-06 | Partial — complete uncapped authority signature (8d39ecf3); refresh-forcing coverage pending | A1.4 | Intelligence + Mobile | Canonical complete versioned signature tests cover all schedule, contact, area, sync, and uncapped entity changes and force correct refresh | Open |
| P1-07 | Partial — analysis identity bound to content bytes and versions (f203f9ec); server upsert verification pending | A4.1, A4.4 | Mobile + Intelligence | Evidence, cache, and analyzer-run identity includes content SHA plus analyzer, prompt, and policy version; changed bytes cannot reuse a stale result | Open |
| P1-08 | Partial — duplicate byte lineage preserved instead of failure (290ceadd); constraint staging test pending | A4.4 | Intelligence + Data/Supabase | Duplicate-byte tests resolve the existing evidence or explicit lineage without analysis failure; `duplicate_of_evidence_id` and version behavior verified | Open |
| P1-09 | Partial — prior selection orders by immutable capturedAt via resolveImmutablePhotoCapturedAt (landed earlier); timezone/equal-time device cases pending | A4.1, A4.4 | Mobile + Intelligence | Prior-photo ordering uses immutable per-photo `capturedAt`; analysis/import/send timestamps remain distinct; timezone and equal-time cases pass | Open |
| P1-10 | Partial — metadata-first selection, only winning pair prepared (f2fcfb60); large-set memory profile pending | A4.4 | Mobile + Intelligence | Memory/profile test with large candidate set proves metadata-first selection, one winning pair prepared, and byte/dimension limits before base64 | Open |
...
| P1-15 | Partial — stable blocker identities with explicit close events (b3351887); parser-corpus negation cases pending | A1.3, A1.5 | Intelligence + QA/Release | Stable blocker ID/type and explicit close-event tests cover negation, same-area unrelated issues, safety, reopen, and resolution | Open |
...
| P1-17 | Partial — CSV parsed into records before normalization (8b41f54c); full RFC case matrix pending | A5.3 | Mobile + Intelligence | RFC-compatible CSV tests cover quoted commas, CRLF, slash dates, multiline cells, BOM, and malformed-row reporting | Open |
| P1-18 | Partial — exact status enums and status/percent invariant (04521428); device regression pending | A1.3, A5.3 | Intelligence + QA/Release | Exact status normalization and status/percent invariant tests reject Incomplete, Not complete, and Not done as Complete | Open |
...
| P1-23 | Partial — stale uploading recovers to retryable failed at hydration (a2f16ffa); owned-copy-before-record and cloud byte download still open | A2.3, A2.4 | Mobile | Selected documents are copied to owned persistence before record creation; stale uploading becomes retryable; verified cloud download restores missing local bytes | Open |
...
| P1-37 | Partial — currentDecisionSnapshot operative across validation/automation/UI, unit-tested (e5b7fe7c); full transition matrix pending | A5.1 | Intelligence | All transition, automation, validation, and UI consumers resolve `versions[currentVersion]`; original snapshot remains audit-only | Open |
| P1-38 | Partial — supersession derived from append-only history for cloud+local reads, unit-tested (7c2ad3c0); staging verification pending | A5.1 | Intelligence + Data/Supabase | Append-only supersession event or `supersedes_id` persists locally/cloud without rewriting old rows and resolves the current judgment correctly | Open |
...
| P1-42 | Partial — re-entry rejected, snapshot generation, unchanged-draft-gated clear verified in code (bf67e3ee); device double-tap test pending | A2.1 | Mobile | Save re-entry is serialized, draft generation is snapshotted, and only the exact committed generation is cleared while later edits survive | Open |
| P1-43 | Partial — cancel inert during commit incl. system back, state labeled (bb66f90c); device race test pending | A2.1 | Mobile | Memory save either disables cancel while committing or invalidates via generation token; cancel/save race tests prove accurate UI and persistence | Open |
| P1-44 | Partial — volatile timestamps excluded from semantic fingerprints (e095fab0); repeated-recompute device proof pending | A1.5, A5.4 | Intelligence + Data/Supabase | Semantic fingerprints exclude volatile nested timestamps and preserve meaningful revision history under repeated equivalent recomputation | Open |
| P1-45 | Partial — history fingerprint migration written (e095fab0, includes SQL migration NOT yet applied to live DB — requires sign-off + supabase db push); staging A-B-A test pending | A1.5, A5.4 | Intelligence + Data/Supabase | A-to-B-to-A history persists or links the historical snapshot; deduplication compares against head and retries satisfy constraints | Open |
| P1-46 | Partial — explicit perturbation rescoring with disqualification (8e9dff81); adversarial cases pending | A5.1, A5.4 | Intelligence | Sensitivity analysis perturbs explicit inputs, rescoring all eligible options, excluding disqualified choices, and computing robustness from margins | Open |
...
| P1-50 | Partial — hydration gate, 100-entry cap, visible write failures, non-reentrant capture, unit-tested (0791d89d); quota/device tests pending | A2.1, A2.2, A5.5 | Mobile | Input is gated until hydration, submissions serialize, history is bounded/pageable, writes are journaled, and quota/failure is visible | Open |
...
| P1-52 | Partial — follow-through lifecycle history retained (858e017f); reactivation device test pending | A1.5, A5.4 | Intelligence | Follow-through history retains inactive/resolved records, firstSeen, cadence, and deterministic reactivation after temporary disappearance | Open |
| P1-53 | Partial — mission criteria require typed evidence (48a056ea); zero-evidence adversarial corpus pending | A1.3, A1.5, A5.4 | Intelligence + QA/Release | Mission criteria evaluate typed evidence only; description words cannot satisfy a criterion in zero-evidence adversarial tests | Open |
...
| P1-56 | Partial — workspace binds to opened update project on sent/queued path (e972b006); A/B device test pending | A1.1, A5.2 | Mobile + Intelligence | Opening any update binds or derives its project context; Back, Talk, reports, and actions remain on the same project in A/B tests | Open |
...
| P1-58 | Partial — version compare-and-swap before applying hydrated cover bytes (82296b4e); device race test pending | A2.2, A5.6 | Mobile | Cover hydration uses immutable version/generation compare-and-swap; delayed old bytes cannot overwrite a new or removed cover | Open |
...
| P2-02 | Partial — Maestro flows realigned to live navigation (c23f8d9a); CI execution on both platforms pending | A6.1 | QA/Release | Current Overview, Tasks, Talk, and Reports Maestro flows execute on iOS and Android in CI and cover real camera/file/update journeys | Open |
...
| P2-05 | Partial — test runs Jest and mobile CI workflow added (ea1b1003); clean-clone full CI with exports/secret scan/migrations pending | A6.1 | QA/Release + Technical Lead | Default test runs type checks plus Jest; clean CI performs install, checks, unit/behavior tests, exports, secret scan, and migration validation | Open |
...
| P2-16 | Partial — renderer peers aligned, npm override removed (2079dce3); full-suite verification recorded | A6.4 | Mobile + QA/Release | React and testing-library peer graph is compatible without an unsafe override and full Jest/React Native tests pass | Open |
...
| P2-18 | Partial — store:false set for vision provider (f8c5002f); deployed edge verification pending (function redeploy requires sign-off) | A4.3, A6.4 | Security/Privacy + Data/Supabase | Provider retention option is explicit and tested; privacy/data-flow documentation matches deployed edge behavior | Open |
| P2-19 | Partial — corrupt offline queue bytes quarantined (0abfc42c); recovery/export device test pending | A6.4 | Security/Privacy + Mobile | Corrupt queue bytes are quarantined unchanged, valid empty queue initializes separately, and recovery/export plus retry tests pass | Open |
