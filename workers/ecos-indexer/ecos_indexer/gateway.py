...
SOURCE_ACCOUNTABILITY_RPC_READ_TIMEOUT_SECONDS = 25
...
class SupabaseWorkerGateway:
    ...
    def rpc(self, name: str, payload: dict[str, Any]) -> Any:
        ...
        read_timeout_seconds = (
            ...
            else SOURCE_ACCOUNTABILITY_RPC_READ_TIMEOUT_SECONDS
            if name in (
                "ecos_snapshot_hosted_source_accountability",
                "ecos_record_hosted_page_source_excerpts",
                "ecos_load_hosted_page_source_excerpt_heads",
                "ecos_record_hosted_page_table_sources",
                "ecos_load_hosted_page_table_source_heads",
                "ecos_snapshot_hosted_table_source_accountability",
            )
            else DEFAULT_RPC_READ_TIMEOUT_SECONDS
        )
        sanitized_payload = sanitize_database_json(payload)
        if name in ("ecos_record_hosted_page_source_excerpts", "ecos_record_hosted_page_table_sources") and sanitized_payload != payload:
            # Native source text offsets and hashes must refer to the exact
            # transported bytes. Legacy checkpoint sanitation is not authority.
            raise ProtectedGatewayError("V2 native source payload contains unsupported text encoding")
    ...
    def prune_stale_visual_exceptions(
        ...
        *,
        ...
        """Remove only obsolete non-resolved rows under the active claim."""
