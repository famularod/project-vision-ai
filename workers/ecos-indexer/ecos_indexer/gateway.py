...
class SupabaseWorkerGateway:
    ...
    def prune_stale_visual_exceptions(
        self,
        job: HostedJob,
        *,
        page_number: int,
        evidence_version: str,
        current_exceptions: list[dict[str, str]],
    ) -> int:
        """Remove only obsolete unresolved rows while the exact claim is live."""

        result = self.rpc("ecos_prune_stale_hosted_visual_exceptions_v1", {
            "p_job_id": job.job_id,
            "p_claim_token": job.claim_token,
            "p_page_number": page_number,
            "p_evidence_version": evidence_version,
            "p_current_exceptions": current_exceptions,
        })
        if isinstance(result, bool) or not isinstance(result, int) or result < 0:
            raise ProtectedGatewayError(
                "Stale visual exception reconciliation returned an invalid count"
            )
        return result
    ...
    def dismiss_visual_exception(
        ...
        *,
        ...
        """Persist an exact dual-provider agreement that a candidate is noise."""
    ...
    def shadow_chunks_for_embedding(self, job: HostedJob) -> list[dict[str, Any]]:
        if job.mode != "shadow":
            raise ProtectedGatewayError("Semantic preparation requires a shadow job")
        rows: list[dict[str, Any]] = []
        page_size = 1000
        for offset in range(0, 25000, page_size):
            response = requests.get(
                f"{self.base_url}/rest/v1/ecos_hosted_shadow_chunks",
                headers=self.headers,
                params={
                    "job_id": f"eq.{job.job_id}",
                    "organization_id": f"eq.{job.organization_id}",
            ...
            if not isinstance(page, list):
                raise ProtectedGatewayError("Shadow chunk response was invalid")
            rows.extend(row for row in page if isinstance(row, dict))
            if len(page) < page_size:
                return rows
        raise ProtectedGatewayError("Shadow chunk inventory exceeded the semantic limit")

    def replace_shadow_chunk_embeddings(
        self,
        job: HostedJob,
        *,
        embedding_model: str,
    ...
    def ready_shadow_job_for_semantic_backfill(
        ...
        *,
        ...
        try:
            canonical_job_id = str(UUID(str(job_id).strip()))
        except (AttributeError, TypeError, ValueError) as error:
            raise ProtectedGatewayError("Semantic backfill job id is invalid") from error
        canonical_source_sha256 = str(source_sha256).strip().lower()
        if not re.fullmatch(r"[a-f0-9]{64}", canonical_source_sha256):
            raise ProtectedGatewayError("Semantic backfill source checksum is invalid")
        response = requests.get(
            f"{self.base_url}/rest/v1/ecos_hosted_index_jobs",
            headers=self.headers,
            params={
                "id": f"eq.{canonical_job_id}",
                "source_sha256": f"eq.{canonical_source_sha256}",
                "mode": "eq.shadow",
                "state": "in.(ready,assuring)",
                "committed_evidence_version": "eq.ecos-hosted-evidence/1.3",
                "select": (
                    "id,organization_id,project_id,document_id,source_provider,"
                    "source_locator,source_sha256,source_page_count,source_revision,"
                    "mode,retry_count"
                ),
                "limit": "2",
            },
        ...
        if not isinstance(records, list) or len(records) != 1 or not isinstance(records[0], dict):
            raise ProtectedGatewayError("Exact ready shadow job was not found")
        record = {**records[0], "job_id": records[0].get("id"), "claim_token": "semantic-backfill"}
        return HostedJob.from_record(record)

    def begin_ready_shadow_semantic_backfill(
        self,
        job: HostedJob,
        *,
        embedding_model: str,
        ...
        result = self.rpc("ecos_begin_ready_shadow_semantic_backfill", {
            "p_job_id": job.job_id,
            "p_expected_source_sha256": job.source_sha256,
            "p_expected_evidence_version": "ecos-hosted-evidence/1.3",
            "p_embedding_model": embedding_model,
            "p_embedding_dimensions": embedding_dimensions,
        })
        if not isinstance(result, dict):
            raise ProtectedGatewayError("Semantic backfill begin response was invalid")
        required = ("runId", "runToken", "jobId", "expectedChunkCount", "stagedChunkCount")
        if any(not result.get(key) and result.get(key) != 0 for key in required):
            raise ProtectedGatewayError("Semantic backfill begin response was incomplete")
        if str(result.get("jobId")) != job.job_id or str(result.get("state")) != "staging":
            raise ProtectedGatewayError("Semantic backfill begin identity did not match")
        try:
            expected_count = int(result["expectedChunkCount"])
            staged_count = int(result["stagedChunkCount"])
        except (TypeError, ValueError) as error:
            raise ProtectedGatewayError("Semantic backfill counts were invalid") from error
        if expected_count < 1 or staged_count < 0 or staged_count > expected_count:
            raise ProtectedGatewayError("Semantic backfill counts were outside the exact bounds")
        return {
            **result,
    ...
    def staged_semantic_backfill_identities(self, *, run_id: str) -> list[dict[str, Any]]:
        rows: list[dict[str, Any]] = []
        page_size = 1000
        for offset in range(0, 25000, page_size):
            response = requests.get(
                f"{self.base_url}/rest/v1/ecos_hosted_semantic_backfill_embeddings",
                headers=self.headers,
                params={
                    "run_id": f"eq.{run_id}",
                    "select": "page_number,region_id,chunk_index,chunk_sha256",
                    "order": "page_number.asc,region_id.asc,chunk_index.asc",
                    "limit": str(page_size),
                    "offset": str(offset),
                },
            ...
            page = response.json() if response.content else []
            if not isinstance(page, list):
                raise ProtectedGatewayError("Semantic backfill staging response was invalid")
            rows.extend(row for row in page if isinstance(row, dict))
            if len(page) < page_size:
                return rows
        raise ProtectedGatewayError("Semantic backfill staging inventory exceeded the limit")

    def stage_ready_shadow_semantic_backfill_batch(
        self,
        *,
        run_id: str,
        run_token: str,
        ...
        if not rows or len(rows) > 128:
            raise ProtectedGatewayError("Semantic backfill batch was outside the exact bounds")
        result = self.rpc("ecos_stage_ready_shadow_semantic_backfill_batch", {
            "p_run_id": run_id,
        ...
        try:
            ...
            raise ProtectedGatewayError("Semantic backfill staging response was invalid") from error

    def commit_ready_shadow_semantic_backfill(
        self,
        *,
        run_id: str,
        run_token: str,
    ) -> int:
        result = self.rpc("ecos_commit_ready_shadow_semantic_backfill", {
            "p_run_id": run_id,
            "p_run_token": run_token,
        })
        try:
            ...
            raise ProtectedGatewayError("Semantic backfill commit response was invalid") from error

    def reconcile_queued_shadow_semantic_page(
        self,
        *,
        job_id: str,
        source_sha256: str,
    ) -> dict[str, Any]:
        result = self.rpc("ecos_reconcile_queued_shadow_semantic_page", {
            "p_job_id": job_id,
            "p_expected_source_sha256": source_sha256,
        })
        if not isinstance(result, dict):
            raise ProtectedGatewayError("Shadow semantic reconciliation response was invalid")
        status = str(result.get("status") or "")
        try:
            remaining_count = int(result.get("remainingPageCount"))
        except (TypeError, ValueError) as error:
            raise ProtectedGatewayError(
                "Shadow semantic reconciliation count was invalid"
            ) from error
        if str(result.get("jobId")) != job_id or status not in ("reconciled", "complete"):
            raise ProtectedGatewayError("Shadow semantic reconciliation identity was invalid")
        if remaining_count < 0 or (status == "complete" and remaining_count != 0):
            raise ProtectedGatewayError("Shadow semantic reconciliation made invalid progress")
        return {**result, "remainingPageCount": remaining_count}

    def materialize_ready_shadow_semantic_backfill_batch(
        self,
        *,
        run_id: str,
        run_token: str,
        ...
        if batch_size < 1 or batch_size > 64:
            raise ProtectedGatewayError("Semantic promotion batch was outside the exact bounds")
        result = self.rpc("ecos_materialize_ready_shadow_semantic_backfill_batch", {
            "p_run_id": run_id,
            "p_run_token": run_token,
        ...
        try:
            ...
            raise ProtectedGatewayError("Semantic promotion response was invalid") from error

    def cancel_ready_shadow_semantic_backfill(
        self,
        *,
        run_id: str,
        run_token: str,
    ) -> bool:
        return self.rpc("ecos_cancel_ready_shadow_semantic_backfill", {
            "p_run_id": run_id,
            "p_run_token": run_token,
        }) is True
