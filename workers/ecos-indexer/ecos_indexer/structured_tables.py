...
def _normalize_regions(
    ...
    for index, item in enumerate(raw):
        ...
        if not text:
            continue
        if not _embedded_provenance_is_rendered(item):
            # Embedded strings and any synthetic region derived from them stay
            # outside table reconstruction unless their exact coordinates and
            # text were corroborated by rendered OCR.
            continue
        ...
        for key in (
            ...
            if item.get(key) is not None:
                normalized_region[key] = item[key]
...
def _anchor_candidate(
    ...
    if schema == "landscape_summary":
        return _landscape_anchor_candidate(identity, regions)
    if schema == "hydrozone_summary":
        return _hydrozone_anchor_candidate(identity, regions)
...
def _landscape_anchor_candidate(
    identity: Mapping[str, Any],
    regions: list[dict[str, Any]],
) -> dict[str, Any] | None:
    """Open only a compact landscape metrics table, never drawing prose.

    Words such as ``LANDSCAPE``, ``REQUIRED``, and ``PROVIDED`` occur in
    irrigation notes and water-budget worksheets.  They identify a topic, not
    the four-row landscape summary.  Automatic detection therefore requires
    at least two distinct coordinate rows that each bind an exact supported
    metric label to a numeric value, including at least one area row.
    """

    signed_rows = _landscape_signed_rows(regions)
    if len({item["role"] for item in signed_rows}) < 2:
        return None

    candidates: list[tuple[tuple[Any, ...], list[dict[str, Any]]]] = []
    for seed in signed_rows:
        seed_bounds = _envelope([
            region["bounds"] for region in seed["constituents"]
        ])
        local = [
            item for item in signed_rows
            if abs(
                _center_y(_envelope([
                    region["bounds"] for region in item["constituents"]
                ])) - _center_y(seed_bounds)
            ) <= 0.08
            and abs(
                _center_x(_envelope([
                    region["bounds"] for region in item["constituents"]
                ])) - _center_x(seed_bounds)
            ) <= 0.20
        ]
        roles = {item["role"] for item in local}
        if (
            len(roles) < 2
            or not roles.intersection({"landscapeArea", "irrigatedArea"})
        ):
            continue
        values_by_role: dict[str, set[Any]] = {}
        for item in local:
            values_by_role.setdefault(item["role"], set()).add(
                item["valueCandidate"].get("value")
            )
        # Multiple compact signed rows for one metric are not evidence that
        # either value is authoritative.  Reject this automatic candidate
        # instead of allowing input order to select an arbitrary last row.
        if any(len(values) > 1 for values in values_by_role.values()):
            continue
        exact = list({
            region["id"]: region
            for item in local
            for region in item["constituents"]
        }.values())
        envelope = _envelope([region["bounds"] for region in exact])
        candidates.append((
            (
                len(roles),
                len(roles.intersection({"landscapeArea", "irrigatedArea"})),
    ...
    if not candidates:
        return None
    _score, exact = max(candidates, key=lambda item: item[0])
    bounds = _pad_bounds(
        _envelope([region["bounds"] for region in exact]),
        x_pad=0.02,
        y_pad=0.015,
    )
    selected = [
        region for region in regions
        if _center_inside(region["bounds"], bounds)
    ]
    return _block(
        identity,
        "landscape_summary",
...
def _hydrozone_anchor_candidate(
    identity: Mapping[str, Any],
    regions: list[dict[str, Any]],
) -> dict[str, Any] | None:
    """Open only a coordinate-bound hydrozone table, never prose.

    Landscape narratives commonly contain all four lexical anchors
    ``HYDROZONE``, ``HIGH``, ``MEDIUM``, and ``LOW``.  Those words identify a
    topic, not a table.  Automatic detection therefore requires a hydrozone
    heading plus at least two distinct coordinate rows with a leading zone
    label and table-shaped evidence (an ``SF`` area, or a ``WATER ZONE`` label
    with a percentage cell that can safely request a missing-area retry).
    Trusted block hints remain geometry-only and are evaluated by the same row
    signature below.
    """

    headers = _primary_anchor_regions("hydrozone_summary", regions)
    signed_rows = _hydrozone_signed_rows(regions)
    if not headers or len({item["zone"] for item in signed_rows}) < 2:
        return None

    candidates = []
    for header in headers:
        window = _pad_bounds(header["bounds"], x_pad=0.30, y_pad=0.20)
        local_rows = [
            item
            for item in signed_rows
            if _center_inside(
                _envelope([region["bounds"] for region in item["row"]]),
                window,
            )
        ]
        zone_count = len({item["zone"] for item in local_rows})
        if zone_count < 2:
            continue
        matched = {
            region["id"]: region
            for item in local_rows
            for region in item["row"]
        }
        matched[header["id"]] = header
        envelope = _envelope([region["bounds"] for region in matched.values()])
        candidates.append((
            (
                zone_count,
                len(local_rows),
    ...
    if not candidates:
        return None
    _score, matched_regions = max(candidates, key=lambda item: item[0])
    bounds = _pad_bounds(
        _envelope([region["bounds"] for region in matched_regions.values()]),
        x_pad=0.025,
        y_pad=0.02,
    )
    selected = [
    ...
    *,
    ...
    payload = {
        ...
        "regionIds": sorted({
            ...
            for region_id in [
                region["id"],
                *(region.get("duplicateRegionIds") or []),
            ]
        }),
...
_LANDSCAPE_ROW_DEFINITIONS = (
    ...
    (
        "landscapeArea",
        r"(?<!IRRIGATED\s)\b(?:TOTAL\s+)?LANDSCAPE\s+AREA\b",
        r"(?<!IRRIGATED\s)\bLANDSCAPE\s+AREA\b",
        "sf",
    ),
    (
        "requiredTrees",
...
_LANDSCAPE_AUTO_TREE_SIGNATURES = {
    "requiredTrees": (
        r"\b(?:TREES?\s+REQUIRED|REQUIRED\s+(?:\d+\s+)?TREES?)\b",
        r"\b(?:TREES?\s+REQUIRED|REQUIRED\s+(?:\d+\s+)?TREES?)\b",
    ),
    "providedTrees": (
        r"\b(?:TREES?\s+PROVIDED|PROVIDED\s+(?:\d+\s+)?TREES?)\b",
        r"\b(?:TREES?\s+PROVIDED|PROVIDED\s+(?:\d+\s+)?TREES?)\b",
    ),
}


def _landscape_signed_rows(
    regions: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Return landscape metric rows with exact labels and numeric cells."""

    result = []
    for row in _line_clusters(regions):
        joined = _joined_text(row)
        for role, row_pattern, label_pattern, unit in _LANDSCAPE_ROW_DEFINITIONS:
            signed_row_pattern, signed_label_pattern = (
                _LANDSCAPE_AUTO_TREE_SIGNATURES.get(
                    role,
                    (row_pattern, label_pattern),
                )
            )
            if not re.search(signed_row_pattern, joined, re.I):
                continue
            metric_constituents = _matching_regions(row, signed_label_pattern)
            values = _suppress_overlapping_numeric_suffix_candidates(
                _integer_candidates(
                    row,
                    exclude={"LANDSCAPE", "IRRIGATED", "REQUIRED", "PROVIDED"},
                    unit=unit,
                )
            )
            if not metric_constituents or not values:
                continue
            metric_bounds = _envelope([
                region["bounds"] for region in metric_constituents
            ])
            row_tolerance = max(
                0.006,
                _median_height(metric_constituents) * 1.5,
            )
            row_values = [
                value for value in values
                if any(
                    abs(_center_y(constituent["bounds"]) - _center_y(metric_bounds))
                    <= row_tolerance
                    and -0.01 <= (
                        _center_x(constituent["bounds"])
                        - (metric_bounds["x"] + metric_bounds["width"])
                    ) <= 0.08
                    for constituent in value.get("constituents") or []
                )
            ]
            unique_values = {value.get("value") for value in row_values}
            ...
            value_constituents = [
                constituent
                for constituent in selected_value.get("constituents") or []
            ]
            result.append({
                "role": role,
                "row": row,
                "metricConstituents": metric_constituents,
                "valueCandidate": selected_value,
                "constituents": sorted(
                    {
                        region["id"]: region
                        for region in [*metric_constituents, *value_constituents]
                    }.values(),
...
def _evaluate_landscape(identity, block, regions):
    if block.get("detectionMethod") == "coordinate_anchor_fallback":
        signed_rows = _landscape_signed_rows(regions)
        values_by_role: dict[str, set[Any]] = {}
        for item in signed_rows:
            values_by_role.setdefault(item["role"], set()).add(
        ...
        if any(len(values) > 1 for values in values_by_role.values()):
            return []
        signed_by_role = {item["role"]: item for item in signed_rows}
        return [
            _relationship(
                identity,
                block,
                ...
                role,
                _envelope([
                    region["bounds"] for region in signed_by_role[role]["constituents"]
                ]),
                {
                    "metric": [_candidate(
                        role,
                        signed_by_role[role]["metricConstituents"],
                    )],
                    "value": [signed_by_role[role]["valueCandidate"]],
                },
                required=("metric", "value"),
            )
            for role, _row_pattern, _label_pattern, _unit in _LANDSCAPE_ROW_DEFINITIONS
            if role in signed_by_role
        ]
...
_HYDROZONE_ROW_LABELS = (
    (
        "high",
        re.compile(
            r"^\s*(?:(?:\(\s*H\s*\))(?:\s*HIGH)?|HIGH|H)(?=\s|$)",
            re.I,
        ),
    ),
    (
...
def _hydrozone_signed_rows(
    regions: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Return only rows carrying coordinate-bound hydrozone table evidence."""

    result = []
    for row in _line_clusters(regions):
        joined = _joined_text(row)
        area_candidates = _regex_role_candidates(
            row,
            r"(?<!\d)([\d,]+)\s*S\.?\s*F\b\.?",
            lambda value: int(value.replace(",", "")),
            unit="sf",
        )
        has_water_zone_label = bool(re.search(r"\bWATER\s+ZONE\b", joined, re.I))
        has_percentage_cell = bool(re.search(r"(?<!\d)\(?\d{1,3}\s*%\)?", joined))
        for zone, label_pattern in _HYDROZONE_ROW_LABELS:
            zone_constituents = []
            for region in row:
                region_text = _clean_spacing(
                    str(region.get("text") or "").replace("_", " ")
                )
                match = label_pattern.search(region_text)
                if match is not None:
                    zone_constituents.extend(
                        _phrase_regions([region], label_pattern, exact_match=match)
                    )
            zone_constituents = sorted(
                {region["id"]: region for region in zone_constituents}.values(),
                key=_region_sort_key,
            )
            if not zone_constituents:
                continue
            # A leading H/M/L label plus an exact SF cell is a complete table
            # row.  A WATER ZONE label plus a percentage is also sufficiently
            # table-shaped to retain an incomplete row and request bounded OCR
            # for a missing area.  Topic prose supplies neither signature.
            if not area_candidates and not (
                has_water_zone_label and has_percentage_cell
            ):
                continue
            result.append({
                "zone": zone,
                "row": row,
                "zoneConstituents": zone_constituents,
                "areaCandidates": area_candidates,
            })
            break
    return result
...
def _evaluate_hydrozones(identity, block, regions):
    ...
    for zone in ("high", "medium", "low"):
        matches = [item for item in signed_rows if item["zone"] == zone]
        if not matches:
            # A missing lexical anchor is not a relationship.  Emitting all
            # three expected row keys with no constituents made ordinary
            # narrative prose look like a conflicted table and violated the
        ...
        zone_constituents = sorted(
            {
                region["id"]: region
                for item in matches
                for region in item["zoneConstituents"]
            }.values(),
            key=_region_sort_key,
        )
        area_candidates = [
            candidate
            for item in matches
            for candidate in item["areaCandidates"]
        ]
        result.append(_relationship(
            identity, block, "hydrozone_area", zone,
            _envelope([region["bounds"] for region in row]),
            {
                "hydrozone": [_candidate(zone, zone_constituents)],
...
def _suppress_overlapping_numeric_suffix_candidates(
    candidates: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Drop a tighter OCR suffix only when it is the same printed cell.

    Overlapping OCR passes can read ``26,532`` once as the full value and once
    as the right-aligned suffix ``532``.  Value-only deduplication correctly
    treats those as different, so this spatial guard runs before relationship
    conflict evaluation.  It deliberately requires vertical and horizontal
    overlap plus a shared right edge; the same digits in another cell remain
    independent evidence and keep the relationship fail-closed.
    """

    suppressed: set[int] = set()
    for suffix_index, suffix in enumerate(candidates):
        suffix_value = suffix.get("value")
        if isinstance(suffix_value, bool) or not isinstance(suffix_value, int):
            continue
        suffix_digits = str(abs(suffix_value))
        suffix_constituents = suffix.get("constituents") or []
        if not suffix_constituents:
            continue
        suffix_bounds = _envelope([
            constituent["bounds"]
            for constituent in suffix_constituents
        ])
        for full_index, full in enumerate(candidates):
            if suffix_index == full_index or suffix.get("unit") != full.get("unit"):
                continue
            full_value = full.get("value")
            if isinstance(full_value, bool) or not isinstance(full_value, int):
                continue
            full_digits = str(abs(full_value))
            if (
                len(full_digits) <= len(suffix_digits)
                or not full_digits.endswith(suffix_digits)
            ):
                continue
            full_constituents = full.get("constituents") or []
            if not full_constituents:
                continue
            full_bounds = _envelope([
                constituent["bounds"]
                for constituent in full_constituents
            ])
            if _same_right_aligned_numeric_cell(suffix_bounds, full_bounds):
                suppressed.add(suffix_index)
                break
    return [
        candidate
...
def _same_right_aligned_numeric_cell(
    suffix: Mapping[str, float],
    full: Mapping[str, float],
) -> bool:
    suffix_right = suffix["x"] + suffix["width"]
    full_right = full["x"] + full["width"]
    suffix_bottom = suffix["y"] + suffix["height"]
    full_bottom = full["y"] + full["height"]
    if min(suffix["width"], full["width"], suffix["height"], full["height"]) <= 0:
        return False

    # Different OCR passes may move a duplicated word box by a fraction of a
    # glyph.  Permit only that small drift: the tighter suffix box must remain
    # nearly contained by the full-number box in both axes.  Mere overlap is
    # insufficient because adjacent cells on a ruled schedule can overlap
    # after OCR expansion.
    horizontal_slack = max(0.0005, full["width"] * 0.03)
    vertical_slack = max(0.0005, full["height"] * 0.25)
    nearly_contained = (
        suffix["x"] >= full["x"] - horizontal_slack
        and suffix_right <= full_right + horizontal_slack
        and suffix["y"] >= full["y"] - vertical_slack
        and suffix_bottom <= full_bottom + vertical_slack
    )
    tight_right_edge_tolerance = max(
        0.0005,
        full["width"] * 0.04,
        full["height"] * 0.25,
    )
    return (
        nearly_contained
        and abs(suffix_right - full_right) <= tight_right_edge_tolerance
        and _center_x(suffix) >= _center_x(full)
    )
...
def _infer_schema(regions: list[dict[str, Any]]) -> str | None:
    ...
    for schema, patterns in _SCHEMA_ANCHORS.items():
        ...
        if (
            ...
            and len({item["zone"] for item in _hydrozone_signed_rows(regions)}) < 2
        ):
            continue
        if (
            schema == "landscape_summary"
            and len({item["role"] for item in _landscape_signed_rows(regions)}) < 2
        ):
            continue
...
def _embedded_provenance_is_rendered(
    ...
    *,
    ...
    if source in EMBEDDED_PDF_TEXT_SOURCES:
        region_ids = value.get("renderedCorroboratingRegionIds")
    ...
    for field in ("constituentEvidence", "corroboratingEvidence"):
        ...
        if any(
            ...
            or not _embedded_provenance_is_rendered(item, depth=depth + 1)
            for item in nested
        ):
            return False
    return True
