...
def document_sheet_identity_map(document: Any) -> dict[int, StructuralSheetIdentity]:
    ...
    source = pdf_decode_source(document)
    if source is not None and not resource_limits._ISOLATED_CHILD:
        payload = run_isolated_pdf_operation(
            source,
            "structure",
            timeout_seconds=resource_limits.STRUCTURAL_SCAN_TIMEOUT_SECONDS,
            timeout_code="pdf_structural_scan_timeout",
        )
        return structural_sheet_identity_map_from_payload(payload)

    validate_document_object_budget(document)
    with resource_deadline(
        resource_limits.STRUCTURAL_SCAN_TIMEOUT_SECONDS,
        "pdf_structural_scan_timeout",
    ):
        return _document_sheet_identity_map_direct(document)


def _document_sheet_identity_map_direct(
    document: Any,
) -> dict[int, StructuralSheetIdentity]:
    """Perform the scan inside the resource-limited decoder child."""
...
def structural_sheet_identity_map_from_payload(
    payload: Any,
) -> dict[int, StructuralSheetIdentity]:
    if not isinstance(payload, dict):
        raise DocumentResourceRejected("pdf_structural_scan_payload_invalid")
    result: dict[int, StructuralSheetIdentity] = {}
    for raw_page_number, raw_identity in payload.items():
        if not isinstance(raw_identity, dict):
            raise DocumentResourceRejected("pdf_structural_scan_payload_invalid")
        try:
            page_number = int(raw_page_number)
            sheet_number = str(raw_identity["sheetNumber"])
            source = str(raw_identity["source"])
            raw_evidence = raw_identity["evidence"]
        except (KeyError, TypeError, ValueError) as error:
            raise DocumentResourceRejected("pdf_structural_scan_payload_invalid") from error
        if page_number < 1 or not sheet_number or not source or not isinstance(raw_evidence, list):
            raise DocumentResourceRejected("pdf_structural_scan_payload_invalid")
        evidence: list[StructuralIdentityEvidence] = []
        for item in raw_evidence:
            if not isinstance(item, dict):
                raise DocumentResourceRejected("pdf_structural_scan_payload_invalid")
            bounds = item.get("normalizedBounds")
            ...
            if bounds is not None:
                try:
                    ...
                    raise DocumentResourceRejected(
                        "pdf_structural_scan_payload_invalid",
                    ) from error
            try:
                evidence.append(StructuralIdentityEvidence(
                    ...
                    annotation_subtype=(
                        str(item["annotationSubtype"])
                        if item.get("annotationSubtype") is not None
                        else None
                    ),
                ))
            except (KeyError, TypeError, ValueError) as error:
...
def bookmark_sheet_identity_map(document: Any) -> dict[int, StructuralSheetIdentity]:
    ...
    if not isinstance(table_of_contents, list):
        raise DocumentResourceRejected("pdf_structural_toc_invalid")
    if len(table_of_contents) > resource_limits.MAX_STRUCTURAL_TOC_ENTRIES:
        raise DocumentResourceRejected("pdf_structural_toc_count_outside_limit")
...
def native_canopy_sheet_identity_map(document: Any) -> dict[int, StructuralSheetIdentity]:
    ...
    for page_number in range(1, page_count + 1):
        ...
        except DocumentResourceRejected:
            raise
        except (AttributeError, RuntimeError, TypeError, ValueError):
            return {}
        if len(matches) != 1:
...
def pdf_annotation_title_band_identities_for_page(
    ...
    while annotation is not None:
        if annotation_index >= resource_limits.MAX_STRUCTURAL_ANNOTATIONS_PER_PAGE:
            raise DocumentResourceRejected(
                "pdf_structural_annotation_count_outside_limit",
            )
        try:
