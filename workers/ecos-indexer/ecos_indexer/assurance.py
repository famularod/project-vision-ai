...
EXACT_STRUCTURAL_2321_PAGE2_PROJECT_ID = "607c7eed-5dea-4a5a-8b52-0f165c71c4b5"
...
EXACT_STRUCTURAL_2321_PAGE2_REQUIRED_FACTS = frozenset({
    "6-inch-thick reinforced concrete slab",
    "Footing F1: 5'-0\" x 5'-0\" x 18\"; reinforcing 6-#5 each way, top and bottom",
    "Footing F2: 4'-0\" x 4'-0\" x 18\"; reinforcing 5-#5 each way, top and bottom",
    "Footing F3: 3'-0\" x 3'-0\" x 18\"; reinforcing 4-#5 each way, top and bottom",
})


def exact_structural_2321_page2_required_facts_complete(
    *,
    ...
    """Fail the exact SB-1.1 page closed unless every required fact exists."""

    exact_target = (
        str(expected_project_id or "").strip()
        == EXACT_STRUCTURAL_2321_PAGE2_PROJECT_ID
        and expected_page_number == EXACT_STRUCTURAL_2321_PAGE2_NUMBER
        and str(expected_source_sha256 or "").strip().lower()
        == EXACT_STRUCTURAL_2321_PAGE2_SOURCE_SHA256
        and str(expected_evidence_version or "").strip()
        == EXACT_STRUCTURAL_2321_PAGE2_EVIDENCE_VERSION
    )
    if not exact_target:
        return True
    facts = {
        str(region.get("text") or region.get("label") or "").strip()
        for region in regions
        if isinstance(region, dict) and region.get("factKind") == "drawing_fact"
    }
    return EXACT_STRUCTURAL_2321_PAGE2_REQUIRED_FACTS.issubset(facts)
...
def assure_page(
    *,
    ...
    exact_required_facts_complete = exact_structural_2321_page2_required_facts_complete(
        regions=regions,
    ...
    if not exact_required_facts_complete:
        failures.append("exact_2321_structural_page2_required_facts_incomplete")
    ...
    return {
        ...
        "checks": {
            ...
            "exactRequiredFactsComplete": exact_required_facts_complete,
