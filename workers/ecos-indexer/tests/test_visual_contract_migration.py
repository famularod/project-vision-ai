...
DISMISSAL_MIGRATION = REPOSITORY_ROOT / "supabase" / "migrations" / (
...
class VisualContractMigrationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.dismissal = DISMISSAL_MIGRATION.read_text(encoding="utf-8")
    ...
    def test_edge_provider_receives_bounded_candidate_contract(self) -> None:
        ...
        self.assertIn("acceptedCandidateIndexes", self.edge)
        self.assertIn("complete server-supplied candidate phrase", self.edge)
        self.assertIn("candidate-index acceptance is the authoritative declaration", self.edge)

    def test_edge_requires_primary_and_independent_complete_dismissal(self) -> None:
        for required in (
            ...
            "jointlyDismissedCandidateIndexes",
            "normalizeCandidateIndexDisposition",
            "Leave an uncertain, partial, clipped, or potentially material candidate out of both arrays",
        ):
            self.assertIn(required, self.edge)
        candidate_schema = self.edge.split("function candidateIndexesSchema(", 1)[1].split("\n}", 1)[0]
        disposition_normalizer = self.edge.split(
            "function normalizeCandidateIndexDisposition(", 1,
        )[1].split("\n}", 1)[0]
        self.assertNotIn("uniqueItems", candidate_schema)
        self.assertIn("new Set(indexes).size === indexes.length", disposition_normalizer)
        self.assertNotIn("analysis.facts.length === 0 &&", self.edge)

    def test_edge_describes_exception_images_as_review_crop_and_review_tiles(self) -> None:
        self.assertIn("Image 1 is a low-resolution review crop", self.edge)
        self.assertIn("Every later image is a high-resolution review tile", self.edge)
        self.assertIn("the first high-resolution review tile", self.edge)

    def test_dismissal_rpc_is_claim_bound_service_only_and_fact_free(self) -> None:
        for required in (
            ...
            "p_normalized_dismissal->>'assuranceProvider'",
            "bounded_visual_exception_dismissal_v1",
            "from public, anon, authenticated",
            "to service_role",
        ):
            self.assertIn(required, self.dismissal)
