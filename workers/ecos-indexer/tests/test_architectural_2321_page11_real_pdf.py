"""Opt-in exact-SHA regression for Architectural 2321 page 11."""
...
PAGE_NUMBER = 11
RUN_EXACT = os.getenv("ECOS_RUN_ARCHITECTURAL_2321_PAGE11_PDF_TESTS") == "1"


def site_exceptions(result: dict[str, object]) -> list[dict[str, object]]:
    matches = [
        item
...
@unittest.skipUnless(
    ...
    "Set ECOS_RUN_ARCHITECTURAL_2321_PAGE11_PDF_TESTS=1 with the exact source.",
)
class ExactArchitectural2321Page11ProductionTests(unittest.TestCase):
    @classmethod
    ...
    def test_complete_notes_replace_only_fragment_authority(self) -> None:
        expected = {
            str(spec["text"]): spec
            for spec in EXACT_ARCHITECTURAL_2321_SITE_NOTE_CANDIDATES
        }
        exceptions = site_exceptions(self.result)
        ...
        self.assertGreaterEqual(len(superseded), 9)
        for region in superseded:
            raw = raw_by_id.get(str(region["id"]))
            if raw is None:
                self.assertTrue(str(region["id"]).endswith(
                    "-two-resolution-measurement"
                ))
                continue
        ...
        for text in expected:
            self.assertNotIn(text, searchable_texts)
            self.assertNotIn(text, self.result["final"]["text"])

        target_bounds = {
            (
                float(constituent["bounds"]["x"]),
                float(constituent["bounds"]["y"]),
                float(constituent["bounds"]["width"]),
                float(constituent["bounds"]["height"]),
            )
            for spec in EXACT_ARCHITECTURAL_2321_SITE_NOTE_CANDIDATES
            for constituent in spec["constituents"]
            if constituent.get("role") == "measurement"
        }
    ...
    def test_checkpoint_replay_preserves_notes_and_fingerprints(self) -> None:
        ...
        try:
            ...
            document.close()
        original = site_exceptions(self.result)
        replay = site_exceptions(replayed)
    ...
    def test_dual_provider_exact_notes_are_required_before_publication(self) -> None:
        appended: list[dict[str, object]] = []
        for exception in site_exceptions(self.result):
