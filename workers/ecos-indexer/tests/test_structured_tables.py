...
class StructuredTableContractTests(unittest.TestCase):
    ...
    def test_slab_relationship_is_complete_and_needs_no_ocr_when_native_boxes_suffice(self):
        regions = [
            word(
                region_id, text, x, 0.10, source="embedded_text",
                renderedCorroborated=True,
                renderedCorroboratingRegionIds=[f"render-{region_id}"],
                renderedCorroboratingSources=["fixed_visual_tile_coordinate_ocr"],
            )
            for region_id, text, x in (
                ("thickness", '6" THK.', 0.10),
                ("reinforced", "REINFORCED", 0.18),
                ("concrete", "CONCRETE", 0.29),
                ("slab", "SLAB", 0.39),
                ("add", "ADD.", 0.47),
            )
        ]
    ...
    def test_uncorroborated_embedded_slab_text_cannot_complete_relationship(self):
        regions = [
        ...
        self.assertFalse(any(
            relationship.get("status") == "complete"
            for relationship in result["relationships"]
        ))
    ...
    def test_coordinate_landscape_metric_rows_auto_detect_as_one_compact_block(self):
        regions = [
            word("landscape-label", "TOTAL LANDSCAPE AREA", 0.66, 0.80, width=0.15),
            word("landscape-value", "26,532 SF", 0.82, 0.80, width=0.08),
            word("required-label", "TREES REQUIRED", 0.66, 0.82, width=0.13),
            word("required-value", "71", 0.82, 0.82, width=0.03),
            word("provided-label", "TREES PROVIDED", 0.66, 0.84, width=0.13),
            word("provided-value", "78", 0.82, 0.84, width=0.03),
            word("irrigated-label", "TOTAL IRRIGATED LANDSCAPE AREA", 0.66, 0.86, width=0.22),
            word("irrigated-value", "21,104 SF", 0.89, 0.86, width=0.08),
        ]
        identity = page_identity(sheet="L-1")

        blocks = detect_blocks(identity, regions)
        landscape_blocks = [
            block for block in blocks if block["schema"] == "landscape_summary"
        ]
        self.assertEqual(len(landscape_blocks), 1)
        self.assertLess(landscape_blocks[0]["bounds"]["height"], 0.18)
        rows = relationships_by_key(evaluate_blocks(identity, blocks, regions))
        self.assertEqual(
            {key: rows[key]["roles"]["value"]["value"] for key in (
    ...
    def test_landscape_topic_and_numbered_irrigation_prose_do_not_open_a_table(self):
        regions = [
            word("worksheet", "Efficient Landscape Worksheet", 0.70, 0.42, width=0.20),
            word("regular", "Regular Landscape Areas", 0.69, 0.45, width=0.18),
            word("irrigated", "IRRIGATED systems shall be verified", 0.55, 0.53, width=0.28),
            word("provided", "PROVIDED 12 controller outputs", 0.55, 0.70, width=0.25),
            word("required", "REQUIRED 10 flexible risers", 0.55, 0.81, width=0.23),
        ]

        blocks = detect_blocks(page_identity(sheet="L-2"), regions)

        self.assertNotIn("landscape_summary", {block["schema"] for block in blocks})

    def test_one_landscape_area_row_alone_does_not_open_a_summary(self):
        regions = [
            word("landscape-label", "TOTAL LANDSCAPE AREA", 0.66, 0.80, width=0.15),
            word("landscape-value", "26,532 SF", 0.82, 0.80, width=0.08),
            word("provided-prose", "PROVIDED by contractor", 0.54, 0.60, width=0.20),
            word("required-prose", "REQUIRED for approval", 0.54, 0.70, width=0.20),
        ]
    ...
    def test_two_auto_landscape_rows_emit_no_absent_role_phantoms(self):
        regions = [
            word("landscape-label", "TOTAL LANDSCAPE AREA", 0.66, 0.80, width=0.15),
            word("landscape-value", "26,532 SF", 0.82, 0.80, width=0.08),
            word("required-label", "TREES REQUIRED", 0.66, 0.83, width=0.13),
            word("required-value", "71", 0.82, 0.83, width=0.03),
        ]
        identity = page_identity(sheet="L-1")

        blocks = detect_blocks(identity, regions)
        result = evaluate_blocks(identity, blocks, regions)

        self.assertEqual(
            {(row["rowKey"], row["status"]) for row in result["relationships"]},
            {("landscapeArea", "complete"), ("requiredTrees", "complete")},
        )
        self.assertEqual(result["targetedOcrRequests"], [])
        self.assertEqual(result["status"], "complete")

    def test_conflicting_duplicate_landscape_role_rejects_auto_block(self):
        regions = [
            word("landscape-label-a", "TOTAL LANDSCAPE AREA", 0.66, 0.80, width=0.15),
            word("landscape-value-a", "26,532 SF", 0.82, 0.80, width=0.08),
            word("required-label", "TREES REQUIRED", 0.66, 0.82, width=0.13),
            word("required-value", "71", 0.82, 0.82, width=0.03),
            word("landscape-label-b", "TOTAL LANDSCAPE AREA", 0.66, 0.84, width=0.15),
            word("landscape-value-b", "99,999 SF", 0.82, 0.84, width=0.08),
        ]

        blocks = detect_blocks(page_identity(sheet="L-1"), regions)

        self.assertNotIn("landscape_summary", {block["schema"] for block in blocks})

    def test_conflicting_duplicate_landscape_role_rejects_auto_block_when_shuffled(self):
        regions = [
            word("landscape-value-b", "99,999 SF", 0.82, 0.84, width=0.08),
            word("required-label", "TREES REQUIRED", 0.66, 0.82, width=0.13),
            word("landscape-label-a", "TOTAL LANDSCAPE AREA", 0.66, 0.80, width=0.15),
    ...
    def test_landscape_summary_suppresses_right_aligned_overlapping_numeric_suffix_ocr(self):
        regions = [
            word("landscape-label", "LANDSCAPE AREA", 0.10, 0.10, width=0.16),
            word("landscape-value", "26,532 SF", 0.35, 0.10, width=0.10),
            # A tighter OCR tile redetected only the right-aligned suffix of
            # the same printed cell.  It must corroborate, not conflict with,
            # the complete number read from the wider tile.
            word("landscape-suffix", "532", 0.405, 0.101, width=0.044),
            word("irrigated-label", "IRRIGATED AREA", 0.10, 0.20, width=0.16),
            word("irrigated-value", "21,104 SF", 0.35, 0.20, width=0.10),
            word("irrigated-suffix", "104", 0.405, 0.201, width=0.044),
        ]

        rows = relationships_by_key(evaluate("landscape_summary", regions))

        self.assertEqual(rows["landscapeArea"]["status"], "complete")
        self.assertEqual(rows["landscapeArea"]["roles"]["value"]["value"], 26532)
        self.assertEqual(rows["irrigatedArea"]["status"], "complete")
        self.assertEqual(rows["irrigatedArea"]["roles"]["value"]["value"], 21104)

    def test_landscape_summary_preserves_numeric_suffix_in_a_separate_cell_as_conflict(self):
        regions = [
            word("landscape-label", "LANDSCAPE AREA", 0.10, 0.10, width=0.16),
            word("landscape-value", "26,532 SF", 0.35, 0.10, width=0.10),
            # The same digits in a non-overlapping cell are independent
            # evidence and must remain visible to the fail-closed conflict.
            word("separate-value", "532", 0.58, 0.10, width=0.044),
        ]

        rows = relationships_by_key(evaluate("landscape_summary", regions))

        self.assertEqual(rows["landscapeArea"]["status"], "conflicted")
        self.assertEqual(rows["landscapeArea"]["conflictCodes"], ["ambiguous_role:value"])
        self.assertEqual(
            {candidate["value"] for candidate in rows["landscapeArea"]["roles"]["value"]["candidates"]},
            {532, 26532},
        )

    def test_landscape_summary_preserves_partially_overlapping_adjacent_cell_as_conflict(self):
        regions = [
            word("landscape-label", "LANDSCAPE AREA", 0.10, 0.10, width=0.16),
            word("landscape-value", "26,532 SF", 0.35, 0.10, width=0.10),
            # OCR expansion makes this adjacent cell overlap 60% of its width
            # with the full-number box and places its right edge nearby.  It
            # is not contained by that box, so it must remain independent.
            word("adjacent-value", "532", 0.42, 0.10, width=0.05),
        ]
    ...
    def test_narrative_hydrozone_words_do_not_open_a_table_or_create_phantom_rows(self):
        regions = [
            word(
                "statement",
            ...
            word(
                "narrative",
                "HYDROZONE design may use HIGH, MEDIUM, or LOW water demand plants.",
                0.10,
                0.14,
                width=0.60,
            ),
        ...
        blocks = detect_blocks(identity, regions)
        hinted = evaluate("hydrozone_summary", regions, identity=identity)

        self.assertNotIn("hydrozone_summary", {block["schema"] for block in blocks})
        self.assertEqual(hinted["relationships"], [])
        self.assertEqual(hinted["targetedOcrRequests"], [])

    def test_coordinate_hydrozone_rows_auto_detect_and_remain_complete(self):
        regions = [
            word("header", "HYDROZONE SUMMARY", 0.10, 0.05, width=0.18),
            word("h", "(H) HIGH WATER ZONE", 0.10, 0.10, width=0.20),
            word("h-percent", "(0%)", 0.32, 0.10),
            word("h-area", "0 SF", 0.42, 0.10),
            word("m", "(M) MEDIUM WATER ZONE", 0.10, 0.16, width=0.23),
            word("m-percent", "(2%)", 0.35, 0.16),
            word("m-area", "146 SF", 0.45, 0.16),
            word("l", "(L) LOW WATER ZONE", 0.10, 0.22, width=0.20),
            word("l-percent", "(98%)", 0.32, 0.22),
            word("l-area", "9,470 SF", 0.42, 0.22),
        ]
        ...
        self.assertEqual(
            [block["schema"] for block in blocks],
            ["hydrozone_summary"],
        )
        self.assertEqual(rows["high"]["status"], "complete")
    ...
    def test_hydrozone_labels_bind_from_coordinate_regions_after_unrelated_left_cells(self):
        regions = [
            word("header", "HYDROZONE DATA", 0.40, 0.05, width=0.18),
            word("h-prefix", "CLOCK A", 0.02, 0.10, width=0.10),
            word("h", "(H) HIGH WATER ZONE", 0.40, 0.10, width=0.20),
            word("h-percent", "(0%)", 0.62, 0.10),
            word("h-area", "0 SF", 0.72, 0.10),
            word("m-prefix", "VALVE 1", 0.02, 0.16, width=0.10),
            word("m", "(M) MEDIUM WATER ZONE", 0.40, 0.16, width=0.23),
            word("m-percent", "(2%)", 0.65, 0.16),
            word("m-area", "146 SF", 0.75, 0.16),
            word("l-prefix", "VALVE 2", 0.02, 0.22, width=0.10),
            word("l", "(L) LOW WATER ZONE", 0.40, 0.22, width=0.20),
            word("l-percent", "(98%)", 0.62, 0.22),
            word("l-area", "9,470 SF", 0.72, 0.22),
        ]
        identity = page_identity(sheet="L-2")
        ...
        self.assertEqual(
            {zone: rows[zone]["roles"]["area"]["value"] for zone in (
                "high", "medium", "low",
            )},
            {"high": 0, "medium": 146, "low": 9470},
        )
        self.assertTrue(all(rows[zone]["status"] == "complete" for zone in rows))

    def test_structured_hydrozone_row_with_missing_area_stays_incomplete_and_bounded(self):
        regions = [
            word("h", "(H) HIGH WATER ZONE", 0.10, 0.10, width=0.20),
            word("h-percent", "(0%)", 0.32, 0.10),
        ]

        result = evaluate("hydrozone_summary", regions)
        rows = relationships_by_key(result)

        self.assertEqual(set(rows), {"high"})
        self.assertEqual(rows["high"]["status"], "incomplete")
        self.assertEqual(rows["high"]["missingRoles"], ["area"])
        self.assertTrue(rows["high"]["constituents"])
        self.assertEqual(len(result["targetedOcrRequests"]), 1)
        self.assertEqual(
            result["targetedOcrRequests"][0]["relationshipId"],
            rows["high"]["id"],
        )
