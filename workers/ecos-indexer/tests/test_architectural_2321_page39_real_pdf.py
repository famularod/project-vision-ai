"""Opt-in exact-SHA regression for Architectural 2321 page 39."""
...
PAGE_NUMBER = 39
RUN_EXACT = os.getenv("ECOS_RUN_ARCHITECTURAL_2321_PAGE39_PDF_TESTS") == "1"
SUPERSEDED_STATUS = (
...
@unittest.skipUnless(
    ...
    "Set ECOS_RUN_ARCHITECTURAL_2321_PAGE39_PDF_TESTS=1 with the exact source.",
)
class ExactArchitectural2321Page39ProductionTests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_fragments_become_two_untrusted_complete_candidates(self) -> None:
        ...
        self.assertEqual({"6'-\"", "4'-O\""}, {region["text"] for region in superseded})
        ...
        target_fragment_bounds = {
            tuple(spec["authority"]["bounds"][key] for key in (
                "x", "y", "width", "height",
            ))
            for spec in EXACT_ARCHITECTURAL_2321_PAGE39_PROPOSITION_SPECS
        }
        self.assertFalse(any(
            candidate["text"] in {"6'-\"", "6'-O\"", "4'-O\""}
            and tuple(candidate["bounds"][key] for key in (
                "x", "y", "width", "height",
            )) in target_fragment_bounds
    ...
    def test_dual_provider_exact_facts_are_required_before_assurance(self) -> None:
        ...
        for exception in exact:
            ...
            partial["facts"][0]["statement"] = "4'-0\""
            partial["facts"][0]["subject"] = "4'-0\""
            partial["facts"][0]["evidenceText"] = "4'-0\""
        ...
        self.assertEqual(canonical_texts, [region["text"] for region in accepted])
