...
class DocumentStructureTests(unittest.TestCase):
        ...
        try:
            ...
            for page_number, identity in structural_map.items():
                with self.subTest(page_number=page_number):
                    ...
                    self.assertEqual(mapping["sheetNumber"], identity.sheet_number)
                    self.assertEqual(mapping["sheetMappingSource"], "pdf_bookmark")
        finally:
            document.close()
    ...
    def test_exact_2375_civil_pdf_maps_all_annotation_title_cells(self) -> None:
        ...
        try:
            ...
            for page_number, identity in identity_map.items():
                with self.subTest(page_number=page_number):
                    ...
                    self.assertEqual(mapping["sheetMappingStatus"], "verified")
                    self.assertEqual(mapping["sheetNumber"], f"C{page_number}")
                    self.assertEqual(
                        mapping["sheetMappingSource"],
    ...
    def test_exact_native_landscape_series_preserves_page_bound_identities(self) -> None:
        ...
        self.assertEqual(mapping["sheetMappingSource"], "native_title_band")
        self.assertNotEqual(mapping["sheetMappingSource"], "pdf_bookmark")
        self.assertEqual(mapping["sheetMappingEvidence"], [evidence.as_dict()])
