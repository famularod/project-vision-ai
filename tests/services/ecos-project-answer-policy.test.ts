...
describe('ECOS project answer assurance policy', () => {
  ...
  it('treats compound compass directions atomically in compact, hyphenated, and spaced forms', () => {
    const cases = [
      {
        question: 'What thickness is specified for PCC paving at the east lot?',
        requested: 'east',
        exact: 'EAST LOT — CONSTRUCT 6.0 INCH THICK PCC PAVING',
        compounds: [
          'NORTHEAST LOT',
          'NORTH-EAST LOT',
          'NORTH EAST LOT',
      ...
      {
        question: 'What thickness is specified for PCC paving at the west lot?',
        requested: 'west',
        exact: 'WEST LOT — CONSTRUCT 6.0 INCH THICK PCC PAVING',
        compounds: [
          'SOUTHWEST LOT',
          'SOUTH-WEST LOT',
          'SOUTH WEST LOT',
    ...
    for (const testCase of cases) {
      expect(analyzeECOSQuestionEvidenceContext(testCase.question, testCase.exact)).toMatchObject({
        matchedLocationDirectionTokens: [testCase.requested],
        locationMatched: true,
      });
      expect(ecosFactAnswersQuestion({
        question: testCase.question,
        statement: 'The drawing specifies 6.0-inch-thick PCC paving.',
        sourceExcerpts: [testCase.exact],
      })).toBe(true);
      for (const compound of testCase.compounds) {
        const evidence = `${compound} — CONSTRUCT 8.0 INCH THICK PCC PAVING`;
        expect(analyzeECOSQuestionEvidenceContext(testCase.question, evidence)).toMatchObject({
          matchedLocationDirectionTokens: [],
          locationMatched: false,
        });
        expect(ecosFactAnswersQuestion({
          question: testCase.question,
          statement: 'The drawing specifies 8.0-inch-thick PCC paving.',
          sourceExcerpts: [evidence],
        })).toBe(false);
      }
    }

    for (const compoundQuestion of ['north east', 'north-east', 'north/east', 'north.east', 'northeast']) {
      expect(analyzeECOSQuestionEvidenceContext(
        `What thickness is specified for PCC paving at the ${compoundQuestion} lot?`,
        'NORTHEAST LOT — CONSTRUCT 8.0 INCH THICK PCC PAVING',
      )).toMatchObject({
        matchedLocationDirectionTokens: ['northeast'],
        locationMatched: true,
      });
    }
  });
