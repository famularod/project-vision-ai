...
describe('reference document cloud recovery', () => {
  it('does not replay current cloud documents because of device URIs or ECOS authority', () => {
    ...
    expect(daveReferenceDocumentsNeedingCloudUpload({
      local: [local],
      cloud: [cloud, document({ id: 'cloud-only' })],
    })).toEqual([]);
  });

  it('uploads local-only documents and genuinely newer user metadata', () => {
    const cloud = document({
      isCurrent: true,
      storagePath: 'owner/schedules/schedule.pdf',
      cloudUpdatedAt: '2026-07-20T13:00:00.000Z',
    ...
    const renamed = document({
      name: 'Schedule - field corrected',
      uri: 'file:///device/schedule.pdf',
      updatedAt: '2026-07-20T14:00:00.000Z',
    });
    const localOnly = document({ id: 'local-only', name: 'Local only' });

    expect(daveReferenceDocumentsNeedingCloudUpload({
      local: [renamed, localOnly],
      cloud: [cloud],
    })).toEqual([
  ...
  it('keeps a local file eligible until protected storage exists', () => {
    const local = document({ uri: 'file:///device/schedule.pdf' });
    const cloud = document({ cloudUpdatedAt: '2026-07-20T13:00:00.000Z' });
