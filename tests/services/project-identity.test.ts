...
describe('restoreProjectRecords (audit P1-41)', () => {
  it('uses the portable immutable records from the backup itself', () => {
    const restored = restoreProjectRecords([
      { id: PROJECT_A_ID, name: 'Kept Site', coverPhoto: 'cover.jpg' },
      { id: PROJECT_B_ID, name: 'New Site' },
    ]);
  ...
  it('rejects legacy name-only records instead of borrowing target-device identity', () => {
    expect(() => restoreProjectRecords([{ name: 'Legacy Site' }])).toThrow(
      /immutable project UUID/i,
    );
  });

  it('rejects duplicate display names and immutable IDs', () => {
    expect(() => restoreProjectRecords([
      { id: PROJECT_A_ID, name: 'Shared Site' },
      { id: PROJECT_B_ID, name: ' shared  site ' },
    ])).toThrow(/duplicate/i);
    expect(() => restoreProjectRecords([
      { id: PROJECT_A_ID, name: 'Site A' },
      { id: PROJECT_A_ID, name: 'Site B' },
    ])).toThrow(/duplicate/i);
  });
});
