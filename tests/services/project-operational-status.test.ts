...
describe('deriveDAVEProjectOperationalStatus', () => {
  it('binds operational task scope to the exact project id when names collide', () => {
    const shared = {
    ...
    const items: ScheduleItem[] = [
      { ...shared, id: 'task-a', projectId: 'project-a' },
      { ...shared, id: 'task-b', projectId: 'project-b' },
      { ...shared, id: 'task-legacy' },
    ];

    expect(operationalScheduleItemsForProject(
      'Shared Project',
      items,
      'project-b',
    ).map(item => item.id)).toEqual(['task-b']);
    expect(operationalScheduleItemsForProject(
      'Shared Project',
      items,
      ' project-b ',
