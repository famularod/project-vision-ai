...
describe('combined report authority scope', () => {
  ...
  it('scopes every supporting evidence category to the selected parent', () => {
    ...
    const memory = (id: string, projectId: string, projectName: string) => ({
      id,
      projectId,
      status: 'confirmed',
    ...
    const scope = buildDailyReportAuthorityScope({
      ...
      projectDocuments,
      captureMemories: [
        memory('memory-a', 'project-record-a', PROJECT_A),
        memory('memory-b', 'project-record-b', PROJECT_B),
      ],
      contacts: {
        contacts: [
    ...
    expect(scope.captureMemoryProjectIds).toEqual(['project-record-a']);
    expect(scope.contacts.contacts.map(contact => contact.id)).toEqual(['contact-a']);
  });

  it('does not treat one exact record plus a same-name legacy row as unambiguous memory authority', () => {
    const memoryA = {
      id: 'memory-a',
      projectId: 'project-record-a',
      status: 'confirmed',
      confirmedAt: '2026-07-18T12:00:00.000Z',
      cancelledAt: null,
      recommendedProject: { value: PROJECT_A },
    } as unknown as DAVEConfirmedCaptureMemory;
    const scope = buildDailyReportAuthorityScope({
      selectedProjectName: PROJECT_A,
      selectedProjectNames: [PROJECT_A],
    ...
    expect(scope.captureMemories).toEqual([]);
    expect(scope.captureMemoryProjectIds).toEqual([]);
  });
