...
const cachedReadyDocument: ReferenceDocument = {
  id: 'drawing-1',
  name: 'A101',
...
describe('ECOS hosted indexer client boundary', () => {
  ...
  it('treats an authorized empty result as authority to clear cached Ready state', async () => {
    const client = {
      rpc: jest.fn().mockResolvedValue({ data: [], error: null }),
    } as unknown as SupabaseClient;

    const snapshot = await loadECOSHostedIndexStatusSnapshot({
      client,
      documentIds: [cachedReadyDocument.id],
    });
    const reconciled = reconcileECOSHostedIndexStatus(cachedReadyDocument, null, snapshot.authority);

    expect(snapshot).toEqual({ authority: 'available', statuses: [] });
    expect(reconciled).toMatchObject({
      ecosHostedIndexStatus: null,
      ecosHostedIndexProgressPercent: null,
      ecosHostedIndexEvidenceVersion: null,
    });
    expect(buildECOSDocumentReadiness(reconciled).eligibleForAnswers).toBe(false);
  });

  it('marks status authority unavailable without reusing cached Ready proof', async () => {
    const client = {
    ...
    const snapshot = await loadECOSHostedIndexStatusSnapshot({ client });
    const reconciled = reconcileECOSHostedIndexStatus(cachedReadyDocument, null, snapshot.authority);

    expect(snapshot).toEqual({ authority: 'unavailable', statuses: [] });
    expect(reconciled).toMatchObject({
  ...
  it('rejects a same-document status receipt bound to another project', () => {
    const reconciled = reconcileECOSHostedIndexStatus(cachedReadyDocument, {
      documentId: cachedReadyDocument.id,
  ...
  it.each([
    ...
    ['ready', 'Prepared'],
    ['ready', 'Prepared with limitations'],
    ['needs_review', 'Needs Review'],
    ['failed_internal', 'Needs Review'],
    ['cancelled', 'Needs Review'],
    ['reconnect_source', 'Reconnect Files'],
    ['temporarily_unavailable', 'Temporarily Unavailable'],
  ] as const)(
    'revokes cached Ready proof when authoritative %s status is %s',
    (state, customerStatus) => {
      const reconciled = reconcileECOSHostedIndexStatus(cachedReadyDocument, {
        ...
        progressPercent: 0,
        customerMessage: null,
        limitationCount: customerStatus === 'Prepared with limitations' ? 1 : 0,
        supportReference: null,
        committedEvidenceVersion: state === 'ready' ? 'ecos-hosted-evidence/1.3' : null,
        updatedAt: null,
        readyAt: null,
      }, 'available');

      expect(reconciled).toMatchObject({
        ecosHostedIndexStatus: customerStatus,
        ecosVerifiedIndexCommitVersion: null,
  ...
  it.each([
    ['extracting', 'Ready for ECOS', 'ecos-hosted-evidence/1.3'],
    ['ready', 'Ready for ECOS', null],
    ['ready', 'Ready with limitations', 'wrong-evidence-version'],
  ] as const)(
    'downgrades malformed Ready tuple state=%s status=%s evidence=%s',
    (state, customerStatus, committedEvidenceVersion) => {
      const reconciled = reconcileECOSHostedIndexStatus(cachedReadyDocument, {
        documentId: cachedReadyDocument.id,
        projectId: 'project-1',
        state,
        ...
        sourcePageCount: 1,
        progressPercent: 100,
        customerMessage: 'This value must not authorize readiness.',
        limitationCount: customerStatus === 'Ready with limitations' ? 1 : 0,
        supportReference: 'ECOS-MALFORMED',
        committedEvidenceVersion,
