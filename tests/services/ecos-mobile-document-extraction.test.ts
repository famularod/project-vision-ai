...
describe('ECOS mobile document extraction', () => {
  ...
  it('routes PDFs to hosted preparation even when a stale native module advertises extraction', async () => {
    ...
    expect(result).toMatchObject({
      ...
      extractedPages: [],
      extractionLimitations: [expect.stringMatching(/hosted ECOS worker/i)],
    });
  });
  ...
  it('routes a PDF filename to hosted preparation even when MIME metadata is absent', async () => {
    const result = await extractECOSMobileDocument({
      uri: 'file:///A101.pdf',
      mimeType: null,
      fileName: 'A101.pdf',
  ...
  it('does not send a PDF URI to native image OCR when metadata claims PNG', async () => {
    const result = await extractECOSMobileDocument({
    ...
    expect(mockRecognizeTextFromImage).not.toHaveBeenCalled();
    expect(result).toMatchObject({
      extractionStatus: 'not_supported',
      extractedPages: [],
      extractionLimitations: [expect.stringMatching(/hosted ECOS worker/i)],
