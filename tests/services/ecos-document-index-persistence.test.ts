...
describe('ECOS document index persistence', () => {
  it('keeps routine mobile document reads bounded without mutating the source records', () => {
    const source: ReferenceDocument[] = [{
      id: 'hosted-drawing-1',
      name: 'Issued drawing',
      originalFileName: 'issued.pdf',
      uri: '',
      category: 'Drawing',
      notes: '',
      isCurrent: true,
      ...
      extractedText: 'large duplicated index',
      extractedPages: [{
        pageNumber: 1,
        text: 'large duplicated page',
        regions: [{
          id: 'region-1',
          text: 'large duplicated region',
          x: 0.1,
          y: 0.2,
          width: 0.3,
          height: 0.04,
    ...
    const compact = compactECOSReferenceDocumentsForOperationalRead(source);
    ...
    expect(source[0].extractedPages).toHaveLength(1);
    expect(compact[0]).not.toBe(source[0]);
  });
