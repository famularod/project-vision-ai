...
const mockListProjects = jest.fn((..._args: unknown[]): Promise<{
  ok: boolean;
  configured: boolean;
  stubbed: boolean;
...
const mockUploadPhoto = jest.fn((..._args: unknown[]): Promise<{
  ok: boolean;
  configured: boolean;
  stubbed: boolean;
...
const mockCommitProjectCoverPhoto = jest.fn((..._args: unknown[]) => Promise.resolve({
  ok: true,
  configured: true,
  stubbed: false,
...
jest.mock('../../services/SupabaseService', () => ({
  ...
  commitProjectCoverPhoto: (...args: unknown[]) =>
    mockCommitProjectCoverPhoto(...args),
}));
...
beforeEach(() => {
  ...
  mockCommitProjectCoverPhoto.mockReset();
  mockCommitProjectCoverPhoto.mockResolvedValue({
    ok: true,
    configured: true,
    stubbed: false,
    data: { status: 'committed' },
...
describe('offline upload deletion barriers', () => {
  ...
  it('keeps a later cover mutation behind an earlier retryable predecessor', async () => {
    const projectId = '11111111-1111-4111-8111-111111111111';
    const shaA = 'a'.repeat(64);
    const shaB = 'b'.repeat(64);
    const coverA = {
      remotePath: `project-covers/${projectId}/revisions/33333333-3333-4333-8333-333333333333.jpg`,
      mimeType: 'image/jpeg',
      contentSha256: shaA,
      sizeBytes: 1000,
    ...
    const coverB = {
      remotePath: `project-covers/${projectId}/revisions/44444444-4444-4444-8444-444444444444.jpg`,
      mimeType: 'image/jpeg',
      contentSha256: shaB,
      sizeBytes: 2000,
    ...
    await enqueuePendingChange({
      ...
      payload: {
        ...
        data: {
          ...
          coverPhotoUpdatedAt: coverA.updatedAt,
        },
        coverPhotoCommit: {
          expected: { coverPhoto: null, mode: 'automatic', updatedAt: null },
          target: { coverPhoto: coverA, mode: 'manual', updatedAt: coverA.updatedAt },
        },
        coverPhotoUpload: {
          localUri: 'file:///cover-a.jpg',
          remotePath: coverA.remotePath,
          mimeType: coverA.mimeType,
          contentSha256: coverA.contentSha256,
          sizeBytes: coverA.sizeBytes,
        },
      },
      changedAt: '2026-08-11T16:00:00.000Z',
      autoUpload: false,
    });
    await enqueuePendingChange({
      id: 'cover-mutation-b',
      ...
      payload: {
        ...
        data: {
          coverPhoto: coverB,
          coverPhotoMode: 'manual',
          coverPhotoUpdatedAt: coverB.updatedAt,
        },
        coverPhotoCommit: {
          expected: { coverPhoto: coverA, mode: 'manual', updatedAt: coverA.updatedAt },
          target: { coverPhoto: coverB, mode: 'manual', updatedAt: coverB.updatedAt },
        },
        coverPhotoUpload: {
          localUri: 'file:///cover-b.jpg',
          remotePath: coverB.remotePath,
          mimeType: coverB.mimeType,
          contentSha256: coverB.contentSha256,
          sizeBytes: coverB.sizeBytes,
        },
      },
      changedAt: '2026-08-11T16:01:00.000Z',
      autoUpload: false,
    });

    const coverUploadResult = await uploadPendingChanges();
    expect(coverUploadResult).toMatchObject({
      uploaded: 0,
      queued: 2,
      itemOutcomes: {
        'cover-mutation-a': 'failed',
        'cover-mutation-b': 'blocked',
      },
    });
    expect(coverUploadResult.errors).toEqual([
      expect.stringMatching(/could not connect/i),
    ]);
    expect(mockListProjects).toHaveBeenCalledTimes(1);
    expect(mockUploadPhoto).toHaveBeenCalledTimes(1);
    expect(mockCommitProjectCoverPhoto).not.toHaveBeenCalled();
    await expect(getOfflineQueue()).resolves.toEqual([
      expect.objectContaining({ id: 'cover-mutation-a', retryCount: 1 }),
      expect.objectContaining({ id: 'cover-mutation-b', retryCount: 0 }),
    ]);
  });

  it('records the newest rejected cover choice before resolving conflicting queue rows', async () => {
    const projectId = '11111111-1111-4111-8111-111111111111';
    const changedAt = '2026-08-11T16:10:00.000Z';
    const localCover = {
      remotePath: `project-covers/${projectId}/revisions/55555555-5555-4555-8555-555555555555.jpg`,
      mimeType: 'image/jpeg',
    ...
    mockListProjects.mockResolvedValueOnce({
      ...
      data: [{
        ...
        data: {
          coverPhoto: null,
          coverPhotoMode: 'automatic',
          coverPhotoUpdatedAt: '2026-08-11T16:09:00.000Z',
        },
      }],
    });

    await enqueuePendingChange({
      id: 'cover-mutation-conflict',
      entity: 'project',
      operation: 'update',
      payload: {
        id: projectId,
        previousName: 'Shared Project',
        data: {
          coverPhoto: localCover,
          coverPhotoMode: 'manual',
          coverPhotoUpdatedAt: changedAt,
        },
        coverPhotoCommit: {
          expected: { coverPhoto: null, mode: 'automatic', updatedAt: null },
          target: { coverPhoto: localCover, mode: 'manual', updatedAt: changedAt },
        },
        coverPhotoUpload: {
          localUri: 'file:///cover-conflict.jpg',
          remotePath: localCover.remotePath,
          mimeType: localCover.mimeType,
          contentSha256: localCover.contentSha256,
          sizeBytes: localCover.sizeBytes,
        },
      },
      changedAt,
      autoUpload: false,
    ...
    await expect(uploadPendingChanges()).resolves.toMatchObject({
      uploaded: 0,
      queued: 0,
      itemOutcomes: { 'cover-mutation-conflict': 'conflict' },
    });
    await expect(getOfflineQueue()).resolves.toEqual([]);
    await expect(getSyncConflicts()).resolves.toEqual([
      expect.objectContaining({
        ...
        localPayload: expect.objectContaining({
          coverPhotoCommit: expect.objectContaining({
            target: expect.objectContaining({ coverPhoto: localCover }),
          }),
        }),
      }),
    ]);
