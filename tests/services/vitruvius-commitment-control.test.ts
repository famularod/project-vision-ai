...
describe('Vitruvius commitment control', () => {
  ...
  it('preserves the immutable task project id for exact navigation', () => {
    const result = buildVitruviusCommitmentControl({
      scheduleItems: [task({ projectId: 'project-2321' })],
      updates: [],
      now: NOW,
    });

    expect(result.topItem?.projectId).toBe('project-2321');
  });
