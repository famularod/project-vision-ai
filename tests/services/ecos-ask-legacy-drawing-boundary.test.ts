...
describe("Ask ECOS legacy drawing evidence boundary", () => {
  ...
  const shadowPageSelection = fs.readFileSync(
    path.join(
      process.cwd(),
  ...
  it("expands bounded private page evidence for ordinary cross-discipline lighting wording", () => {
    ...
    const decisiveGate = shadowPageSelection.slice(
      shadowPageSelection.indexOf("export function ecosHasDecisiveShadowPageEvidence("),
      shadowPageSelection.indexOf("function value("),
    );
    ...
    expect(
      decisiveGate.indexOf("asksCrossDisciplineLightingQuestion(question)"),
    ).toBeLessThan(
      decisiveGate.search(/requirement\.kind !== ["']presence["']/),
    );
  });
  ...
  it("does not stop on an unrelated area calculation and preserves design authority wording", () => {
    ...
    expect(decisiveGate).toContain("hazardousCanopyAreaRequested");
    expect(decisiveGate).toMatch(
      /!\/ECOS VERIFIED PLAN-FOOTPRINT CALCULATION:\/i\.test\(source\.excerpt\)/,
    );
    expect(decisiveGate).toContain("buildECOSDrawingAreaFallback(question, measurementSources)");
