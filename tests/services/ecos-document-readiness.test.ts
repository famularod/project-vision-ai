...
function drawing(overrides: Partial<ReferenceDocument> = {}): ReferenceDocument {
  ...
  if ((document.extractedPages ?? []).length > 0) {
    ...
    document.ecosVerifiedIndexCommittedPageCount = document.extractedPages!.length;
...
describe('ECOS document readiness', () => {
  ...
  it('revokes answer eligibility when pages change after the hosted receipt', () => {
    const committed = drawing({ isCurrent: true });
    const mutated: ReferenceDocument = {
      ...committed,
      extractedPages: committed.extractedPages!.map((page, index) => index === 0
        ? { ...page, text: 'Attacker-controlled false drawing fact.' }
        : page),
    };

    expect(buildECOSDocumentReadiness(mutated)).toMatchObject({
      status: 'stale',
      eligibleForAnswers: false,
      detail: 'The searchable page evidence does not match the server-issued hosted graph receipt.',
    });
  });
  ...
  it('recognizes a transactionally committed cloud drawing index without inline page JSON', () => {
    expect(buildECOSDocumentReadiness(drawing({
      ...
      ecosVerifiedIndexPageGraphSha256: 'c'.repeat(64),
    }))).toMatchObject({
      status: 'ready',
      label: 'Ready for ECOS',
      eligibleForAnswers: true,
  ...
  ] as const)('never falls back to a cached compact commit while hosted status is %s', hostedStatus => {
    expect(buildECOSDocumentReadiness(drawing({
      isCurrent: true,
      extractedPages: [],
      extractionStatus: 'complete',
      ...
      ecosVerifiedIndexCommittedSha256: HASH_A,
      ecosVerifiedIndexCommittedPageCount: 8,
      ecosHostedIndexStatus: hostedStatus,
    }))).toMatchObject({
      eligibleForAnswers: false,
    });
  });
