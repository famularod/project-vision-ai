...
describe('DAVE report intelligence', () => {
  ...
  it('changes the report source only when semantic project facts change', () => {
    ...
    const firstFingerprint = buildDAVEReportSourceFingerprint([first]);
    expect(firstFingerprint).toMatch(/^dave-report-source\/1\.0:[a-f0-9]{64}$/);
    expect(buildDAVEReportSourceFingerprint([refreshOnly])).toBe(firstFingerprint);
    expect(buildDAVEReportSourceFingerprint([changed])).not.toBe(firstFingerprint);
  });
