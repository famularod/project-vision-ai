...
describe('project document cloud byte restore adapter', () => {
  ...
  test('reference byte recovery cannot create authority before its exact CAS', () => {
    const app = fs.readFileSync(
      path.resolve(__dirname, '../../App.tsx'),
      'utf8',
    );
    const commit = app.match(
      /function commitVerifiedReferenceDocumentBytes[\s\S]+?\n  function markReferenceDocumentCurrent/,
    )?.[0] || '';
    const recovery = app.match(
      /async function ensureVerifiedReferenceDocumentBytes[\s\S]+?\n  async function openReferenceDocument/,
    )?.[0] || '';

    expect(commit).toContain('if (!referenceDocumentsAuthorityRef.current) return null;');
    expect(commit).toContain('commitReferenceDocumentByteRecovery({');
    expect(commit).not.toContain('markReferenceDocumentsAuthorityReady(true)');
    expect(recovery).toContain('prepareReferenceDocumentByteRecovery({');
    expect(recovery).not.toContain('updateReferenceDocument(');
  });
});
