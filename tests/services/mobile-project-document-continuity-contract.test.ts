...
describe('mobile project-document continuity contract', () => {
  ...
  it('shows exact cloud documents on a fresh device and scopes the navigation count', () => {
    expect(source).toContain('const selectedWorkspaceDocuments = projectDocumentsForExactWorkspace({');
    expect(source).toContain('documentCount={selectedWorkspaceDocuments.length}');
    expect(source).toContain('documents={selectedWorkspaceDocuments}');
    expect(source).not.toContain('documentCount={referenceDocuments.length}');
  });
