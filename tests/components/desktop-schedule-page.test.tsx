...
describe('DesktopSchedulePage', () => {
  ...
  test('blocks a dependency mutation when the task id belongs to another exact project', async () => {
    const siblingTask = scheduleItem('sibling-task', {
      projectId: 'project-b',
      taskName: 'Sibling task',
      dependencies: [{ predecessorItemId: 'sibling-predecessor', type: 'FS' }],
    });
    const siblingPredecessor = scheduleItem('sibling-predecessor', {
      projectId: 'project-b',
      taskName: 'Sibling predecessor',
      status: 'Complete',
      percentComplete: 100,
    });
    const screen = render(
      <DesktopSchedulePage
        tasks={[siblingPredecessor, siblingTask]}
        projects={['Shared Project']}
        projectRecords={[
          { id: 'project-a', name: 'Shared Project' },
          { id: 'project-b', name: 'Shared Project' },
        ]}
        selectedProject="Shared Project"
    ...
    fireEvent.press(screen.getByText('Gantt'));
    fireEvent.press(screen.getByText(/Impact preview/));
    fireEvent.press(screen.getByLabelText('Apply calculated dates for Sibling task'));

    await waitFor(() => expect(screen.getByText(
      'This schedule item is outside the selected project. Refresh before changing it.',
    )).toBeTruthy());
