...
import {
  ...
  type DAVECaptureProjectOption,
} from './components/DAVECaptureConfirmationSheet';
...
import {
  ...
  type ProjectDocumentWorkspaceRecord,
...
import {
  ...
  parseCaptureMemoryRepositoryStorageValue,
...
import {
  ...
  projectWalkSessionRemovalOperationForProject,
} from './services/DAVEProjectWalkSessionRepository';
import {
...
type ProjectUpdate = {
  id: string;
  /** Immutable project identity. Name-only legacy updates remain cover-ineligible. */
...
type ProjectDocumentStatus =
  ...
  | 'uploaded'
  | 'failed';
type ProjectDocument = ProjectDocumentWorkspaceRecord;
...
type RestoredAppData = {
  ...
  projectRecords: Array<ProjectRecord & { id: string }>;
...
const PROJECT_DELETION_STORAGE_KEYS: ProjectDeletionStorageKeys = {
  ...
  projectWalkSession: DAVE_PROJECT_WALK_SESSION_STORAGE_KEY,
...
const backupRestoreRuntime = createBackupRestoreRuntime({
  ...
  loadQueuedProjectDeletions: async () => (await getOfflineQueue())
    .flatMap(item => {
      if (item.entity !== 'project' || item.operation !== 'delete') return [];
      const payload = item.payload as Record<string, unknown>;
      const projectName = typeof payload.name === 'string' ? payload.name.trim() : '';
      if (!projectName) return [];
      const projectId = persistedExactProjectId(payload.projectId);
...
function normalizePhoto(photo: Partial<UpdatePhoto>): UpdatePhoto {
  return {
    ...
    contentSha256:
      typeof photo.contentSha256 === 'string' && /^[a-f0-9]{64}$/.test(photo.contentSha256)
        ? photo.contentSha256
        : null,
...
function normalizeUpdate(update: Partial<ProjectUpdate>): ProjectUpdate {
  ...
  return {
    id: updateId,
    projectId: typeof update.projectId === 'string' &&
      update.projectId.length > 0 &&
      update.projectId.trim() === update.projectId
      ? update.projectId
      : null,
    projectName,
...
function normalizeScheduleItem(
  ...
  return {
    id: typeof value.id === 'string' ? value.id : uid(),
    projectId: persistedExactProjectId(value.projectId),
...
function normalizeBackupProjectRecords(
  values: readonly unknown[],
): Array<ProjectRecord & { id: string }> {
  const records = values.map((value, index) => {
    const project = normalizeProjectRecords([value])[0];
    if (!project?.id || !isProjectId(project.id)) {
      throw new Error(`Backup project ${index + 1} is missing its immutable project ID.`);
    }
    return { ...project, id: project.id };
  });
  return restoreProjectRecords(records);
}
...
function normalizeBackupData(value: unknown) {
  const preflight = preflightAppBackup(value, {
    ...
    projectRecord: value => isStartupProjectRecord(value) &&
      isRecord(value) && isProjectId(value.id),
  ...
  return {
    ...
    data: {
      ...
      projectRecords: normalizeBackupProjectRecords(data.projectRecords),
      contactBook: normalizeContacts(data.contacts), projectAreas: normalizeProjectAreas(data.projectAreas),
      referenceDocuments: revokeCachedECOSDocumentAuthority(
        normalizeReferenceDocuments(data.referenceDocuments),
      ),
...
function projectDocumentMatchesProject(
  ...
  const exactId = persistedExactProjectId(projectId);
  if (exactId) return document.projectId === exactId;

  const legacyProjectId = authorityProjectId(projectName);

  return (
    document.projectId === legacyProjectId ||
...
function projectDocumentsForProject(
  ...
  return documents.filter(
    document =>
      !document.isArchived &&
      projectDocumentMatchesProject(document, projectName, projectId),
  );
}
...
function projectDocumentsForScopes(
  ...
  return documents.filter(document =>
    !document.isArchived &&
    projectNames.some(projectName => projectDocumentMatchesProject(document, projectName, projectId)),
  );
}
...
function AppShell() {
  ...
  const talkRequestAuthority = useRef(createECOSTalkRequestAuthority()).current;
  ...
  void (async () => {
    try {
      ...
      const syncResult = await synchronizeLocalData({
        ...
        projectRecords: projectRecordsCurrentRef.current.flatMap(record => {
          const projectId = persistedExactProjectId(record.id);
          return projectId ? [{ id: projectId, name: record.name }] : [];
        }),
      ...
      for (const legacyProject of remainingLegacyProjects) {
        const legacyProjectId = persistedExactProjectId(legacyProject.id);
        if (!legacyProjectId) return;
        await deleteCloudProject(legacyProjectId, legacyProject.name);
      }
...
useEffect(() => {
  ...
  void backupRestoreRuntime.recoverBeforeStartupReads()
    .then(() => localDAVECaptureMemoryRepository.list())
    .then(memories => {
      if (active) {
...
useEffect(() => {
  ...
  void backupRestoreRuntime.recoverBeforeStartupReads()
    .then(() => localDAVEProjectWalkSessionRepository.readActive())
    .then(session => {
...
useEffect(() => {
  async function loadProjects() {
    try {
      ...
      const queuedProjectDeletions = queuedProjectDeletionBarriers(queuedChanges);
      const queuedDeletedProjectIds = new Set<string>(
        queuedProjectDeletions.exactProjectIds,
      );
      const localProjects = normalizeProjectRecords(localResult.value).filter(project =>
        !project.id || !queuedDeletedProjectIds.has(project.id),
      );
      const legacyQueuedDeletedNames = queuedProjectDeletions.legacyProjectNames;
      ...
      projectRecordsCurrentRef.current = localRecords;
      projectsCurrentRef.current = localRecords.map(project => project.name);
      setProjectRecords(localRecords);
      setProjects(projectsCurrentRef.current);
      ...
      try {
        ...
        const nonProjectShellRecords = cloudProjects.filter(project =>
          isLegacyNonProjectShellName(project.name),
        );
        ...
        if (nonProjectShellRecords.length > 0 && shellMigrationComplete !== 'complete') {
          await queueCloudProjectArchives(nonProjectShellRecords);
        ...
        const mergedProjectRecords = mergeProjectRecords(
          [],
          projectRecordsCurrentRef.current,
          visibleCloudProjects,
          currentDeletedNames,
        );
        const mergedProjectNames = mergedProjectRecords
          .map(project => project.name)
          .filter(project => !deletedKeys.has(project.toLowerCase()));
        projectRecordsCurrentRef.current = mergedProjectRecords;
        projectsCurrentRef.current = mergedProjectNames;
        setProjectRecords(mergedProjectRecords);
        setProjects(mergedProjectNames);
...
useEffect(() => {
  ...
  void Promise.all(projectRecords.map(async project => {
    const projectId = project.id?.trim() || null;
    if (!projectId) return;
    const hydrationTarget = coverPhotoForProject(projectRecords, projectId);
    if (!hydrationTarget?.remotePath) return;
    ...
    setProjectRecords(previous => previous.map(item => {
      if (item.id?.trim() !== projectId) return item;
  ...
  useStartupLocalFirstRecovery<ReferenceDocument, ReferenceDocument, ReferenceDocument>({
    ...
    acceptLocal: result => startupHydration.accept([result]),
    normalizeLocal: documents => revokeCachedECOSDocumentAuthority(
      normalizeReferenceDocuments(documents),
    ),
  ...
  useEffect(() => {
    ...
    async function refreshOperationalCollections(
      ...
      refreshCommit.commit(() => {
        ...
        const projectTombstoneRecordIds = deletedDAVERecordIds(
          latestOperationalTombstones,
          'project',
        );
        const legacyProjectTombstoneNames = new Set(
          projectTombstoneRecordIds
            .filter(recordId => !projectTombstoneUsesExactId(
              recordId,
              projectRecordsCurrentRef.current,
            ))
            .map(recordId => recordId.trim().toLowerCase()),
        );
        const tombstonedProjectNames = projectRecordsCurrentRef.current
          .flatMap(record => {
            const nameKey = record.name.trim().toLowerCase();
            // Legacy cloud functions wrote the display name itself. Honor
            // that broad historical barrier. Current UUID tombstones never
            // become name barriers, even when a later project reuses a name.
            if (legacyProjectTombstoneNames.has(nameKey)) return [record.name];
            return [];
          });
        const nextDeletedProjectNames = mergeProjectNames(
          deletedProjectNamesRef.current,
          tombstonedProjectNames,
        );
        if (
  ...
  const selectedWorkspaceCoverProject = useMemo(
    () => exactProjectRecordForCoverName(projectRecords, selectedWorkspaceProject),
    [projectRecords, selectedWorkspaceProject],
  );

  const talkProjectAreas = useMemo(
    () => {
      const selection = resolveECOSProjectSelection({
        projectId: talkProjectId,
        projectName: talkProjectName,
        projectRecords,
      });
      const matchingRecords = selection ? projectRecords.filter(record =>
        Boolean(record.id) &&
        normalizeProjectDisplayName(record.name) === normalizeProjectDisplayName(selection.name)) : [];
      if (!selection || matchingRecords.length !== 1 || matchingRecords[0].id !== selection.id) return [];
      return projectAreasForProject({
        projectAreas,
    ...
    [activeSavedUpdates, projectAreas, projectRecords, scheduleItems, talkProjectId, talkProjectName],
  );
  ...
  async function publishUploadedProjectDocument(
    ...
    const ownedRecord = document.ownedFileId
      ...
      : null;
    const projectMatches = projectRecordsCurrentRef.current.filter(record =>
      persistedExactProjectId(record.id) === document.projectId,
    );
    const projectName = projectMatches.length === 1 ? projectMatches[0].name : null;
    if (!projectName) {
      throw new Error('The document is not bound to one confirmed project ID.');
    }
  ...
  async function createProjectDocumentFromAsset(asset: {
    ...
    const project = exactProjectRecordForCoverName(
      projectRecordsCurrentRef.current,
      projectName,
    );
    const projectId = persistedExactProjectId(project?.id);
    if (!project || !projectId) {
      throw new Error('Choose one project with a confirmed project ID before adding a document.');
    }
  ...
  async function confirmDocumentProjectSelection() {
    ...
    try {
      ...
      const selectedReplacementProjectId = selectedProjectNames.length === 1
        ? persistedExactProjectId(exactProjectRecordForCoverName(
            projectRecordsCurrentRef.current,
      ...
      const replacementDocument = category === 'Drawing' &&
        ...
        ? referenceDocumentsCurrentRef.current.find(document =>
            ...
            document.projectId === selectedReplacementProjectId,
          ) || null
        : null;
  ...
  async function saveFieldUpdateFromReview() {
    ...
    const draftProjectId = persistedExactProjectId(draftSnapshot.projectId);
    const draftProjectName = (
      draftSnapshot.scheduleProjectName || draftSnapshot.projectName
    ).trim();
    const exactDraftProject = draftProjectId
      ? projectRecordsCurrentRef.current.filter(record =>
          persistedExactProjectId(record.id) === draftProjectId &&
          record.name.trim().toLowerCase() === draftProjectName.toLowerCase(),
        )
      : [];
    if (exactDraftProject.length !== 1) {
      Alert.alert(
        'Project confirmation required',
        'This update is not bound to one current project ID. Start a new update from the project workspace before saving.',
      );
      return;
    }
  ...
  function beginDraftForProject(projectName: string) {
    const project = exactProjectRecordForCoverName(projectRecordsCurrentRef.current, projectName);
    const projectId = persistedExactProjectId(project?.id);
    if (!project || !projectId) {
      Alert.alert(
        'Project confirmation required',
        'Choose one project with a confirmed project ID before starting an update.',
      );
      return;
    }
    setDraft({ ...createDraft(project.name), projectId });
    setSelectedWorkspaceProject(project.name);
    setScreen('AddPhotos');
    draftLocationCaptureRef.current = captureDraftLocation();
  }
  ...
  function createNewUpdateForScheduleTask(
    ...
    const taskProjectId = persistedExactProjectId(scheduleItem.projectId);
    const exactTaskProject = taskProjectId
      ? projectRecordsCurrentRef.current.filter(record =>
          persistedExactProjectId(record.id) === taskProjectId &&
          record.name.trim().toLowerCase() === parentProjectName.trim().toLowerCase(),
        )
      : [];
    if (exactTaskProject.length !== 1) {
      Alert.alert(
        'Project confirmation required',
        'Refresh this task or recreate it under one confirmed project before adding a field update.',
      );
      return;
    }
  ...
  function prepareProjectWalkFieldUpdate(
    ...
    const exactWalkProjectId = persistedExactProjectId(projectId);
    const exactWalkProjects = exactWalkProjectId
      ? projectRecordsCurrentRef.current.filter(record =>
          persistedExactProjectId(record.id) === exactWalkProjectId &&
          record.name.trim().toLowerCase() === projectName.trim().toLowerCase(),
        )
      : [];
    if (!exactWalkProjectId || exactWalkProjects.length !== 1) {
      Alert.alert(
        'Project confirmation required',
        'Refresh this project before preparing its Project Walk field update.',
      );
      return;
    }
    let prepared: ReturnType<typeof buildDAVEProjectWalkFieldUpdateDraft>;
    try {
      prepared = buildDAVEProjectWalkFieldUpdateDraft({
        projectId,
        projectName,
        memories,
      });
  ...
  async function startProjectWalk(projectId: string, projectName: string) {
    const requestedProject = resolveECOSProjectSelection({
      projectId,
      projectName,
      projectRecords: projectRecordsCurrentRef.current,
    });
    if (!requestedProject) {
      Alert.alert(
        'Exact project required',
        'Refresh this project before starting a Project Walk.',
      );
      return false;
    }
    const existing = await localDAVEProjectWalkSessionRepository.readActive();
    if (existing) {
      const existingProject = resolveECOSProjectSelection({
        projectId: existing.projectId,
        projectName: null,
        projectRecords: projectRecordsCurrentRef.current,
      });
      if (!existingProject) {
        await localDAVEProjectWalkSessionRepository.deleteActiveForProject(
          existing.projectId,
        );
        setActiveProjectWalkSession(current =>
          current?.id === existing.id ? null : current,
        );
      } else {
        setActiveProjectWalkSession(existing);
        if (existing.projectId === requestedProject.id) {
          return true;
        }
        Alert.alert(
          'Project Walk already in progress',
          `Finish or end the walk for ${existingProject.name} before starting another one.`,
          [
            { text: 'Not Now', style: 'cancel' },
            {
              text: 'Resume Walk',
              onPress: () => openProjectWorkspace(existingProject.name),
            },
          ],
        );
        return false;
  ...
  async function saveCaptureMemory(
    ...
    const walkSession = walkSessionId
      ? await localDAVEProjectWalkSessionRepository.readActive()
      : null;
    if (
      walkSessionId &&
      (!walkSession ||
        walkSession.id !== walkSessionId ||
        walkSession.projectId !== memory.projectId)
    ) {
      throw new Error('The Project Walk and confirmed memory do not share the same exact project.');
    }
  ...
  async function finishProjectWalk(sessionId: string) {
    try {
      ...
      const currentProject = resolveECOSProjectSelection({
        projectId: session.projectId,
        projectName: null,
        projectRecords: projectRecordsCurrentRef.current,
      });
      if (!currentProject) {
        throw new Error('The Project Walk project no longer exists on this device.');
      }
      ...
      const sessionMemories = session.memoryIds
        ...
        .filter((memory): memory is DAVEConfirmedCaptureMemory =>
          Boolean(memory && memory.projectId === session.projectId),
        );
  ...
  async function persistSelectedProjectCoverPhoto(
    ...
    try {
      const record = exactProjectRecordForCoverName(projectRecords, projectName);
      const projectId = record?.id?.trim() || null;
      if (!record || !projectId) {
        Alert.alert(
          'Cover photo unavailable',
          'Choose one project with a confirmed project ID before changing its cover photo.',
        );
        return;
      }
      ...
      saveCloudProjectCoverPhoto(projectId, projectName, coverPhoto, 'manual', record);
    } catch {
  ...
  function removeProjectCoverPhoto(projectName: string) {
    ...
    const current = coverPhotoForProject(projectRecords, projectId);
    Alert.alert(
      'Remove cover photo?',
...
function addProject(projectName: string) {
  ...
  setProjectRecords(prev => [{ id: projectId, name: trimmed }, ...prev]);
  clearProjectDeletion(trimmed);

  saveCloudProject(projectId, trimmed);
  ...
  function closeProject(projectName: string) {
    const project = resolveECOSProjectSelection({
      projectId: null,
      projectName,
      projectRecords,
    });
    if (!project) {
      Alert.alert(
        'Exact project required',
        'This project cannot be archived until its immutable identity is available.',
      );
      return;
    }
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            setCloudProjectArchived(project.id, project.name, true);
            setScreen('Home');
          },
        },
      ],
  ...
  function reopenProject(projectName: string) {
    ...
    if (!project) {
      Alert.alert(
        'Exact project required',
        'This project cannot be reopened until its immutable identity is available.',
      );
      return;
    }
    ...
    setCloudProjectArchived(project.id, project.name, false);
  }
  ...
  async function deleteProjectPermanently(
    ...
    const deletionProject = resolveECOSProjectSelection({
      projectId: requestedProjectId,
      projectName: requestedProjectName,
      projectRecords,
    });
    const matchingProjectRecords = deletionProject ? projectRecords.filter(record =>
      normalizeProjectDisplayName(record.name) ===
        normalizeProjectDisplayName(deletionProject.name)) : [];
    if (
      !deletionProject ||
      !canonicalProjectId(deletionProject.id) ||
      matchingProjectRecords.length !== 1 ||
      matchingProjectRecords[0].id !== deletionProject.id
    ) {
      Alert.alert(
        'Exact project required',
        'This project could not be deleted because its immutable identity is missing or ambiguous.',
      );
      return;
    }
    const { id: projectId, name: projectName } = deletionProject;
    ...
    try {
      ...
      const deletionResult = await projectDeletionRuntime.commit(
        async support => {
          ...
          const remainingProjectNames = projectRecords
            ...
            .map(project => project.name);
          const durableCaptureMemories = parseCaptureMemoryRepositoryStorageValue(
            await AsyncStorage.getItem(DAVE_CAPTURE_MEMORY_STORAGE_KEY),
          );
          const remainingCaptureMemories = captureMemoriesAfterProjectDeletion(
            durableCaptureMemories,
            projectId,
          );
          const projectWalkSessionRemoval = projectWalkSessionRemovalOperationForProject(
            await AsyncStorage.getItem(DAVE_PROJECT_WALK_SESSION_STORAGE_KEY),
            projectId,
          );
          ...
          const draftBelongsToSelectedProject = projectUpdateBelongsToParentProject({
            update: draftRef.current,
            projectName,
            selectedProjectId: projectId,
            scheduleItems,
          });
          const draftHasUnboundProjectScope =
            !canonicalProjectId(draftRef.current.projectId) &&
            projectUpdateBelongsToParentProject({
              update: draftRef.current,
              projectName,
              scheduleItems,
          ...
          const cascade = buildProjectDeletionCascade({
            ...
            draftBelongsToProject: draftBelongsToSelectedProject,
            draftHasUnboundProjectScope,
          ...
          return {
            operations: [
              ...
              {
                kind: 'set' as const,
                key: DAVE_CAPTURE_MEMORY_STORAGE_KEY,
                value: JSON.stringify(captureMemoryRepositoryStorageValue(
                  remainingCaptureMemories,
                )),
              },
              ...(projectWalkSessionRemoval ? [projectWalkSessionRemoval] : []),
            ],
            result: {
              cascade,
              fallbackProject,
              replacementDraft,
              remainingCaptureMemories,
              removedProjectWalkSession: Boolean(projectWalkSessionRemoval),
            },
          };
        },
      );
      ...
      setCaptureMemories(remainingCaptureMemories);
      if (removedProjectWalkSession) {
        setActiveProjectWalkSession(current =>
...
Note: This update was opened through Outlook because PLZ email security may reject messages sent from personal mail accounts. Photos are not attached automatically in this mode.`,
  ...
  async function exportBackup(passphrase: string) {
    ...
    try {
      ...
      const backupDraft = hasMeaningfulDraft(draft)
        ...
        : null;
      const backupProjectRecords = normalizeBackupProjectRecords(projectRecords);
      const backupProjectNames = backupProjectRecords.map(project => project.name);
      const backupProjectNameKeys = new Set(
        backupProjectNames.map(normalizeProjectDisplayName),
      );
      const visibleProjectNameKeys = new Set(
        projects.map(normalizeProjectDisplayName),
      );
      if (
        visibleProjectNameKeys.size !== backupProjectNameKeys.size ||
        [...visibleProjectNameKeys].some(key => !backupProjectNameKeys.has(key))
      ) {
        throw new Error(
          'Project identities are still reconciling. Retry the backup after project refresh finishes.',
        );
      }
      const backup = {
        ...
        projects: backupProjectNames,
        projectRecords: backupProjectRecords,
        archivedProjects: archivedProjects.filter(projectName =>
          backupProjectNameKeys.has(normalizeProjectDisplayName(projectName)),
        ),
  ...
  async function applyRestoredData(data: RestoredAppData): Promise<boolean> {
    ...
    try {
        ...
        const result = buildDeletionSafeRestoreState({
          ...
          referenceDocumentBelongsToProject: (document, name) =>
            referenceDocumentMatchesDeletedProject(
              document,
              name,
              authorityProjectId(name),
              true,
            ),
          projectDocumentBelongsToProject: projectDocumentMatchesProject,
          scheduleItemBelongsToProject: (item, name) =>
            scheduleItemMatchesDeletedProject(
              item,
              name,
              authorityProjectId(name),
              true,
      ...
      const restoredProjectWalkSession =
        await localDAVEProjectWalkSessionRepository.readActive();

      savedUpdatesRef.current = restored.savedUpdates; draftRef.current = restored.draft;
      projectRecordsCurrentRef.current = restored.projectRecords;
      projectsCurrentRef.current = restored.projects;
      archivedProjectsCurrentRef.current = restored.archivedProjects;
      ...
      setActiveProjectWalkSession(restoredProjectWalkSession);
  ...
  function commitVerifiedReferenceDocumentBytes(
    expectedDocument: ReferenceDocument,
    readableDocument: ReferenceDocument,
  ): ReferenceDocument | null {
    if (!referenceDocumentsAuthorityRef.current) return null;
    ...
    if (commit.status !== 'committed') return null;
    if (!commit.changed) return commit.document;

    const nextDocuments = [...commit.documents];
    referenceDocumentsCurrentRef.current = nextDocuments;
    setReferenceDocuments(nextDocuments);
    void queueReferenceDocumentRecord(commit.document);
  ...
  async function ensureVerifiedReferenceDocumentBytes(
    ...
    if ((!info?.exists || !resolvedUri) && document.storagePath) {
      const readableDocument = await prepareReferenceDocumentByteRecovery({
        document,
        recover: restoreReferenceDocumentBytesFromCloud,
      });
      const committedDocument = commitVerifiedReferenceDocumentBytes(
        document,
        readableDocument,
      );
      if (!committedDocument) {
        throw new Error('The document changed while its verified bytes were being recovered.');
      }
  ...
  async function openReferenceDocument(
    document: ReferenceDocument,
    isEvidenceAuthorityCurrent?: (candidate: ReferenceDocument) => boolean,
  ) {
    try {
      const outcome = await openReferenceDocumentWithAuthority({
        document,
        prepareDocument: ensureVerifiedReferenceDocumentBytes,
        isAuthorityCurrent: candidate => {
          if (!referenceDocumentsAuthorityRef.current) return false;
          const matchingDocuments = referenceDocumentsCurrentRef.current.filter(
            current => current.id === document.id,
          );
          if (matchingDocuments.length !== 1) return false;
          const currentDocument = matchingDocuments[0];
          return (
            referenceDocumentRecoveryAuthorityMatches(document, currentDocument) &&
            referenceDocumentRecoveryAuthorityMatches(candidate, currentDocument) &&
            (isEvidenceAuthorityCurrent?.(currentDocument) ?? true)
          );
        },
        isShareAvailable: () => Sharing.isAvailableAsync(),
        shareDocument: readableDocument => Sharing.shareAsync(readableDocument.uri, {
      ...
      if (outcome === 'stale') {
        Alert.alert(
          'Open canceled',
          'The cited current document changed before it could be shared. Open the current source and try again.',
        );
      }
    } catch {
  ...
  function updateProjectDocument(
    ...
    const projectMatches = projectRecordsCurrentRef.current.filter(record =>
      persistedExactProjectId(record.id) === changedDocument.projectId,
    );
    const projectName = projectMatches.length === 1 ? projectMatches[0].name : null;
    if (!projectName) return;
  ...
  async function makeProjectScheduleDocumentCurrent(documentId: string) {
    ...
    try {
      if (!referenceDocument) {
        ...
        const matchingProjectRecords = projectRecordsCurrentRef.current.filter(record =>
          persistedExactProjectId(record.id) === document.projectId,
        );
        const projectName = matchingProjectIds.size === 1 && matchingProjectRecords.length === 1
          ? matchingProjectRecords[0].name
          : null;
        const ownedRecord = document.ownedFileId
  ...
  function addScheduleItem(item: Partial<ScheduleItem>) {
    ...
    if (!project || !projectId) {
      Alert.alert(
        'Project confirmation required',
        'Choose one project with a confirmed project ID before adding a task.',
      );
      return;
    }
  ...
  async function prepareScheduleImportFromAsset(
    ...
    const scopedProjectRecords = scopeProjects.flatMap(projectName => {
      const record = exactProjectRecordForCoverName(
        projectRecordsCurrentRef.current,
        projectName,
      );
      const id = persistedExactProjectId(record?.id);
      return record && id ? [{ id, importIdentity: id, name: record.name }] : [];
    });
    if (scopedProjectRecords.length !== scopeProjects.length) {
      Alert.alert(
        'Project confirmation required',
        'Every selected schedule project must have one confirmed project ID before import.',
      );
      return null;
    }
    ...
    const unavailableScopeProjects = [
      ...archivedProjectsCurrentRef.current.flatMap(name =>
        projectRecordsCurrentRef.current.flatMap(record => {
          const id = persistedExactProjectId(record.id);
          return id && record.name.trim().toLowerCase() === name.trim().toLowerCase()
            ? [{ id, name, state: 'archived' as const }]
            : [];
        }),
      ),
      ...deletedProjectNamesRef.current.flatMap(name =>
        projectRecordsCurrentRef.current.flatMap(record => {
          const id = persistedExactProjectId(record.id);
          return id && record.name.trim().toLowerCase() === name.trim().toLowerCase()
            ? [{ id, name, state: 'deleted' as const }]
            : [];
        }),
      ),
    ];
    ...
    const onlyProject = scopedProjectRecords.length === 1 ? scopedProjectRecords[0] : null;
    const scheduleDocument = normalizeReferenceDocument({
      ...
      isCurrent: true,
      importedAt: new Date().toISOString(),
      projectId: onlyProject?.id || null,
      projectName: onlyProject?.name || null,
      projectNames: scopedProjectRecords.map(project => project.name),
    ...
    // PDF schedule imports always use the bounded server extractor. A stale
    // native binary must not revive the removed, uncancellable PDFKit path.
    ...
    const reviewItem = {
      ...normalizeScheduleItem({
        ...
        scheduleProjectName: onlyProject?.name || null,
        locationName: '',
  ...
  async function approveScheduleImport(batch: PIEScheduleImportBatch): Promise<void> {
    ...
    const activeProjectNames = projectsCurrentRef.current
      .filter(name => {
        const key = name.trim().toLowerCase();
        return key && !archivedKeys.has(key) && !deletedKeys.has(key);
      });
    const activeProjectRecords = activeProjectNames.flatMap(name => {
      const record = exactProjectRecordForCoverName(
        projectRecordsCurrentRef.current,
        name,
      );
      const id = persistedExactProjectId(record?.id);
      return record && id ? [{ id, name: record.name }] : [];
    });
    if (activeProjectRecords.length !== activeProjectNames.length) {
      throw new Error(
        'Refresh projects before approving this import. Every active project must have one confirmed project ID.',
      );
    }
    ...
    if (approvedBatch.documents.length) {
      ...
      const scopedDocuments = approvedBatch.documents.map(document => {
        ...
        const importedProjectName = documentProjectNames.length === 1
          ...
          : null;
        const importedProject = importedProjectName
          ? selectedProjectRecords.find(project =>
              project.name.trim().toLowerCase() === importedProjectName.trim().toLowerCase(),
            ) || null
          : null;
        if (importedProjectName && !importedProject) {
          throw new Error(
            `Refresh ${importedProjectName} before saving this schedule document. Its exact project ID is unavailable.`,
          );
        }
        return normalizeReferenceDocument({
          ...document,
          projectNames: documentProjectNames,
          projectId: importedProject?.id || null,
          projectName: importedProject?.name || null,
          updatedAt: referenceUpdatedAt,
        });
      });
  ...
  function ensureScheduleParentProjects(
    ...
    const missingProjectRecords = missingNames.map(name => ({
      id: createProjectId(),
      name,
    }));
    const reopeningProjects = reopeningNames.map(name =>
      resolveECOSProjectSelection({
        projectId: null,
        projectName: name,
        projectRecords: projectRecordsCurrentRef.current,
    ...
    if (reopeningProjects.some(project => !project)) {
      Alert.alert(
        'Exact project required',
        'An archived schedule project could not be reopened because its immutable identity is missing or ambiguous.',
      );
      return;
    }

    if (missingProjectRecords.length) {
      setProjects(previous => mergeProjectNames(previous, missingNames));
      setProjectRecords(previous => [
        ...missingProjectRecords.filter(candidate =>
          !previous.some(project => project.name.toLowerCase() === candidate.name.toLowerCase())),
        ...previous,
      ]);
    }
    ...
    if (reopeningNames.length) {
      ...
      reopeningProjects.forEach(project => {
        if (project) setCloudProjectArchived(project.id, project.name, false);
      });
    }

    missingProjectRecords.forEach(project => {
      const key = project.name.toLowerCase();
      if (scheduleParentProjectsQueuedRef.current.has(key)) return;
      scheduleParentProjectsQueuedRef.current.add(key);
      saveCloudProject(project.id, project.name);
    });
  }
  ...
  const talkProjectOptions = useMemo(() => buildECOSProjectSelectionOptions({
    projectRecords,
    candidateProjectNames: reportAvailableProjectNames,
  }), [projectRecords, reportAvailableProjectNames]);
  const currentTalkProject = useMemo(() => resolveECOSProjectSelection({
    projectId: talkProjectId,
    projectName: talkProjectName,
    projectRecords,
  }), [projectRecords, talkProjectId, talkProjectName]);
  const talkCandidateTasks = useMemo(() => {
    if (!currentTalkProject) return [];

    return [...scheduleTasksForParentProject(
      currentTalkProject.name,
      authoritativeScheduleItems as unknown as import('./types').ScheduleItem[],
      currentTalkProject.id,
    )]
      .sort((left, right) => {
  ...
  }, [authoritativeScheduleItems, currentTalkProject]);

  function talkProjectAllowsLegacyNameScope(projectId: string, projectName: string) {
    const matchingRecords = projectRecords.filter(record =>
      normalizeProjectDisplayName(record.name) === normalizeProjectDisplayName(projectName));
    return matchingRecords.length === 1 && matchingRecords[0].id === projectId;
  }

  function projectIntelligenceForTalk(projectId: string, projectName: string, taskId: string | null = null,
    talkReferenceDocuments: readonly ReferenceDocument[] = referenceDocuments) {
    return buildECOSTalkProjectIntelligence({
      projectId,
      projectName,
      taskId,
      legacyNameScopeIsUnambiguous: talkProjectAllowsLegacyNameScope(projectId, projectName),
      updates: activeSavedUpdates,
  ...
  function openTalk() {
    talkRequestAuthority.invalidate();
    const contextualProject = talkContextProjectForScreen(
      screen,
      selectedWorkspaceProject,
      selectedReportProjectNames[0] || null,
    );
    const contextualSelection = resolveECOSProjectSelection({
      projectId: null,
      projectName: contextualProject,
      projectRecords,
    });
    setTalkProjectId(contextualSelection?.id || null);
    setTalkProjectName(contextualSelection?.name || '');
    setTalkTaskId(null);
    setTalkAnswer(null);
  ...
  function openGuidedTaskFromTalk() {
    talkRequestAuthority.invalidate();
    const projectName = currentTalkProject?.name || '';
    if (!currentTalkProject || !projectName) return;
  ...
  function openTalkSupportingEvidence(
    ...
    const intelligence = projectIntelligenceForTalk(projectId, projectName);
  ...
  function reportTalkAnswerPersistenceFailure(projectId: string, error: unknown) {
    reportStoragePersistenceFailure({
      storageKey: daveAskHistoryStorageKey(projectId),
      label: 'Talk history',
      error,
    });
  }
  ...
  async function handleTalkInput(
    ...
    // A submission supersedes every older Talk request, even if this one later
    // fails exact-project resolution.
    talkRequestAuthority.invalidate();
    ...
    const selection = resolveECOSProjectSelection({
      projectId: mentionedProject ? null : talkProjectId,
      projectName: mentionedProject || talkProjectName,
      projectRecords,
    });
    if (!selection) {
      Alert.alert(
        'Choose the exact project',
        'This project name is missing an immutable identity or matches more than one project. Choose the exact project before using Talk.',
      );
      return;
    }
    const { id: projectId, name: projectName } = selection;
    const taskContextId = mentionedProject && projectId !== talkProjectId
      ? null
      : talkTaskId;
    const requestTicket = talkRequestAuthority.begin({
      projectId,
      projectName,
      question: transcript,
      taskId: taskContextId,
    });
    const requestIsCurrent = () => talkRequestAuthority.isCurrent(requestTicket);
    ...
    let talkDocuments: readonly ReferenceDocument[];
    try {
      talkDocuments = await loadECOSTalkReferenceDocuments({
      ...
      Alert.alert(
        ...
        'Talk could not load current verified project evidence. Try again when the hosted preparation service is available.',
      );
      return;
    }
    if (!requestIsCurrent()) return;
    const intelligence = projectIntelligenceForTalk(projectId, projectName, taskContextId, talkDocuments);
    ...
    if (contextualAnswer) {
      ...
      setTalkAnswer({
        projectId,
        projectName,
        question: requestTicket.identity.question,
        answer: contextualAnswer,
      });
      void persistTalkAnswer(
        projectId,
        projectName,
        requestTicket.identity.question,
        contextualAnswer,
        context,
      ).catch(error => {
  ...
  function confirmTalkTaskAction() {
    ...
    const actionProject = resolveECOSProjectSelection({
      projectId: talkTaskAction.projectId,
      projectName: talkTaskAction.projectName,
      projectRecords,
    });
    const currentTask = authoritativeScheduleItems.find(
      item => item.id === talkTaskAction.selectedTaskId,
    ) || null;
    const task = actionProject && currentTask
      ? scheduleTasksForParentProject(
          actionProject.name,
          [currentTask] as unknown as import('./types').ScheduleItem[],
          actionProject.id,
        )[0] as unknown as ScheduleItem | undefined
      : null;
    if (!task) {
      setTalkTaskAction(null);
      Alert.alert(
        'Task scope changed',
        'This task is no longer in the selected project. Refresh Talk and choose the task again.',
      );
      return;
    }
    const previous = {
    ...
    const actionProjectId = talkTaskAction.projectId;
    const actionProjectName = talkTaskAction.projectName;
    ...
    Alert.alert('Task updated', successMessage, [
      {
        ...
        style: 'cancel',
        onPress: () => {
          const latestTask = scheduleItemsCurrentRef.current.find(item => item.id === task.id);
          const stillInScope = latestTask && scheduleTasksForParentProject(
            actionProjectName,
            [latestTask] as unknown as import('./types').ScheduleItem[],
            actionProjectId,
          ).length === 1;
          if (!stillInScope) {
            Alert.alert(
              'Task scope changed',
              'This task is no longer in the selected project. Refresh before changing it.',
            );
            return;
          }
          updateScheduleItem(task.id, previous);
        },
      },
      { text: 'Done' },
    ]);
  ...
  const ecosQuestionContextProjectName = talkContextProjectForScreen(
    screen,
    selectedWorkspaceProject,
    selectedReportProjectNames[0] || null,
  );
  const ecosQuestionContextProject = resolveECOSProjectSelection({
    projectId: null,
    projectName: ecosQuestionContextProjectName,
    projectRecords,
  });
  const ecosProjectQuestion = useECOSProjectQuestionExperience({
    contextualProjectId: ecosQuestionContextProject?.id || null,
    contextualProjectName: ecosQuestionContextProject?.name || ecosQuestionContextProjectName,
    projectRecords,
    candidateProjects: reportAvailableProjectNames,
    onOpenEvidence: (projectId, projectName, evidence) => {
      if (evidence.sourceType === 'document' && evidence.documentCitation) {
        void ecosDocumentEvidence.openEvidence(evidence);
        return;
      }
      openTalkSupportingEvidence(projectId, projectName, evidence);
    },
  });
  ...
  const liveAuthorityInput = useMemo<PIELiveAuthorityInput>(() => {
    ...
    const contextualProjectId = authorityMode === 'capture' || authorityMode === 'capture-review'
      ? persistedExactProjectId(draft.projectId)
      : authorityMode === 'workspace'
        ? persistedExactProjectId(selectedWorkspaceCoverProject?.id)
        : dailyReportScope?.captureMemoryProjectIds.length === 1
          ? dailyReportScope.captureMemoryProjectIds[0]
          : null;
    const authorityProject = combinedReportScope
      ? null
      : resolveECOSProjectSelection({
    ...
    const scopedProjectId = combinedReportScope?.projectId || authorityProject?.id || null;
    const authorityEvidence = buildExactPIELiveAuthorityProjectEvidence({
      reportScope: reportEvidenceScope,
    ...
    return {
      organizationId: layer4Identity?.cloudTrusted
        ...
        : null,
      projectId: scopedProjectId,
      projectName: reportEvidenceScope?.projectName || authorityProject?.name || projectName,
      projectNames: reportEvidenceScope?.projectNames || (authorityProject ? [authorityProject.name] : []),
      reportType: authorityReportType,
      updates: authorityEvidence.updates as unknown as PIELiveAuthorityInput['updates'],
      scheduleItems: authorityEvidence.scheduleItems as unknown as PIELiveAuthorityInput['scheduleItems'],
      currentUpdate: authorityEvidence.currentUpdate as unknown as PIELiveAuthorityInput['currentUpdate'],
      projectAreas: authorityEvidence.projectAreas as unknown as PIELiveAuthorityInput['projectAreas'],
      contacts: (reportEvidenceScope ? reportEvidenceScope.contacts : contactBook) as unknown as PIELiveAuthorityInput['contacts'],
      referenceDocuments: authorityEvidence.referenceDocuments as unknown as PIELiveAuthorityInput['referenceDocuments'],
      projectDocuments: authorityEvidence.projectDocuments as unknown as PIELiveAuthorityInput['projectDocuments'],
      captureMemories: authorityEvidence.captureMemories,
      captureMemoryProjectIds: reportEvidenceScope
        ? reportEvidenceScope.captureMemoryProjectIds
        : authorityProject
          ? [authorityProject.id]
          : [],
      ...
      identityTrusted: Boolean(
        (combinedReportScope || authorityProject) &&
        layer4Identity?.cloudTrusted &&
        layer4Identity.organizationStatus === 'verified',
      ),
    ...
    authoritativeScheduleItems,
    selectedWorkspaceProject,
    selectedWorkspaceCoverProject,
  ]);

  const liveDetailUpdate = selectedDetailUpdate
    ? savedUpdates.find(item => item.id === selectedDetailUpdate.id) || selectedDetailUpdate
    : null;
  const selectedWorkspaceDocuments = projectDocumentsForExactWorkspace({
    projectId: selectedWorkspaceCoverProject?.id,
    projectDocuments,
    referenceDocuments,
  });
    ...
    <PIELiveAuthorityProvider input={liveAuthorityInput}>
      ...
      >
        <AppShellFrame
          ...
          askEcosActive={ecosProjectQuestion.active}
          ...
          documentCount={selectedWorkspaceDocuments.length}
        ...
        >
          ...
          {screen === 'ProjectWorkspace' && projectStatusReady && (
            <ProjectWorkspaceScreen
              contentStyle={contentStyle}
              projectId={selectedWorkspaceCoverProject?.id?.trim() || null}
              projectName={selectedWorkspaceProject}
              savedUpdates={activeSavedUpdates}
              captureMemories={captureMemories}
              projectWalkSession={
                activeProjectWalkSession?.projectId ===
                selectedWorkspaceCoverProject?.id
              ...
              scheduleItems={authoritativeScheduleItems}
              contactBook={contactBook}
              coverPhoto={coverPhotoForProject(projectRecords, selectedWorkspaceCoverProject?.id)}
              coverPhotoMode={selectedWorkspaceCoverProject?.coverPhotoMode || 'automatic'}
              ...
              onStartProjectWalk={() => selectedWorkspaceCoverProject?.id
                ? startProjectWalk(
                    selectedWorkspaceCoverProject.id,
                    selectedWorkspaceProject,
                  )
              ...
              onPrepareWalkUpdate={memories => {
                if (!selectedWorkspaceCoverProject?.id) {
                  Alert.alert(
                    'Exact project required',
                    'Choose a project with an immutable identity before preparing a Project Walk update.',
                  );
                  return;
                }
                prepareProjectWalkFieldUpdate(
          ...
          {screen === 'ProjectDocuments' && (
            <ProjectDocumentsScreen
              contentStyle={contentStyle}
              projectName={selectedWorkspaceProject}
              documents={selectedWorkspaceDocuments}
          ...
          {screen === 'Admin' && (
            <AdminScreen
              ...
              localProjectOptions={talkProjectOptions}
              ...
              documentUploadRequest.selected.size === 1
                ? referenceDocuments
                    .filter(document => {
                      ...
                      const projectId = persistedExactProjectId(
                        exactProjectRecordForCoverName(projectRecords, projectName)?.id,
                      );
          ...
          <DAVEVoiceCaptureSheet
            ...
            projectId={currentTalkProject?.id || null}
            projectName={currentTalkProject?.name || ''}
            candidateProjects={[]}
            candidateProjectOptions={talkProjectOptions}
            ...
            onProjectSelectionChange={project => {
              talkRequestAuthority.invalidate();
              setTalkProjectId(project.id);
              setTalkProjectName(project.name);
              setTalkTaskId(null);
            }}
            onTaskChange={taskId => {
              talkRequestAuthority.invalidate();
          ...
          <DAVEConversationAnswerSheet
            ...
            onOpenEvidence={citation => openTalkSupportingEvidence(
              talkAnswer?.projectId || talkProjectId || '',
...
function persistedExactProjectId(value: unknown): string | null {
...
function canonicalProjectId(value: unknown): string | null {
  return typeof value === 'string' &&
    value === value.trim() &&
    value === value.toLowerCase() &&
    isCanonicalProjectId(value)
    ? value
    : null;
}
...
function HomeScreen({
  ...
  function overviewPhotoForProject(projectName: string) {
    const coverProject = exactProjectRecordForCoverName(projectRecords, projectName);
    return resolveProjectCoverPhotoUri(
      projectRecords,
      coverProject?.id,
      mostRecentHeroPhotoUri(
        exactProjectUpdatesForCover(savedUpdates, coverProject?.id),
      ) || undefined,
    );
  }
...
function ProjectTaskControlPanel({
  ...
  const operationalScheduleItems = useMemo(
    () => operationalScheduleItemsForProject(projectName, scheduleItems, projectId),
    [projectId, projectName, scheduleItems],
  );
  const scopedFieldUpdates = useMemo(
    ...
    [projectId, projectName, savedUpdates, scheduleItems],
  );
...
function ProjectWorkspaceScreen({
  ...
  const scopedProjectDocuments = useMemo(
    () => projectDocumentsForScopes(projectScopeNames, projectDocuments, projectId),
    [projectId, projectScopeNames, projectDocuments],
  );
  ...
  const projectCaptureMemories = useMemo(
    () => captureMemories
      .filter(memory => Boolean(projectId && memory.projectId === projectId))
      .sort((left, right) => right.confirmedAt.localeCompare(left.confirmedAt)),
    [captureMemories, projectId],
  );
  const captureProjectOptions = useMemo<readonly DAVECaptureProjectOption[]>(
    () => projectId ? [{ id: projectId, name: projectName }] : [],
  ...
  async function startProjectWalkCapture() {
    if (!projectId) {
      Alert.alert(
        'Exact project required',
        'Choose a project with an immutable identity before recording Project Walk memories.',
      );
      return;
    }
    ...
    >
      ...
      <DAVEVoiceCaptureSheet
        ...
        onMemoryReady={result => {
          if (!projectId) {
            ...
            Alert.alert(
              'Exact project required',
              'This memory was not saved because the project identity is unavailable.',
            );
            return;
          }
          projectWalkLocationRequest.current += 1;
      ...
      {captureDraft ? (
        <DAVECaptureConfirmationSheet
          ...
          draft={captureDraft}
          projects={captureProjectOptions}
        ...
        <HoldToDeleteButton
          ...
          onConfirm={() => {
            if (!projectId) {
              Alert.alert(
                'Exact project required',
                'This project cannot be deleted until it has one immutable identity.',
              );
              return;
            }
...
function ProjectDocumentsScreen({
  ...
  const renderDocument = ({ item }: { item: ProjectDocument }) => {
    const sharedReferenceDocument = findSharedReferenceDocumentForProjectDocument(
      item,
      referenceDocuments,
    );
    return (
      <ProjectDocumentCard
        ...
        onSetCurrentSchedule={() => item.sharedReferenceOnly && sharedReferenceDocument
          ? onMakeCurrentDocument(sharedReferenceDocument.id)
          : onSetCurrentSchedule(item.id)}
        onMakeCurrentDocument={onMakeCurrentDocument}
        onRetry={() => onRetry(item.id)}
        onReplaceFile={() => onReplaceFile(item.id)}
        onDelete={() => onDelete(item.id)}
        editable={!item.sharedReferenceOnly}
      />
    );
  };
        ...
        inspector={selectedDocument ? (() => {
          ...
          return (
            <ProjectDocumentCard
              ...
              onSetCurrentSchedule={() => selectedDocument.sharedReferenceOnly && sharedReferenceDocument
                ? onMakeCurrentDocument(sharedReferenceDocument.id)
                : onSetCurrentSchedule(selectedDocument.id)}
              onMakeCurrentDocument={onMakeCurrentDocument}
              onRetry={() => onRetry(selectedDocument.id)}
              onReplaceFile={() => onReplaceFile(selectedDocument.id)}
              onDelete={() => onDelete(selectedDocument.id)}
              editable={!selectedDocument.sharedReferenceOnly}
            />
          );
        })() : emptyState}
      />
